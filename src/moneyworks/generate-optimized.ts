...
for (const file of files) {
  ...
  const dataTypes: Record<string, "string" | "number" | "boolean" | "integer" | "date" | "date-time"> = {};
  ...
  fs.writeFileSync(`./optimized/${file.replace(".json", ".ts")}`, `export default ${JSON.stringify(dataTypes, null, 2)};\n`);
}

...
export async function testCustomFormat() {
  try {
    ...
    console.log("=== Test 1: Custom format with Code/Description/Type fields ===");
    await testCustomWithManualParsing(api, tableName, ["Code", "Description", "Type"]);
    
    console.log("\n\n=== Test 2: Custom format with different fields ===");
    await testCustomWithManualParsing(api, tableName, ["Code", "BankAccountNumber"]);
...
async function testCustomWithManualParsing(
  ...
  try {
    ...
    while (position < rawData.length) {
      ...
      while (typeStart < rawData.length && 
             !["CA", "SA", "FA", "CL", "TL", "SF", "IN", "EX", "CS"].some(t => 
               rawData.substring(typeStart, typeStart + 2) === t)) {
        typeStart++;
      }

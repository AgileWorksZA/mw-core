...
import { LinkMany, LinkOne } from "../types/eden/Link";
...
export const linkRoutes = new Elysia({ prefix: "/api" })
  ...
  .get(
    "/links/for-record/:recordType/:recordId",
    async ({ params }) => {
      try {
        ...
        console.error(
          `Error in GET /links/for-record/${params.recordType}/${params.recordId}:`,
          error,
        );
        throw error;
      }

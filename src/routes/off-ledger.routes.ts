...
export const offLedgerRoutes = new Elysia({ prefix: "/api" })
  ...
  .get(
    "/off-ledger/:sequenceNumber",
    async ({ params }) => {
      try {
        const sequenceNumber = Number(params.sequenceNumber);
        return await offLedgerService.getOffLedgerEntryBySequenceNumber(
          sequenceNumber,
        );
      } catch (error) {
        console.error(
          `Error in GET /off-ledger/${params.sequenceNumber}:`,
          error,
        );
        throw error;
      }
  ...
  .get(
    "/off-ledger/for-account/:accountCode",
    async ({ params }) => {
      try {
        return await offLedgerService.getOffLedgerEntriesByAccount(
          params.accountCode,
        );
      } catch (error) {
        console.error(
          `Error in GET /off-ledger/for-account/${params.accountCode}:`,
          error,
        );
        throw error;
      }

...
import { MoneyWorksConfig } from '../types/moneyworks';
...
export function loadMoneyWorksConfig(): MoneyWorksConfig {
  ...
  if (process.env.MW_HOST && process.env.MW_PORT && process.env.MW_DATA_FILE) {
    const config: MoneyWorksConfig = {
      ...
      port: parseInt(process.env.MW_PORT, 10),
  ...
  try {
    ...
    if (!config.dataFile) throw new Error('Missing dataFile in MoneyWorks config');
    if (!config.username) throw new Error('Missing username in MoneyWorks config');
    if (!config.password) throw new Error('Missing password in MoneyWorks config');

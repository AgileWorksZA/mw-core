...
try {
  ...
  console.error("Make sure you've registered with /cloudios-register first");
  process.exit(1);
}
...
try {
  const endpoint = mode === "next" 
    ? `${CLOUDIOS_SERVER}/api/kanban/cards/next`
    : `${CLOUDIOS_SERVER}/api/kanban/cards/next?assigned_to=${agentName}`;
  ...
  if (!result.card) {
    ...
    if (mode !== "next") {
      console.log("Try: /cloudios-wip ${agentName} next");
    }
    process.exit(0);
  }

  // Move card to In Progress
  const moveResponse = await fetch(`${CLOUDIOS_SERVER}/api/kanban/cards/${result.card.id}/move`, {
    method: "POST",
...
2. Run: /cloudios-complete "${agentName}" ${card.id} "Your completion message"
`;

...
if (!agentName || !voiceName) {
  ...
  console.error("Example: change-voice.ts herman-cloudios Samantha");
  process.exit(1);
}
...
async function changeVoice() {
  try {
    ...
    const voicesResponse = await fetch(`${CLOUDIOS_SERVER}/api/voices`);
    ...
    const response = await fetch(`${CLOUDIOS_SERVER}/api/agents/${agentData.session_id}/voice`, {
      method: "PATCH",

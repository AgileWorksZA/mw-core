...
async function identifyAgent() {
  ...
  console.log(`🎯 Registering with CLOUDIOS at ${CLOUDIOS_SERVER}...`);
  console.log(`   Project: ${PROJECT_NAME}`);
  console.log(`   Agent: ${AGENT_NAME} (${AGENT_ROLE})`);

  try {
    const response = await fetch(CLOUDIOS_API_URL, {
      method: "POST",
      headers: {
    ...
    if (response.ok) {
      ...
      console.log(`✅ Successfully registered with CLOUDIOS!`);
    ...
    console.error(`   Is CLOUDIOS running at ${CLOUDIOS_SERVER}?`);
    return false;
  }
}

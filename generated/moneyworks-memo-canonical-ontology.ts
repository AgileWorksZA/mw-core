...
export const MONEYWORKS_MEMO_FIELDS = [
  {
    ...
    dataType: "N" as const,
    canonicalDescription: "Primary key - unique memo identifier",
  ...
  },
  {
    fieldName: "NameSeq",
    dataType: "N" as const,
    canonicalDescription: "The sequence number of the Name record to which the memo belongs",
    manualSource: "moneyworks_appendix_memo_file.html",
    isRequired: true,
    isIndexed: true,
 ...
 */
export function validateMemoDateCanonical(memoDate: Date, recallDate?: Date): {
  isValid: boolean;
  warnings: string[];
  ...
  if (recallDate) {
    ...
    const daysDifference = Math.ceil((recallDate.getTime() - memoDate.getTime()) / (1000 * 60 * 60 * 24));
    if (daysDifference === 0) {
      businessInsights.push("Same-day recall suggests immediate follow-up required");
    } else if (daysDifference > 365) {
      businessInsights.push("Recall date more than 1 year in future - confirm intentional long-term reminder");
    }
  }
...
export function validateMemoTextCanonical(text: string): {
  ...
  if (!text || text.trim().length === 0) {
    ...
    warnings.push(`Memo Text exceeds maximum length of 255 characters (current: ${text.length})`);
  }
  
  // Business optimization suggestions
  if (text && text.length > 200) {
    optimization.push("Consider breaking long memos into multiple records for better readability");
  }
  
  if (text && text.includes("TODO") || text.includes("FOLLOW UP")) {
...
export function validateMemoNameRelationshipCanonical(nameSeq: number): {
  ...
  return {
    isValid: warnings.length === 0,
    warnings,
    requirement: "NameSeq must reference valid Names.Seq (sequence number of existing Name record)"
  };
}
...
export function getCanonicalMemoEntityRelationships(): {
  ...
  return {
    requiredReferences: [
      "Names.Seq (NameSeq field - parent Name record)"
    ],
    optionalReferences: [],
    businessRules: [
...
export function getCanonicalMemoBusinessContext(): {
  ...
  return {
    purpose: "MoneyWorks Memo entity provides unlimited note-taking and reminder functionality for Name records, supporting comprehensive customer relationship management",

...
export function hasFlag(transaction: Transaction, flag: TransactionFlags): boolean {
...
export function validateTransaction(transaction: Partial<Transaction>): ValidationResult {
  ...
  // Enum validations
  if (transaction.Status && ![TransactionStatus.Unposted, TransactionStatus.Posted].includes(transaction.Status as TransactionStatus)) {
  ...
  if (transaction.Gross !== undefined && transaction.Gross < 0 && !isJournal(transaction as Transaction)) {
...
export function isCustomerInvoice(transaction: Partial<Transaction>): transaction is Partial<Transaction> & { Type: string } {
...
export function isSupplierInvoice(transaction: Partial<Transaction>): transaction is Partial<Transaction> & { Type: string } {
...
export function hasPaymentInfo(transaction: Partial<Transaction>): transaction is Partial<Transaction> & { PaymentMethod: number } {

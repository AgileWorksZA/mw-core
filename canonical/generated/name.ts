...
export function getCustomerTypeLabel(type: CustomerType): string {
  switch (type) {
    case CustomerType.NotCustomer: return "Not a Customer";
    case CustomerType.Customer: return "Customer";
    case CustomerType.Debtor: return "Debtor";
...
export function getSupplierTypeLabel(type: SupplierType): string {
  switch (type) {
    case SupplierType.NotSupplier: return "Not a Supplier";
    case SupplierType.Supplier: return "Supplier"; 
    case SupplierType.Creditor: return "Creditor";
    default: return "Unknown";
  }
}
...
export function validateName(name: Partial<Name>): ValidationResult {
  ...
  if (name.Name && name.Name.length > 255) {
    errors.push({ field: "Name", message: "Name cannot exceed 255 characters", value: name.Name });
  }

  if (name.Address1 && name.Address1.length > 59) {
    errors.push({ field: "Address1", message: "Address1 cannot exceed 59 characters", value: name.Address1 });
  }

  // Enum validations
  if (name.CustomerType !== undefined && !Object.values(CustomerType).includes(name.CustomerType)) {
    errors.push({ field: "CustomerType", message: "Invalid CustomerType", value: name.CustomerType });
  }

  if (name.SupplierType !== undefined && !Object.values(SupplierType).includes(name.SupplierType)) {
    errors.push({ field: "SupplierType", message: "Invalid SupplierType", value: name.SupplierType });
  }

  if (name.ProductPricing && !Object.values(ProductPricingLevel).includes(name.ProductPricing)) {
    errors.push({ field: "ProductPricing", message: "Invalid ProductPricing level", value: name.ProductPricing });
  }
...
export function hasCustomerData(name: Partial<Name>): name is Partial<Name> & { CustomerType: CustomerType } {
  return name.CustomerType !== undefined && name.CustomerType !== CustomerType.NotCustomer;
}

/** Type guard to check if a name has supplier fields */
export function hasSupplierData(name: Partial<Name>): name is Partial<Name> & { SupplierType: SupplierType } {
  return name.SupplierType !== undefined && name.SupplierType !== SupplierType.NotSupplier;
} 
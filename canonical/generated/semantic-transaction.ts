...
export interface MoneyWorksSemanticMCP {
  ...
  execute_workflow(intent: BusinessIntent, context: any): Promise<WorkflowResult>;
...
export function parseBusinessIntent(naturalLanguage: string): BusinessIntent {
  ...
  if (lower.includes("invoice") || lower.includes("bill")) {
    if (lower.includes("customer") || lower.includes("client") || lower.includes("unpaid")) {
...
export function generateBusinessStory(transaction: SemanticTransaction): string {
...
export function determineTransactionState(transaction: SemanticTransaction): TransactionState {
  ...
  if (transaction.intent === BusinessIntent.CUSTOMER_BILLING || 
      transaction.intent === BusinessIntent.SUPPLIER_INVOICING) {
    
    if (transaction.amounts.outstanding <= 0) return TransactionState.PAID;
    if (transaction.amounts.outstanding < transaction.amounts.gross) return TransactionState.PARTIALLY_PAID;
    if (transaction.dates.due && new Date() > transaction.dates.due) return TransactionState.OVERDUE;
  }
...
export function generateSemanticTags(transaction: SemanticTransaction): SemanticTag[] {
  ...
  if (transaction.party.type === MoneyWorksEntityType.CUSTOMER) {
    tags.push({ category: "party", tag: "customer_transaction", confidence: 1.0 });
  }
...
export function upgradeToSemantic(legacy: any): SemanticTransaction {
  ...
  const intent = determineBusinessIntentFromType(legacy.Type, legacy.Description || "");
  ...
  return {
    ...
    intent: intent,
    state: legacy.Status === "P" ? TransactionState.POSTED : TransactionState.DRAFT,
...
export function enrichBusinessContext(transaction: SemanticTransaction): BusinessContext {
...
export function determineBusinessIntentFromType(type: string, description: string): BusinessIntent {
  switch (type) {
    case "CP":
    case "CPC":
      return BusinessIntent.SUPPLIER_PAYMENT;

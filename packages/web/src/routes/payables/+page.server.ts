...
export const load: PageServerLoad = async ({ locals }) => {
	...
	try {
		txRes = await apiGet<ApiResponse<TransactionRecord[]>>('/tables/transaction', {
			token,
			filter: 'left(Type,2)="CI" AND Status="P"',
			limit: 2000
		});
	} catch {
	...
	return {
		today,
		invoices,
		summary: { totalOutstanding, invoiceCount: invoices.length, overdueCount, overdueAmount, aging }
	};
};

function emptySummary() {
	return {
		totalOutstanding: 0, invoiceCount: 0, overdueCount: 0, overdueAmount: 0,

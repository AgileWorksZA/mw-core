...
<div class="flex h-full flex-col">
	...
	<div class="flex-1 overflow-auto p-6">
				...
				<table class="w-full text-sm">
					<thead class="sticky top-0">
						<tr class="border-b border-border bg-muted/50">
							...
							<th class="px-3 py-2.5 text-left font-medium text-muted-foreground">Supplier Ref</th>
					...
					<tbody>
						{#each data.orders as o}
							<tr class="border-b border-border last:border-0 hover:bg-muted/50">
								...
								<td class="px-3 py-2 font-mono text-xs text-muted-foreground">{o.supplierRef}</td>
		...
		{:else}
			<div class="flex h-40 items-center justify-center text-muted-foreground">No purchase orders found</div>
		{/if}
	</div>
</div>

<script lang="ts">
	...
	const columns = [
		...
		{ key: 'supplierRef', label: 'Supplier Ref', mono: true, class: 'text-muted-foreground' },
...
<div class="flex h-full flex-col">
	<PageHeader title="Purchase Orders" subtitle="{data.summary.total} orders">
		<div class="flex items-center gap-3">
			...
			<a href="/orders/purchases/new" class="rounded-xl bg-primary px-4 py-1.5 text-sm font-medium text-primary-foreground hover:bg-primary/90">+ New PO</a>
		</div>
	</PageHeader>

	<div class="flex-1 overflow-auto p-6">
		<DataTable {columns} rows={data.orders} rowHref={(row) => `/transactions/${row.seq}`} emptyMessage="No purchase orders found">

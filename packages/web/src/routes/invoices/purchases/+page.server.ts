...
export const load: PageServerLoad = async ({ locals, url }) => {
	...
	const filters: string[] = ['left(Type,2)="CI"'];
	...
	try {
		txRes = await apiGet<ApiResponse<TransactionRecord[]>>('/tables/transaction', {
			token, filter: filters.join(' AND '), limit: 500
		});
	} catch {
	...
	const invoices = all.map((t) => {
		...
		return {
			...
			description: t.Description ?? '', date: t.Transdate ?? '', dueDate,
			period: t.Period ?? 0, gross, paid, outstanding, status: t.Status ?? '',
	...
	return {
		today, invoices, status,
		summary: {
			total: invoices.length, posted, unposted: invoices.length - posted,
			totalGross: invoices.reduce((s, i) => s + i.gross, 0),

<script lang="ts">
	...
	let activeTab = $state<'monthly' | 'graph' | 'invoices' | 'orders'>('monthly');
	...
	const totalPurchases = $derived(data.invoices.reduce((sum: number, i: any) => sum + i.gross, 0));
	...
	const summaryCards = $derived([
		...
		{ label: 'Bills', value: data.invoices.length },
...
<div class="flex h-full flex-col">
	<PageHeader
		...
		subtitle={data.supplier ? `${data.supplier.name} (${data.supplier.code})` : ''}
	>
	...
	{:else}
		...
		<div class="border-b border-border px-6">
			<div class="flex gap-0">
				{#each (['monthly', 'graph', 'invoices', 'orders'] as const) as tab}
					<button
					...
					>
						{tab === 'invoices' ? 'Bills' : tab.charAt(0).toUpperCase() + tab.slice(1)}
					</button>
				{/each}
			</div>
		</div>
		...
		<div class="flex-1 overflow-auto p-6">
			...
			{#if activeTab === 'monthly'}
				{#if data.monthly.length === 0}
					<p class="text-muted-foreground text-sm">No purchase data available</p>
				{:else}
						...
						<table class="w-full text-sm">
							<thead class="sticky top-0">
								<tr class="bg-surface-container-low">
									...
									<th class="px-4 py-2.5 text-right font-medium text-muted-foreground">Purchase Value</th>
								</tr>
							</thead>
							<tbody>
								...
								<tr class="bg-surface-container-low font-bold">
									...
									<td class="px-4 py-2.5 text-right"><CurrencyDisplay amount={totalPurchases} /></td>
								</tr>
							</tbody>
						</table>
					</div>
			...
			{:else if activeTab === 'graph'}
				<div class="rounded-xl bg-surface-container-lowest p-6">
					<h3 class="mb-4 text-sm font-semibold text-muted-foreground uppercase tracking-wider">
						Purchases by Period — {data.supplier.name}
					</h3>
						...
						<svg viewBox="0 0 {chartW} {chartH}" class="w-full max-w-3xl">
							...
							{#each data.monthly as m, i}
								...
								<rect
									...
									fill="var(--color-chart-6)" opacity="0.7" rx="2"
								/>
					...
					<table class="w-full text-sm">
						...
						<tbody>
							{#if data.invoices.length === 0}
								<tr><td colspan="7" class="px-3 py-8 text-center text-muted-foreground">No bills found</td></tr>
							{:else}
					...
					<table class="w-full text-sm">
						...
						<tbody>
							{#if data.orders.length === 0}
								<tr><td colspan="6" class="px-3 py-8 text-center text-muted-foreground">No open purchase orders</td></tr>
							{:else}

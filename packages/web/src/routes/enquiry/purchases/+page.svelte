...
<div class="flex h-full flex-col">
	...
	<PageHeader title="Purchase Enquiry" subtitle={data.supplier ? `${data.supplier.name} (${data.supplier.code})` : ''}>
		<form onsubmit={(e) => { e.preventDefault(); search(); }} class="flex gap-2">
			<input type="text" placeholder="Supplier code..." bind:value={nameInput} class="w-48 rounded-md border border-input bg-background px-3 py-1.5 text-sm focus:outline-none focus:ring-2 focus:ring-ring" />

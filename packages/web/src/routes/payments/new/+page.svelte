<script lang="ts">
	...
	const hasDetailData = $derived(
		...
		(isSupplierMode && allocationTotal > 0)
	);
	const isValid = $derived(
		bankAccount !== '' &&
		amount > 0 &&
		(isSupplierMode ? nameCode !== '' && allocationTotal > 0 && !allocationError : hasDetailData)
	);
	...
	function onItemSelect(index: number) {
		...
		if (prod) {
			...
			line.unitPrice = prod.buyPrice || prod.sellPrice;
			line.unit = prod.unit;
	...
	async function handleSubmit() {
		...
		try {
			...
			const lines = !isSupplierMode && activeTab === 'by-account'
				? detailLines.filter((l) => l.account && l.gross > 0)
				: [];

			const items = !isSupplierMode && activeTab === 'by-item'
		...
		<div class="mx-auto max-w-4xl space-y-6">
			...
			<div class="flex items-center justify-between rounded-xl bg-surface-container-lowest p-5">
				<div class="text-sm text-muted-foreground">
					...
					{:else if !isSupplierMode && (detailTotal > 0 || itemTotal > 0)}

...
<div class="flex h-full flex-col">
	<div class="bg-surface-container-lowest px-6 py-4">
		<h1 class="font-headline text-xl font-bold">Banking</h1>
	...
	<div class="flex-1 overflow-auto p-6 space-y-8">
		...
		<div class="grid grid-cols-2 gap-8">
				...
				{#if data.recentReceipts.length > 0}
					<div class="space-y-1">
						{#each data.recentReceipts as r}
							<div class="flex items-center justify-between rounded-xl px-3 py-2 text-sm hover:bg-surface-container-low transition-colors">
								<div class="flex items-center gap-3">
									<span class="text-xs text-muted-foreground">{r.date}</span>
									<span>{r.name}</span>
								</div>
								<span class="font-semibold text-positive"><CurrencyDisplay amount={r.amount} /></span>
							</div>
						{/each}
					</div>
				{:else}
			...
			<!-- Recent Payments -->
			<div>
				<h2 class="font-headline mb-3 text-sm font-semibold text-muted-foreground uppercase tracking-wider">Recent Payments</h2>

...
const registry: Record<string, Adapter<any, any, any>> = {};
...
export function registerAdapter<TContext, I, O>(
  ...
  if (config.metadata?.category && !isValidCategory(config.metadata.category)) {
    console.warn(`Invalid category '${config.metadata.category}' for adapter ${config.type}`);
  }

  // Register the adapter
  registry[config.type] = { ...config, ...logic } as Adapter<any, any, any>;
...
export function getAdapter<TContext, I = any, O = any>(
  type: string,
...
export function useAdapter<TContext, I = any, O = any>(
  adapter?: string,
): Adapter<TContext, I, O> {

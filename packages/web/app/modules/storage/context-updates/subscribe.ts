 ...
 * unsubscribe();
 */
export function subscribeToContextUpdates(options: ContextSubscriptionOptions): () => void {
  const { type, id, sessionId, onUpdate, onError, onConnect, onDisconnect } = options;

  // Validate required parameters
  if (!type || !id || !sessionId) {
    console.error("[storage] subscribeToContextUpdates: type, id, and sessionId are required");
    return () => {}; // Return no-op unsubscribe
  }

  // Create EventSource connection to our new SSE endpoint
  const eventSource = new EventSource(`/api/context-updates/${type}/${id}/${sessionId}`);
 ...
 * subscription.start();
 */
export function createManagedContextSubscription(options: ContextSubscriptionOptions) {

 ...
 */
export function validateTaxRateValue(rate, fieldName) {
    const warnings = [];
    if (rate < 0) {
 ...
 */
export function validateTaxAccountReferences(paidAccount, recAccount) {
    const warnings = [];
...
export function validateTaxRate(taxRate) {
    ...
    if (taxRate.PaidAccount !== undefined || taxRate.RecAccount !== undefined) {
        const accountValidation = validateTaxAccountReferences(taxRate.PaidAccount || '', taxRate.RecAccount || '');
 ...
 */
export function validateRequiredFields(taxRate) {
    const warnings = [];

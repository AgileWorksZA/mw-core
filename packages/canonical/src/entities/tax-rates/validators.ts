...
import type { MoneyWorksValidationResult } from "../../common/data-types";
import type { MoneyWorksTaxRate } from "./types";
 ...
 */
export function validateTaxRateValue(
	rate: number,
	fieldName: "Rate1" | "Rate2" | "CombineRate1" | "CombineRate2",
	...
	if (decimalPlaces > 4) {
		warnings.push(
			`${fieldName} should not exceed 4 decimal places in MoneyWorks`,
		);
	}
...
export function validateTaxAccountReferences(
	...
	if (paidAccount && paidAccount.length > 7) {
		warnings.push(
			"PaidAccount exceeds MoneyWorks maximum length of 7 characters",
		);
	}

	if (recAccount && recAccount.length > 7) {
		warnings.push(
			"RecAccount exceeds MoneyWorks maximum length of 7 characters",
		);
	}

	if (paidAccount === recAccount) {
		warnings.push(
			"PaidAccount and RecAccount should be different in MoneyWorks",
		);
	}
	...
	if (paidAccount && !accountPattern.test(paidAccount)) {
		warnings.push(
			"PaidAccount should follow MoneyWorks format (e.g., '2-1520')",
		);
	}

	if (recAccount && !accountPattern.test(recAccount)) {
		warnings.push(
			"RecAccount should follow MoneyWorks format (e.g., '2-1510')",
		);
	}
...
export function validateTaxRate(
	taxRate: Partial<MoneyWorksTaxRate>,
): MoneyWorksValidationResult {
	const warnings: string[] = [];
	...
	if (taxRate.CombineRate1 !== undefined) {
		const combineRate1Validation = validateTaxRateValue(
			taxRate.CombineRate1,
			"CombineRate1",
		);
	...
	if (taxRate.CombineRate2 !== undefined) {
		const combineRate2Validation = validateTaxRateValue(
			taxRate.CombineRate2,
			"CombineRate2",
		);
	...
	if (taxRate.GSTPaid !== undefined || taxRate.GSTReceived !== undefined) {
		warnings.push(
			"GSTPaid and GSTReceived are set by MoneyWorks finalisation process",
		);
	}

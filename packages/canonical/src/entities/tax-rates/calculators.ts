...
import { type YYYYMMDD, d } from "@moneyworks/utilities";
...
export function getCurrentTaxRate(
	rate1: number,
	rate2: number,
	changeoverDate: YYYYMMDD,
	currentDate: YYYYMMDD = d`${new Date()}`,
): {
	currentRate: number;
...
export function calculateCombinedTax(
	...
	if (isTaxInclusive) {
		...
		const totalRate = calculateTotalEffectiveRate(
			primaryRate,
			secondaryRate,
			combineMode,
		);
	...
	return {
		...
		explanation:
			`MoneyWorks tax calculation using ${modeDescriptions[combineMode]} Combine mode: ` +
 ...
 */
export function calculateTaxForRate(
	taxRate: MoneyWorksTaxRate,
	amount: number,
	options: {
		date?: YYYYMMDD;
	...
	const { currentRate: primaryRate, ratePeriod } = getCurrentTaxRate(
		taxRate.Rate1,
		taxRate.Rate2,
		taxRate.Date,
		currentDate,
	...
	// Calculate tax
	const calculation = calculateCombinedTax(
		amount,
		primaryRate,
		secondaryRate,
		taxRate.Combine || MoneyWorksTaxCombineMode.NONE,
		options.isTaxInclusive || false,
	);
 ...
 */
function calculateTotalEffectiveRate(
	primaryRate: number,
	secondaryRate: number,
	combineMode: MoneyWorksTaxCombineMode,
): number {
 ...
 */
export function calculateGSTFinalization(
	transactions: Array<{
		amount: number;
		tax: number;
		type: "paid" | "received";
	...
	return {
		...
		explanation:
			`MoneyWorks GST finalisation: GSTPaid ${GSTPaid.toFixed(2)}, ` +

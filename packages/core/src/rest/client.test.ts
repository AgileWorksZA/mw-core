...
describe('MoneyWorks REST Client', () => {
  ...
  describe('Authentication', () => {
    ...
    test('builds two-level auth headers', () => {
      ...
      expect(headers.Authorization).toBe('Basic dGVzdHVzZXI6dGVzdHBhc3M=');
      expect(headers['Authorization-Folder']).toBe('Basic Zm9sZGVyOmZvbGRlcnBhc3M=');
    });
  });
  ...
  describe('URL Building', () => {
    test('builds REST URL without params', () => {
      ...
      expect(url).toBe('http://localhost:6710/REST/testuser:testpass@test.moneyworks/export');
    });
    ...
    test('builds REST URL with params', () => {
      ...
      expect(url).toBe('http://localhost:6710/REST/testuser:testpass@test.moneyworks/export?table=Name&format=xml-verbose');
    });
  ...
  describe('Config Validation', () => {
    ...
    test('throws on missing required field', () => {
      ...
      expect(() => validateConfig(invalidConfig)).toThrow('Missing required configuration field: username');
    });
    ...
    test('validates data file extension', () => {
      ...
      expect(() => validateConfig(invalidFile)).toThrow('Invalid data file extension');
    });
    
    test('validates folder auth completeness', () => {
      const incompleteFolder = { ...testConfig, folderName: 'folder' };
      
      expect(() => validateConfig(incompleteFolder)).toThrow('Both folderName and folderPassword must be provided');
    });
  });

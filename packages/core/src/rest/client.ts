...
import { REST_ENDPOINTS, DEFAULT_CONFIG, MoneyWorksErrorCode } from './types';
...
export class MoneyWorksRESTClient {
  ...
  constructor(config: MoneyWorksConfig) {
    ...
    if (this.config.debug) {
      console.log('MoneyWorks REST Client initialized:', maskConfig(this.config));
    }
  }
  ...
  async export<T extends TableName>(
    ...
    const format = options.format || 'xml-verbose';
    ...
    if (typeof format === 'string' && (format === 'xml-terse' || format === 'xml-verbose')) {
      return ExportParser.parseXML<T>(data, table, format);
    }
  ...
  async *exportStream<T extends TableName>(
    ...
    while (hasMore) {
      ...
      if (offset === 0 && batch.length > 0) {
        try {
          const countResult = await this.evaluate(`Count(${table}, ${options.filter || 'true'})`);
          total = parseInt(countResult);
        } catch {
          // Ignore count errors
        }
      }
      ...
      if (options.onProgress) {
        options.onProgress({
          ...
          total,
          percentage: total ? ((offset + batch.length) / total) * 100 : undefined
        });
      }
   ...
   */
  private addCustomFormatParams(params: URLSearchParams, format: ExportFormat): void {
   ...
   */
  private parseImportResult(responseText: string, recordCount: number): ImportResult {
    ...
    if (createdMatch) {
      result.created = parseInt(createdMatch[1]);
    }
    if (updatedMatch) {
      result.updated = parseInt(updatedMatch[1]);
    }
   ...
   */
  private async parseResponse<T = unknown>(response: Response): Promise<RESTResponse<T>> {

...
import { type XMLBuildOptions, XMLBuilder } from "../xml/builder";
...
export class ImportBuilder<T extends TableName> {
  ...
  validate(): string[] {
    ...
    this.records.forEach((record, index) => {
      const recordErrors = XMLBuilder.validateRecord(this.table, record);
      errors.push(
        ...recordErrors.map((err: string) => `Record ${index + 1}: ${err}`),
      );
    });

...
export const accountTypeSchema = z.enum(
  ...
  {
    description:
      "Account type code. IN=Income, EX=Expense, CA=Current Asset, CL=Current Liability, etc.",
  },
);
...
export const friendlyAccountTypeSchema = z.enum(
  ...
  {
    description:
      "Friendly account type names. Use 'income', 'expense', 'current_asset', etc.",
  },
);
...
export const accountOperationSchema = z
  .discriminatedUnion("operation", [
    ...
    z.object({
      operation: z
        .literal("list")
        .describe("List accounts. Use filters to narrow results."),
      ...paginationSchema.shape,
      ...sortSchema.shape,
      ...fieldSelectionSchema.shape,
      ...statusFilterSchema.shape,
      accountType: friendlyAccountTypeSchema
        .optional()
        .describe(
          "Filter by account type (e.g., 'income', 'expense', 'current_asset')",
        ),
      withBalance: z
        .boolean()
        .optional()
    ...
    z.object({
      operation: z
        .literal("search")
        .describe("Search accounts by text in code, description, or comment"),
    ...
    z.object({
      operation: z
        .literal("count")
        .describe("Count accounts matching criteria"),
      ...statusFilterSchema.shape,
      accountType: friendlyAccountTypeSchema.optional(),
      withBalance: z.boolean().optional(),
    ...
    z.object({
      operation: z
        .literal("export")
        .describe("Export accounts to various formats"),
    ...
    z.object({
      operation: z
        .literal("types")
        .describe("Get information about account types"),
    }),
    ...
    z.object({
      operation: z
        .literal("hierarchy")
        .describe("Get account hierarchy (parent-child relationships)"),
      parentCode: z
        .string()
        .optional()
  ...
  .describe(
    createOperationDescription("Account", {
...
export const searchAccountsSchema = z
  .object({
    query: z
      .string()
      .describe(
        "Search text. Use '*' for all. Searches code, description, and comments.",
      ),
    accountType: friendlyAccountTypeSchema.optional(),
    limit: z.number().min(1).max(1000).default(100),
  })

      ...
      <div className="p-4 space-y-3">
        ...
        <div className="grid grid-cols-2 gap-4">
          <div>
            ...
            <div className="font-semibold text-lg">{formatPercentage(data.TaxRate)}</div>
          </div>
          <div>
        ...
        {(data.TaxRateReduced !== undefined || data.TaxRateForeign !== undefined) && (
          <div className="grid grid-cols-2 gap-4">
            {data.TaxRateReduced !== undefined && (
              <div>
                <div className="text-sm text-gray-600">Reduced Rate</div>
                <div className="font-semibold">{formatPercentage(data.TaxRateReduced)}</div>
              </div>
            )}
            {data.TaxRateForeign !== undefined && (
              <div>
                <div className="text-sm text-gray-600">Foreign Rate</div>
                <div className="font-semibold">{formatPercentage(data.TaxRateForeign)}</div>
              </div>
            )}
          </div>
        )}
          ...
          <div className="text-sm space-y-1">
            <div className="flex justify-between">
              <span className="text-gray-600">
                {isInclusive ? 'Gross Amount (incl. tax):' : 'Net Amount (excl. tax):'}
              </span>
            ...
            <div className="flex justify-between">
              <span className="text-gray-600">Tax Amount ({formatPercentage(data.TaxRate)}):</span>

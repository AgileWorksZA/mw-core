...
export function ReportDisplay({ data }: ReportDisplayProps) {
	...
	if (
		data.reportType === "aged_receivables" ||
		data.reportType === "aged_payables"
	) {
		...
		const totalAmount = Object.values(typedData.summary).reduce(
			(sum: number, bucket: { count: number; total: number }) =>
				sum + bucket.total,
			0,
		);
			...
			<div className="bg-white rounded-lg border overflow-hidden">
				...
				<div className="p-4">
					...
					<div className="space-y-3">
						{Object.entries(typedData.summary).map(
							([key, bucket]: [string, { count: number; total: number }]) => {
								const percentage =
									totalAmount > 0 ? (bucket.total / totalAmount) * 100 : 0;
					...
					{data.summary.over90.total > 0 && (
						<div className="mt-4 p-3 bg-red-50 rounded-lg flex items-center gap-2">
							...
							<span className="text-sm text-red-800">
								{formatCurrency(data.summary.over90.total)} is over 90 days
								overdue
							</span>
						</div>
					)}

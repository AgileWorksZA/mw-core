...
export function createMoneyWorksTools(
	...
	console.log(
		"[TOOLS] Creating MoneyWorks tools with client:",
	...
	return {
		...
		[MoneyWorksTools.GET_TAX_RATE]: tool({
			...
			execute: async ({ taxCode }) => {
				try {
					...
					throw new Error(
						`Failed to fetch tax rate: ${error instanceof Error ? error.message : "Unknown error"}`,
					);
				}
			},
		}),
		...
		[MoneyWorksTools.LIST_TAX_RATES]: tool({
			...
			execute: async ({ limit }) => {
				try {
					...
					throw new Error(
						`Failed to list tax rates: ${error instanceof Error ? error.message : "Unknown error"}`,
					);
				}
			},
		}),
		...
		[MoneyWorksTools.SEARCH_NAMES]: tool({
			...
			execute: async ({ query, type, limit }) => {
				...
				try {
					console.log("[SEARCH_NAMES] Starting search with:", {
						query,
						type,
						limit,
						filterQuery,
					...
					throw new Error(
						`Failed to search names: ${error instanceof Error ? error.message : "Unknown error"}`,
					);
				}
			},
		}),
		...
		[MoneyWorksTools.RUN_REPORT]: tool({
			description:
				"Run financial reports like aged receivables, P&L, balance sheet",
			...
			execute: async (args) => {
				...
				try {
					switch (args.reportType) {
						...
						default:
							throw new Error(
								`Report type ${args.reportType} not yet implemented`,
							);
					}
				} catch (error) {
					throw new Error(
		...
		[MoneyWorksTools.GET_COMPANY_INFO]: tool({
			...
			execute: async () => {
				try {
					const { CompanyInformationRepository } = await import(
						"@moneyworks/data"
					);
					...
					return {
						...
						summary: {
							...
							tax: {
								...
								gstCycle: companyData.GSTCycleMonths
									? `${companyData.GSTCycleMonths} months`
									: undefined,
							},
							accounting: {
					...
					throw new Error(
						`Failed to fetch company information: ${error instanceof Error ? error.message : "Unknown error"}`,
					);
				}
			},
		}),

...
async function testCompleteFlow() {
	...
	try {
		...
		if (finalUrl.includes("/connections/new")) {
			console.log(
				"✅ Correctly redirected to create connection (no connections exist)\n",
			);
			console.log("=== TEST PASSED ===");
			console.log("Authentication flow is working correctly!");
			console.log("- Unauthenticated users are redirected to sign-in");
			console.log(
				"- Authenticated users without connections are redirected to create one",
			);
		} else if (finalUrl.includes("/chat/")) {
			console.log(
				"✅ Successfully redirected to chat session (user has connections)\n",
			);
			console.log("=== TEST PASSED ===");

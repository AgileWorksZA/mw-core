...
async function testFullChatFlow() {
	...
	try {
		...
		if (currentUrl.includes("/sign-in")) {
			...
			const password =
				process.env.TEST_USER_PASSWORD || "the-sun-shines-0n-NARNIA!";
		...
		if (
			pageContent.includes("No connections") ||
			pageContent.includes("Create your first connection")
		) {
			console.log("   No connections found. Creating one...");

			// Click create connection button
			const createButton = await page.$(
				'a[href="/connections/new"], button:has-text("Create Connection"), button:has-text("Add Connection")',
			);
			if (createButton) {
				await createButton.click();
			} else {
				await page.goto("http://localhost:5173/connections/new", {
					waitUntil: "networkidle0",
				});
			}
			...
			// Connection name
			await page.waitForSelector('input[name="connection_name"]', {
				timeout: 5000,
			});
		...
		if (!chatUrl.includes("/chat/")) {
			...
			await page.screenshot({
				path: "chat-navigation-error.png",
				fullPage: true,
			});
			return;
		}
		...
		await page.waitForSelector(
			'textarea[placeholder*="Ask"], textarea[placeholder*="Type"], input[placeholder*="message"]',
			{ timeout: 10000 },
		);
			...
			await page.waitForFunction(
				() => {
					const messages = document.querySelectorAll(
						'[role="article"], .message, .chat-message',
					);

...
describe("MoneyWorks NOW Integration", () => {
  ...
  beforeEach(async () => {
    // Clean up test data
    await db.prepare("DELETE FROM mw_connections WHERE clerk_user_id = ?").run(testUserId);
    await db.prepare("DELETE FROM mw_now_accounts WHERE clerk_user_id = ?").run(testUserId);
  });
  ...
  describe("NOW Account Service", () => {
    ...
    test("should delete NOW account and associated connections", async () => {
      ...
      // Delete account
      const deleted = await nowAccountService.deleteNowAccount(testUserId, account.id);
      ...
      const connections = await connectionService.getConnectionsByUser(testUserId);
  ...
  describe("Connection Service - NOW Support", () => {
    ...
    test("should retrieve connections by NOW account", async () => {
      ...
      expect(nowConnections.every(c => c.connection_type === "now")).toBe(true);
      expect(nowConnections.every(c => c.mw_now_account_id === account.id)).toBe(true);
    });
  });

...
export function ConnectionProvider({ children }: { children: ReactNode }) {
	...
	const [currentConnection, setCurrentConnection] =
		useState<MWConnection | null>(null);
	...
	const refreshConnections = useCallback(async () => {
		if (!userId || !isSignedIn) {
			console.log(
				"[ConnectionContext] No userId or not signed in, clearing connections",
			);
			setConnections([]);
		...
		try {
			...
			if (!response.ok) {
				...
				throw new Error(
					`Failed to load connections: ${response.status} ${errorText}`,
				);
			}
			...
			console.log(
				"[ConnectionContext] Loaded connections:",
			...
			if (savedConnectionId) {
				const savedConnection = userConnections.find(
					(c: MWConnection) => c.id === savedConnectionId,
				);
				if (savedConnection) {
					...
					const defaultConnection =
						userConnections.find((c: MWConnection) => c.is_default) ||
						userConnections[0];

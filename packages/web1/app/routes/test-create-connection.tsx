...
export default function TestCreateConnection() {
  ...
  const createTestConnection = async () => {
    ...
    try {
      ...
      if (response.ok) {
        setResult(`Success! Connection created with ID: ${data.connection?.id}`);
      } else {
  ...
  const clearConnections = async () => {
    ...
    try {
      ...
      setResult("To clear connections, run: bun run scripts/clear-connections.ts " + testUserId);
    } finally {
      setLoading(false);
    }
  };
      ...
      <Card className="max-w-2xl mx-auto">
        ...
        <CardContent className="space-y-4">
          <p className="text-sm text-gray-600">
            This page bypasses authentication to test the connection creation directly.
            Using test user ID: <code className="bg-gray-100 px-1">{testUserId}</code>
          </p>
          ...
          {result && (
            <div className={`p-4 rounded border ${
              result.includes("Success") ? "bg-green-50 border-green-300" : "bg-red-50 border-red-300"
            }`}>
          ...
          <div className="pt-4 border-t">
            ...
            <div className="flex gap-2">
              <Button onClick={() => navigate("/test-auth")} variant="outline" size="sm">
                Auth Debug
              </Button>

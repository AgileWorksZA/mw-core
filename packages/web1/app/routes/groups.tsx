...
import { Plus, Users, Building2, FolderTree, Settings } from "lucide-react";
...
import type { CompanyGroup, CompanyGroupMember, MWConnection } from "~/db/schema";
...
export async function loader({ request }: LoaderFunctionArgs) {
  try {
    ...
    let groupMembers: Array<CompanyGroupMember & { connection: MWConnection }> | undefined;
    
    if (selectedGroupId) {
      // Find selected group in hierarchy
      const findGroup = (groups: GroupWithChildren[]): CompanyGroup | undefined => {
      ...
      if (selectedGroup) {
        ...
        const membersWithConnections = await Promise.all(
          members.map(async (member) => {
            const connection = await connectionService.getConnection(userId, member.connection_id);
...
function GroupsContent() {
  ...
  const { organizations, groups, selectedGroup, groupMembers, currentOrganizationId } = useLoaderData<typeof loader>();
              ...
              <Card>
                ...
                <CardContent>
                  <div className="space-y-4">
                    <div>
                      <h3 className="font-semibold mb-2">Companies in this group</h3>
                          ...
                          {groupMembers.map((member) => (
                            ...
                            >
                              <div className="flex items-center gap-3">
                                ...
                                <div>
                                  <p className="font-medium">{member.connection.connection_name}</p>
                                  <p className="text-sm text-muted-foreground">
                                    {member.connection.mw_data_file} • {member.connection.mw_host}
                                  </p>
                                </div>
                              </div>
                              {member.company_code && (
                    ...
                    <div>
                      ...
                      <div className="grid gap-2 sm:grid-cols-2">
                        <Button variant="outline" size="sm" asChild>
                          <Link to={`/groups/${selectedGroup.id}/tax-comparison`}>
                            Compare Tax Codes
                          </Link>
                        </Button>
                        <Button variant="outline" size="sm">

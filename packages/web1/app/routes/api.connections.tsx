...
export async function loader({ request }: LoaderFunctionArgs) {
	...
	try {
		...
		console.log(
			"[API Connections Loader] Found connections:",
			connections.length,
		);
		return json({ connections });
	} catch (error) {
		console.error(
			"[API Connections Loader] Failed to load connections:",
			error,
		);
		return json({ connections: [] });
	}
		...
		switch (action) {
			case "create": {
				const data = {
					...
					mw_folder_name:
						(formData.get("mw_folder_name") as string) || undefined,
					mw_folder_password:
						(formData.get("mw_folder_password") as string) || undefined,
					mw_data_file: formData.get("mw_data_file") as string,
					mw_host: formData.get("mw_host") as string,
					mw_port: Number.parseInt(formData.get("mw_port") as string),
				...
				console.log("[API] Creating connection with data:", {
					...
					mw_folder_password: data.mw_folder_password
						? "[REDACTED]"
						: undefined,
				});

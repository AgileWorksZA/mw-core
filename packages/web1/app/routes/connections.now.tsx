...
import type { MWConnection, MWNowAccount } from "~/db/schema";
...
export default function NowAccountsPage() {
	...
	async function loadAccounts() {
		...
		try {
			...
			const accountsResponse = await fetch(
				`/api/now-accounts?userId=${userId}`,
			);
			...
			const accountsWithConnections = accountsData.accounts.map(
	...
	async function handleDelete(accountId: string) {
		if (
			!confirm(
				"Are you sure? This will delete all connections from this NOW account.",
			)
		) {
			return;
		}

...
export default function ChatSessionPage() {
  const { chatContext, sessionId, session, initialMessages, sessions } = useLoaderData<typeof loader>();
  const navigate = useNavigate();
  const [currentMessages, setCurrentMessages] = useState<MoneyWorksMessage[]>(initialMessages);
  ...
  const handleClearHistory = async () => {
    if (!confirm(`Clear all ${sessions.length} chat sessions? This action cannot be undone.`)) {
      return;
    }

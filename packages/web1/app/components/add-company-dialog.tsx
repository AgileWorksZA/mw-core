		...
		if (open && userId) {
			fetch(`/api/connections?userId=${userId}`)
				...
				.then((data) => {
					...
					console.log(
						"[AddCompanyDialog] Available connections after filtering:",
				...
				<form onSubmit={handleSubmit}>
					...
					<div className="py-4">
						...
						{selectedConnections.size > 0 && (
							<div className="mt-4 text-sm text-muted-foreground">
								{selectedConnections.size} company
								{selectedConnections.size !== 1 ? "ies" : "y"} selected
							</div>
						)}
					</div>
					...
					<DialogFooter>
						...
						>
							{loading
								? "Adding..."
								: `Add ${selectedConnections.size} Company${selectedConnections.size !== 1 ? "ies" : ""}`}
						</Button>
					</DialogFooter>
				</form>
			</DialogContent>

...
class GroupService {
   ...
   */
  async getGroupsByOrganization(organizationId: string): Promise<CompanyGroup[]> {
    const db = await getDatabaseAsync();
    const stmt = db.prepare(`
  ...
  async getGroupHierarchy(organizationId: string): Promise<CompanyGroup[]> {
    ...
    // Build hierarchy
    const groupMap = new Map<string, CompanyGroup & { children?: CompanyGroup[] }>();
   ...
   */
  async addGroupMember(input: AddGroupMemberInput): Promise<CompanyGroupMember> {
    const db = await getDatabaseAsync();
    const id = crypto.randomUUID();
    const now = new Date().toISOString();
   ...
   */
  async acceptInvitation(token: string, userId: string): Promise<GroupPermission | null> {
    const db = await getDatabaseAsync();
   ...
   */
  async getUserPermission(groupId: string, userId: string): Promise<GroupPermission | null> {
    const db = await getDatabaseAsync();
    const stmt = db.prepare(`

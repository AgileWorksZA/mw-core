...
export class ConnectionService {
  ...
  async getConnection(userId: string, connectionId: string): Promise<MWConnection | null> {
    ...
    return {
      ...
      mw_folder_name: connection.mw_folder_name ? crypto.decrypt(connection.mw_folder_name) : undefined,
      mw_folder_password: connection.mw_folder_password ? crypto.decrypt(connection.mw_folder_password) : undefined,
  ...
  async createConnection(input: CreateConnectionInput): Promise<MWConnection> {
    ...
    const encryptedData = {
      ...
      mw_folder_name: input.mw_folder_name ? crypto.encrypt(input.mw_folder_name) : null,
      mw_folder_password: input.mw_folder_password ? crypto.encrypt(input.mw_folder_password) : null,
  ...
  async updateConnection(userId: string, input: UpdateConnectionInput): Promise<MWConnection | null> {
    const db = await this.getDb();
    ...
    if (input.mw_folder_name !== undefined) {
      ...
      values.push(input.mw_folder_name ? crypto.encrypt(input.mw_folder_name) : null);
    }
    
    if (input.mw_folder_password !== undefined) {
      updates.push("mw_folder_password = ?");
      values.push(input.mw_folder_password ? crypto.encrypt(input.mw_folder_password) : null);
    }
  ...
  async testConnection(connection: MWConnection): Promise<{ success: boolean; error?: string }> {
    try {
      ...
      try {
        ...
        console.log("[ConnectionService] Test successful, got result:", testResult);
      } catch (testError) {

...
export class ChatService {
	...
	static async createSession(
      ...
      `).run(id, connectionId, userId, sessionTitle, now, now, 0);
			console.log(
				`[ChatService] Created session ${id} for connection ${connectionId}`,
			);
		} catch (error) {
	...
	static async saveMessage(
		...
		const toolInvocations = message.toolInvocations
			...
			: null;
		const mwData = message.mwData ? JSON.stringify(message.mwData) : null;
    ...
    `).run(
		...
		if (session) {
			await ChatService.updateSessionActivity(
				sessionId,
    ...
    `)
		...
		return messages.map((msg) => ({
			...
			toolInvocations: msg.tool_invocations
				...
				: undefined,
			mwData: msg.mw_data ? JSON.parse(msg.mw_data) : undefined,
		}));
	}
	...
	static async getOrCreateSession(
		...
		return await ChatService.createSession(connectionId, userId);
	}
}

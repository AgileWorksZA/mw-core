...
import {
	type MoneyWorksNOWAuthResponse,
	MoneyWorksNOWClient,
	type MoneyWorksNOWFile,
...
export class NowAccountService {
    ...
    `);
		...
		return accounts.map((account) => ({
			...
			mw_now_token: account.mw_now_token
				? crypto.decrypt(account.mw_now_token)
				: undefined,
			mw_now_refresh_token: account.mw_now_refresh_token
				? crypto.decrypt(account.mw_now_refresh_token)
				: undefined,
		}));
	}
	...
	async updateNowAccount(
		userId: string,
		input: UpdateNowAccountInput,
	): Promise<MWNowAccount | null> {
		const db = await this.getDb();
		...
		if (input.mw_now_token !== undefined) {
			...
			values.push(
				input.mw_now_token ? crypto.encrypt(input.mw_now_token) : null,
			);
		}
		...
		if (input.mw_now_refresh_token !== undefined) {
			...
			values.push(
				input.mw_now_refresh_token
					? crypto.encrypt(input.mw_now_refresh_token)
					: null,
			);
		}
	...
	async authenticateAndGetFiles(
		account: MWNowAccount,
	): Promise<MoneyWorksNOWAuthResponse> {

	...
	decrypt(encryptedData: string): string {
		try {
			...
			if (combined.length < minLength) {
				throw new Error(
					`Invalid encrypted data: too short (${combined.length} bytes, need at least ${minLength})`,
				);
			}
			...
			console.error(
				"[CRYPTO] Decryption failed for data:",
...
export function createCryptoService(userId: string): CryptoService {
	...
	if (!masterKey) {
		throw new Error(
			"No encryption key found. Set ENCRYPTION_KEY or CLERK_SECRET_KEY",
		);
	}

...
async function testChatSuppliers() {
	...
	try {
		...
		if (currentUrl.includes("/chat/")) {
			...
			try {
				...
				if (messageInput) {
					...
					await page.keyboard.type(
						"Can you list all suppliers in the MoneyWorks system?",
					);
					...
					while (Date.now() - startTime < responseTimeout && !foundResponse) {
						...
						const messages = await page.$$eval(
							'[role="article"], .message, .chat-message, [data-message], div[class*="message"]',
							(elements) =>
								elements.map((el) => ({
					...
					await page.screenshot({
						path: "chat-suppliers-result.png",
						fullPage: true,
					});
			...
			await page.screenshot({
				path: "unexpected-chat-redirect.png",
				fullPage: true,
			});
		}
	} catch (error) {

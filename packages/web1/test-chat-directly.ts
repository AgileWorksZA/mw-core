...
async function testChatDirectly() {
  ...
  try {
    ...
    const messageInput = await page.$('textarea') || await page.$('input[type="text"][placeholder*="message" i], input[type="text"][placeholder*="ask" i], input[type="text"][placeholder*="type" i]');
    ...
    while (Date.now() - startTime < 30000 && !foundResponse) {
      const messages = await page.$$eval('div[role="article"], .message, .chat-message, [data-role="message"]', elements => 
        elements.map(el => ({
      ...
      for (const msg of lastFewMessages) {
        if (msg.text.toLowerCase().includes('supplier') && 
            !msg.text.includes('Please list all suppliers')) {
          foundResponse = true;
          console.log("✅ AI Response received!");
          console.log("Response preview:", msg.text.substring(0, 200) + "...\n");
          break;
        }
      }
    ...
    if (!foundResponse) {
      ...
      const errorElements = await page.$$eval('.error, [role="alert"], .text-destructive', elements =>

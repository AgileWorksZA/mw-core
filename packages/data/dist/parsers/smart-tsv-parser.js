 ...
 */
export function parseTSVWithStructure(tsvData, structure) {
 ...
 */
export function validateTSVStructure(tsvData, structure) {
    const warnings = [];
    const lines = tsvData.trim().split('\n');
    if (lines.length === 0) {
    ...
    if (firstLineFields.length !== expectedFieldCount) {
        warnings.push(`Field count mismatch: expected ${expectedFieldCount}, got ${firstLineFields.length}`);
    }

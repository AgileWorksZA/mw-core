...
export const COMPANY_INFORMATION_FIELDS: (keyof CompanyInformation)[] = [
  "Name", "Address1", "Address2", "Address3", "Address4", "State", "PostCode",
  "Delivery1", "Delivery2", "Delivery3", "Delivery4", "DeliveryState", "DeliveryPostCode",
  "Phone", "Fax", "Email", "WebURL", "Mobile", "TaxName", "GstNum", "RegNum", "RemittanceMessage",
  "Have_Logo", "BaseCurrency", "Locale", "LocaleFriendlyName", "Version", "LogFilePath",
  "PeriodsInYear", "CurrentPer", "AgingCycle", "PeriodNames", "GstCycleEndDate", "GSTCycleMonths",
  "GstCycleNum", "GstIncomeInvoiceBasis", "CoRegName", "ExtendedJobCosting", "GSTExpensesInvoiceBasis",
  "GSTIncomeInvoiceBasis", "GSTProcessingSuppressed", "GSTGuideName", "GSTRegName", "LastAgedDebtors",
  "LastBackup", "LastStocktake", "MultiCurrencyEnabled", "NavigatorActive", "NetworkLatency",
...
export class CompanyInformationRepository {
  ...
  private parseFieldValue(field: keyof CompanyInformation, value: string): any {
    // Boolean fields
    if ([
      "Have_Logo", "ExtendedJobCosting", "GSTExpensesInvoiceBasis", "GSTIncomeInvoiceBasis", 
      "GSTProcessingSuppressed", "MultiCurrencyEnabled", "NavigatorActive", "GstIncomeInvoiceBasis"
    ...
    // Numeric fields
    if ([
      "PeriodsInYear", "CurrentPer", "AgingCycle", "GSTCycleMonths", "GstCycleNum", 
      "NetworkLatency", "StartupSequenceNumJob"
    ].includes(field as string)) {
   ...
   */
  async getBasicDetails(): Promise<Pick<CompanyInformation, 'Name' | 'Address1' | 'Address2' | 'Address3' | 'Address4' | 'Phone' | 'Email'>> {
    return this.getCompanyInformation([
      'Name', 'Address1', 'Address2', 'Address3', 'Address4', 'Phone', 'Email'
    ]);
  }
   ...
   */
  async getTaxInformation(): Promise<Pick<CompanyInformation, 'TaxName' | 'GstNum' | 'RegNum' | 'GSTCycleMonths' | 'GstCycleEndDate'>> {
    return this.getCompanyInformation([
   ...
   */
  async getSystemInformation(): Promise<Pick<CompanyInformation, 'Version' | 'Locale' | 'LocaleFriendlyName' | 'BaseCurrency' | 'MultiCurrencyEnabled'>> {
    return this.getCompanyInformation([
      'Version', 'Locale', 'LocaleFriendlyName', 'BaseCurrency', 'MultiCurrencyEnabled'
    ]);
  }
}
...
import { type AccountCode, type YYYYMMDD, d } from "@moneyworks/utilities";
...
> {
	 ...
	 */
	protected prepare(
		data: MoneyWorksTaxRateCreateInput | MoneyWorksTaxRateUpdateInput,
	): any {
		const prepared: any = {};
	 ...
	 */
	async findActive(
		asOfDate: YYYYMMDD = d`${new Date()}`,
	): Promise<MoneyWorksTaxRate[]> {
		// Get all tax rates

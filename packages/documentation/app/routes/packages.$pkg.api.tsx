...
async function highlightApiData(
	api: any,
	theme: "light" | "dark",
): Promise<any> {
	if (!api) return api;
		...
		for (const exportGroup of highlighted.exports) {
			if (exportGroup.items) {
				for (const item of exportGroup.items) {
					if (item.example) {
						item.highlightedExample = await highlightCode(
							item.example,
							"typescript",
							theme,
						);
	...
	if (highlighted.usage?.examples) {
		for (const example of highlighted.usage.examples) {
			if (example.code) {
				example.highlightedCode = await highlightCode(
					example.code,
					"typescript",
					theme,
				);
			...
			<div className="flex gap-8">
				...
				<div className="flex-1 min-w-0 space-y-12">
					...
					{highlightedApi.exports?.map((exportGroup: any) => (
						...
						>
							<h2 className="text-3xl font-bold mb-2 scroll-mt-20">
								{exportGroup.name}
							</h2>
					...
					{highlightedApi.usage?.examples?.length > 0 && (
						<section id="usage-examples">
							<h2 className="text-3xl font-bold mb-6 scroll-mt-20">
								Usage Examples
							</h2>
							<div className="space-y-6">
								{highlightedApi.usage.examples.map(
					...
					{/* Import Patterns */}
					{apiData.usage?.importPatterns &&
						apiData.usage.importPatterns.length > 0 && (
							<section>
								<h2 className="text-2xl font-bold mb-4">Import Patterns</h2>
								<div className="space-y-4">
									{apiData.usage?.importPatterns?.map(
							...
							<div className="space-y-2">
								{apiData.futurePlans.map((plan: any, index: number) => (
									<div key={index} className="flex items-start gap-2">
										<Badge
											variant={
												plan.status === "Planned" ? "secondary" : "default"
											}
										>
											{plan.status}
										</Badge>

...
export const mdxComponents: MDXComponents = {
	...
	PackageInfo: ({
		...
		github,
		version,
	}: { npm: string; github: string; version: string }) => (
	...
	Tabs: ({
		...
		const { TabsList, TabsTrigger, TabsContent } =
			extractTabComponents(children);
		return (
			<div className="my-6">
				<div className="flex gap-2 border-b">{TabsList}</div>
				<div className="mt-4">{TabsContent}</div>
			</div>
		);
	},
};
...
export async function processMarkdownContent(
	...
	processedContent = processedContent.replace(
		...
		(match, infoString, code) => {
			...
			const highlightPromise = highlightCode(
				code.trim(),
				meta.language,
				theme,
			).then((html) => {
...
export function MDXContent({
	content,
	processedHtml,
}: { content?: string; processedHtml?: string }) {

...
async function generatePackageContent(packageName: string, info: PackageInfo) {
	...
	try {
		...
		console.log(
			`  ✓ Extracted ${apiData.functions.length} functions, ${apiData.types.length} types, ${apiData.constants.length} constants`,
		);
	} catch (error) {
...
${apiData.types
	.slice(0, 5)
	.map(
		(t) => `- **${t.name}** (${t.kind}) - ${t.description || "No description"}`,
	)
	.join("\n")}

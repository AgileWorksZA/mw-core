...
const createMockAccountService = () => ({
  async getData(options: any) {
    ...
    const mockAccounts = [
      { Code: "1000", Description: "Cash at Bank", Type: "A", Category: "CASH", CurrentBalance: 25000.00 },
      { Code: "1100", Description: "Accounts Receivable", Type: "A", Category: "RECV", CurrentBalance: 15000.00 },
      { Code: "4000", Description: "Sales Revenue", Type: "I", Category: "SALES", CurrentBalance: 0.00 },
      { Code: "5000", Description: "Cost of Goods Sold", Type: "E", Category: "COGS", CurrentBalance: 0.00 },
      { Code: "6000", Description: "Operating Expenses", Type: "E", Category: "OPEX", CurrentBalance: 0.00 },
    ];
    ...
    if (options.search) {
      if (options.search.Code) {
        filteredAccounts = mockAccounts.filter(acc => 
          acc.Code.includes(options.search.Code) || 
          acc.Description.toLowerCase().includes(options.search.Code.toLowerCase())
        );
      }
      if (options.search.Type) {
        filteredAccounts = filteredAccounts.filter(acc => acc.Type === options.search.Type);
      }
    }
...
const createMockTransactionService = () => ({
  async getData(options: any) {
    ...
    if (options.search) {
      if (options.search.Type) {
        filteredTransactions = filteredTransactions.filter(t => t.Type === options.search.Type);
      }
      if (options.search.Status) {
        filteredTransactions = filteredTransactions.filter(t => t.Status === options.search.Status);
      }
      if (options.search.NameCode) {
        filteredTransactions = filteredTransactions.filter(t => t.NameCode === options.search.NameCode);
      }
      if (options.search.SequenceNumber) {
        filteredTransactions = filteredTransactions.filter(t => t.SequenceNumber === options.search.SequenceNumber);
      }
      if (options.search.OurRef) {
        filteredTransactions = filteredTransactions.filter(t => t.OurRef === options.search.OurRef);
      }
    }
    ...
    if (options.search) {
      if (options.search.Code) {
        filteredContacts = filteredContacts.filter(c => 
          ...
          c.Description.toLowerCase().includes(options.search.Code.toLowerCase())
        );
      }
      if (options.search.Type) {
        filteredContacts = filteredContacts.filter(c => c.Type === options.search.Type);
      }
      if (options.search.Description) {
        filteredContacts = filteredContacts.filter(c => 
          c.Description.toLowerCase().includes(options.search.Description.toLowerCase())
        );
      }
    }
...
const tools = {
  searchAccounts: {
    ...
    schema: {
      ...
      type: z.enum(["IN", "SA", "EX", "CS", "CA", "CL", "FA", "TA", "TL", "SF"]).optional(),
  ...
  listAccountFields: {
    ...
    async execute() {
      return {
        fields: ["Code", "Description", "Type", "Category", "CurrentBalance", "OpeningBalance"],
  ...
  searchTransactions: {
    ...
    schema: {
      query: z.string().optional(),
      type: z.enum(["SI", "SC", "SR", "SD", "PI", "PC", "PP", "PD", "JN", "JC", "BR", "BP", "BT", "ST", "SO", "PO"]).optional(),
  ...
  listTransactionFields: {
    ...
    async execute() {
      return {
        fields: ["SequenceNumber", "Type", "Status", "OurRef", "NameCode", "TransDate", "Gross", "AmtPaid", "Period"],
  ...
  searchContacts: {
    ...
    async execute(args: any) {
      ...
      if (args.isActive !== undefined) {
        filteredData = filteredData.filter((contact: any) => contact.IsActive === args.isActive);
      }
  ...
  listContactFields: {
    ...
    async execute() {
      return {
        fields: ["Code", "Description", "Type", "Address", "Phone", "Email", "IsActive"],
...
async function runSingleTest(testName: string, toolName: string, args: any, description: string) {
  console.log(`\n${"=".repeat(80)}`);
...
async function runWorkflowTest(workflowName: string, workflow: any[]) {
  ...
  for (const step of workflow) {
    ...
    if (step.note && step.note.includes("from step")) {
...
async function main() {
  ...
  try {
    if (args.length === 0 || args.includes("--all")) {
      ...
      for (const [categoryName, category] of Object.entries(sampleQueries.categories)) {
        const results = await runTestCategory(categoryName, category);
        allResults.push(...results);
      }
      
      // Run workflow tests
      for (const [workflowName, workflow] of Object.entries(sampleQueries.workflow_examples)) {
      ...
      for (const categoryName of args) {
        if (sampleQueries.categories[categoryName]) {
          const results = await runTestCategory(categoryName, sampleQueries.categories[categoryName]);
          allResults.push(...results);
        } else if (sampleQueries.workflow_examples[categoryName]) {
          const results = await runWorkflowTest(categoryName, sampleQueries.workflow_examples[categoryName]);
          allResults.push(...results);
        } else {

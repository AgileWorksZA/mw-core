...
describe("Contact Tools", () => {
	describe("searchContactsTool", () => {
		...
		it("should validate input schema", () => {
			...
			expect(() =>
				searchContactsTool.inputSchema.parse(validArgs),
			).not.toThrow();
		});
	...
	describe("Type mapping and validation", () => {
		it("should correctly map contact types", () => {
			...
			expect(() =>
				searchContactsTool.inputSchema.parse(customerArgs),
			).not.toThrow();

			// Supplier
			const supplierArgs = { type: "S" as const, limit: 50, offset: 0 };
			expect(() =>
				searchContactsTool.inputSchema.parse(supplierArgs),
			).not.toThrow();

			// Employee
			const employeeArgs = { type: "E" as const, limit: 50, offset: 0 };
			expect(() =>
				searchContactsTool.inputSchema.parse(employeeArgs),
			).not.toThrow();

			// Both (Customer and Supplier)
			const bothArgs = { type: "B" as const, limit: 50, offset: 0 };
			expect(() =>
				searchContactsTool.inputSchema.parse(bothArgs),
			).not.toThrow();
		});
	});

...
import { z } from "zod";
import { setupTestDatabase, cleanupTestDatabase, getTestTicketService, MOCK_RESPONSES, createMockService } from "../setup";

// Mock all services
mock.module("@moneyworks/api/src/services/tables/account.service", () => ({
  AccountService: function() {
    return createMockService(MOCK_RESPONSES.accounts.success);
  }
}));

mock.module("@moneyworks/api/src/services/tables/transaction.service", () => ({
  TransactionService: function() {
    return createMockService(MOCK_RESPONSES.transactions.success);
  }
}));

mock.module("@moneyworks/api/src/services/tables/contact.service", () => ({
  ContactService: function() {
    return createMockService(MOCK_RESPONSES.contacts.success);
  }
}));
...
describe("MCP Server Integration", () => {
    ...
    it("should register searchAccounts tool correctly", () => {
      server.tool(
        ...
        async (args) => {
          return {
            content: [
              { type: "text" as const, text: JSON.stringify({ accounts: [], total: 0, ...args }, null, 2) },
            ],
          };
        },
      );
    ...
    it("should register getAccount tool correctly", () => {
      server.tool(
        ...
        async (args) => {
          return {
            content: [
              { type: "text" as const, text: JSON.stringify({ Code: args.code }, null, 2) },
            ],
          };
        },
      );
    ...
    it("should register transaction tools correctly", () => {
      server.tool(
        ...
        async (args) => {
          return {
            content: [
              { type: "text" as const, text: JSON.stringify({ transactions: [], total: 0, ...args }, null, 2) },
            ],
          };
        },
      );
  ...
  describe("Tool Execution", () => {
    beforeEach(() => {
      ...
      server.tool(
        ...
        async (args) => {
          ...
          return {
            content: [
              { type: "text" as const, text: JSON.stringify(mockResult, null, 2) },
            ],
          };
        },
      );
      ...
      server.tool(
        ...
        async (args) => {
          const account = MOCK_RESPONSES.accounts.success.data.find(acc => acc.Code === args.code);
          if (!account) {
    ...
    it("should execute search tool successfully", async () => {
      ...
      const result = await server["toolHandlers"].get("testSearchAccounts")?.handler(args, {});
    ...
    it("should execute get tool successfully", async () => {
      ...
      const result = await server["toolHandlers"].get("testGetAccount")?.handler(args, {});
  ...
  describe("Error Handling and Ticket Creation", () => {
    it("should handle API errors gracefully", async () => {
      ...
      // Test 404 error
      await expect(
        toolHandler.handler({ errorCode: 404 }, {})
      ).rejects.toThrow("Resource not found");

      // Test 500 error
      await expect(
        toolHandler.handler({ errorCode: 500 }, {})
    ...
    it("should handle multiple concurrent tool executions", async () => {
      server.tool(
        ...
        async (args) => {
          ...
          return {
            content: [
              { type: "text" as const, text: JSON.stringify({ completed: args.identifier }, null, 2) },
            ],
          };
        },
      );
    ...
    it("should support chained tool operations", async () => {
      ...
      server.tool(
        ...
        async (args) => {
          return {
            content: [
              { type: "text" as const, text: JSON.stringify({ customers: [{ code: "CUST001" }] }, null, 2) },
            ],
          };
        },
      );
      ...
      server.tool(
        ...
        async (args) => {
          return {
            content: [
              { type: "text" as const, text: JSON.stringify({ transactions: [{ ref: "INV-001", amount: 1000 }] }, null, 2) },
            ],
          };
        },
      );
      ...
      const transactionHandler = server["toolHandlers"].get("testGetCustomerTransactions");
      ...
      const transactionResult = await transactionHandler.handler({ customerCode }, {});

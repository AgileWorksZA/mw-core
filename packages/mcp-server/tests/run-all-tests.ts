...
class TestRunner {
  ...
  async runAllTests(): Promise<TestSummary> {
    ...
    await this.runTestSuite(
      ...
      "bun",
      ["run", "tests/samples/manual-test-runner.ts", "accounts", "transactions"],
      { timeout: 180000 } // 3 minutes
    );
  ...
  private printSummary(summary: TestSummary) {
    ...
    console.log(`\n⚡ Performance Insights:`);
    console.log(`  Fastest: ${fastestSuite.suite} (${(fastestSuite.duration / 1000).toFixed(1)}s)`);
    console.log(`  Slowest: ${slowestSuite.suite} (${(slowestSuite.duration / 1000).toFixed(1)}s)`);
    console.log(`  Average: ${(summary.totalDuration / summary.totalSuites / 1000).toFixed(1)}s per suite`);
...
async function checkPrerequisites(): Promise<boolean> {
  ...
  const testDirs = ["tests/unit", "tests/integration", "tests/validation", "tests/performance", "tests/samples"];

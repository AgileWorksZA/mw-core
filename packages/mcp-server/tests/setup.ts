  ...
  return {
    async getData(options: any) {
      ...
      if (options.search && Object.keys(options.search).length > 0) {
        ...
        const filtered = mockData.data.filter((item: any) => 
          item[searchKey] === searchValue || 
          item[searchKey]?.toString().includes(searchValue?.toString())
        );
        
        return {
          data: filtered.slice(options.offset || 0, (options.offset || 0) + (options.limit || 50)),
          pagination: {
      ...
      return {
        data: mockData.data.slice(options.offset || 0, (options.offset || 0) + (options.limit || 50)),
        pagination: {

...
class BenchmarkRunner {
	...
	private async runScenario(
		...
		const successRate =
			((testIterations - errors.length) / testIterations) * 100;
	...
	private printScenarioResults(result: BenchmarkResult) {
		const status =
			result.successRate >= 99 ? "✅" : result.successRate >= 95 ? "⚠️" : "❌";
		const performanceStatus =
			result.avgDuration <= this.config.targetResponseTimeMs ? "⚡" : "🐌";
		...
		console.log(
			`   P95/P99:      ${result.p95Duration.toFixed(2)}ms / ${result.p99Duration.toFixed(2)}ms`,
		);
		console.log(`   Throughput:   ${result.throughput.toFixed(1)} req/s`);
		console.log(
			`   Memory:       ${result.memoryUsage.avg.toFixed(1)}MB avg, ${result.memoryUsage.max.toFixed(1)}MB peak`,
		);

		if (result.errors.length > 0) {
			console.log(
				`   Errors:       ${result.errors.length} (${result.errors[0]})`,
			);
		}
	}
	...
	async runConcurrencyTest(
		...
		const concurrencyResult: BenchmarkResult = {
			...
			memoryUsage: {
				avg:
					this.systemMetrics.reduce((sum, m) => sum + m.memoryUsage, 0) /
					this.systemMetrics.length,
				max: Math.max(...this.systemMetrics.map((m) => m.memoryUsage)),
				final:
					this.systemMetrics[this.systemMetrics.length - 1]?.memoryUsage || 0,
			},
			errors: allRequests
				.filter((r) => !r.success)
				.map((r) => r.error)
	...
	async runStressTest(
		...
		console.log(
			`   Successful: ${successfulResults.length} (${((successfulResults.length / completedResults.length) * 100).toFixed(1)}%)`,
		);
		console.log(`   Failed: ${failedResults.length}`);
		console.log(
			`   Avg Load: ${(completedResults.length / (durationMs / 1000)).toFixed(1)} req/s`,
		);
		console.log(
			`   Peak Memory: ${Math.max(...this.systemMetrics.map((m) => m.memoryUsage)).toFixed(1)}MB`,
		);

		if (failedResults.length > 0) {
			console.log(
				`   Sample Errors: ${failedResults
					.slice(0, 3)
					.map((r) => r.error)
					.join(", ")}`,
			);
		...
		const memoryExceeded =
			Math.max(...this.systemMetrics.map((m) => m.memoryUsage)) >
			this.config.memoryLimitMB;
		const successRate =
			(successfulResults.length / completedResults.length) * 100;

		console.log(
			`   Status: ${successRate >= 90 && !memoryExceeded ? "✅ PASS" : "❌ FAIL"}`,
		);
	...
	generateReport() {
		...
		const toolStats = this.results.reduce(
			(acc, result) => {
				if (!acc[result.tool]) {
					acc[result.tool] = {
						scenarios: 0,
		...
		for (const [tool, stats] of Object.entries(toolStats)) {
			...
			const performanceGrade =
				stats.avgDuration <= this.config.targetResponseTimeMs
					? "A"
					: stats.avgDuration <= this.config.targetResponseTimeMs * 1.5
						? "B"
						: stats.avgDuration <= this.config.targetResponseTimeMs * 2
							? "C"
							: "F";
		...
		for (const result of this.results) {
			...
			console.log(
				`     ${result.avgDuration.toFixed(2)}ms avg (P95: ${result.p95Duration.toFixed(2)}ms)`,
			);
			console.log(
				`     ${result.successRate.toFixed(1)}% success, ${result.throughput.toFixed(1)} req/s`,
			);
...
const createBenchmarkTool = (name: string, baseDelay = 50) => {
	return async (args: any) => {
		...
		const mockData = Array(responseSize)
			.fill(null)
			.map((_, i) => ({
				id: `${name}_${i}`,
				data: `mock_data_${i}`,
...
async function main() {
	...
	// Concurrency tests
	await runner.runConcurrencyTest("searchAccounts", tools.searchAccounts, {
		query: "test",
		limit: 20,
	});
	await runner.runConcurrencyTest(
		"searchTransactions",

...
class PerformanceMonitor {
	...
	getStats() {
		...
		const avgMemory =
			this.metrics.reduce((sum, m) => sum + m.memoryUsage, 0) / totalRequests;
...
const createPerformanceTestTool = (name: string, baseDelay = 50) => {
...
describe("Performance and Load Tests", () => {
	...
	describe("Response Time Tests", () => {
		it("should meet target response times for account search", async () => {
			...
			expect(stats.avgDuration).toBeLessThan(
				PERFORMANCE_CONFIG.TARGET_RESPONSE_TIME_MS,
			);
			expect(stats.maxDuration).toBeLessThan(
		...
		it("should meet target response times for transaction search", async () => {
			...
			expect(results).toHaveLength(iterations);
			expect(stats.avgDuration).toBeLessThan(
				PERFORMANCE_CONFIG.TARGET_RESPONSE_TIME_MS * 1.5,
			);
			expect(stats.maxDuration).toBeLessThan(
				PERFORMANCE_CONFIG.MAX_RESPONSE_TIME_MS,
			);
		...
		it("should handle single record lookups efficiently", async () => {
			...
			const promises = Array(iterations)
				.fill(null)
				.map((_, i) =>
					performanceTools.getAccount({
						code: `ACC${i.toString().padStart(3, "0")}`,
					}),
				);
			...
			expect(results).toHaveLength(iterations);
			expect(stats.avgDuration).toBeLessThan(
				PERFORMANCE_CONFIG.TARGET_RESPONSE_TIME_MS * 0.5,
			);
			expect(stats.maxDuration).toBeLessThan(
				PERFORMANCE_CONFIG.TARGET_RESPONSE_TIME_MS,
			);
	...
	describe("Concurrent Load Tests", () => {
		it("should handle concurrent account searches", async () => {
			...
			const userSessions = Array(concurrentUsers)
				.fill(null)
				.map(async (_, userId) => {
			...
			expect(stats.avgDuration).toBeLessThan(
				PERFORMANCE_CONFIG.TARGET_RESPONSE_TIME_MS * 2,
			);
		...
		it("should handle mixed tool usage under load", async () => {
			...
			while (Date.now() - startTime < testDuration) {
				...
				if (toolChoice < 0.4) {
					...
					// 20% contact searches
					requests.push(
						performanceTools.searchContacts({
							type: "C",
	...
	describe("Resource Usage Tests", () => {
		it("should not exceed memory limits under sustained load", async () => {
			...
			const largeDataRequests = Array(iterations)
				.fill(null)
				.map(async (_, i) => {
		...
		it("should handle pagination efficiently", async () => {
			...
			const concurrentPages = Array(totalPages)
				.fill(null)
				.map((_, page) =>
					performanceTools.searchAccounts({
						limit: pageSize,
	...
	describe("Stress Tests", () => {
		...
		it("should recover from temporary overload", async () => {
			...
			const overloadPromises = Array(overloadRequests)
				.fill(null)
				.map(() =>
					performanceTools.searchTransactions({
						limit: 100,
			...
			const recoveryPromises = Array(recoveryRequests)
				.fill(null)
				.map(() => performanceTools.getAccount({ code: "1000" }));
			...
			expect(recoveryStats.avgDuration).toBeLessThan(
				PERFORMANCE_CONFIG.TARGET_RESPONSE_TIME_MS,
			);
		...
		it("should maintain consistent performance across tool types", async () => {
			...
			for (const toolTest of toolTests) {
				...
				const promises = Array(iterations)
					.fill(null)
					.map(() =>
						performanceTools[toolTest.name as keyof typeof performanceTools](
							toolTest.args,
						),
					);
			...
			for (const [tool, perf] of Object.entries(toolPerformance)) {
				console.log(
					`  ${tool}: ${perf.avgDuration.toFixed(2)}ms avg, ${perf.maxDuration.toFixed(2)}ms max`,
				);
			}

			// Ensure no tool is significantly slower than others
			const avgDurations = Object.values(toolPerformance).map(
				(p: any) => p.avgDuration,
			);

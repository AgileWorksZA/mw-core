	...
	describe("Contact Tool Schemas", () => {
		describe("searchContactsTool schema", () => {
			...
			it("should handle boolean isActive", () => {
				const booleanCases = [{ isActive: true }, { isActive: false }];
	...
	describe("Type Safety", () => {
		it("should enforce type safety for enum values", () => {
			...
			for (const type of accountTypes) {
				expect(() =>
					searchAccountsTool.inputSchema.parse({ type }),
				).not.toThrow();
			}
			...
			for (const type of transactionTypes) {
				expect(() =>
					searchTransactionsTool.inputSchema.parse({ type }),
				).not.toThrow();
			}
			...
			for (const type of contactTypes) {
				expect(() =>
					searchContactsTool.inputSchema.parse({ type }),
				).not.toThrow();
			}
		});
	});

...
import { getContactTool, searchContactsTool } from "../../src/tools/contact";
import {
	getTransactionByRefTool,
	getTransactionTool,
	searchTransactionsTool,
} from "../../src/tools/transaction";
import { MockApiError, createMockService } from "../setup";
...
describe("Error Handling Tests", () => {
	describe("Network and API Errors", () => {
		...
		it("should handle authentication errors (401)", async () => {
			...
			await expect(searchTransactionsTool.execute(args)).rejects.toThrow(
				"Unauthorized",
			);
		...
		it("should handle forbidden errors (403)", async () => {
			...
			await expect(searchContactsTool.execute(args)).rejects.toThrow(
				"Forbidden",
			);
		...
		it("should handle rate limiting errors (429)", async () => {
			...
			await expect(getContactTool.execute(args)).rejects.toThrow(
				"Too Many Requests",
			);
	...
	describe("Resource Not Found Errors", () => {
		it("should handle account not found", async () => {
			const notFoundService = createMockService({
				data: [],
				pagination: { total: 0 },
			});
		...
		it("should handle transaction not found by sequence number", async () => {
			...
			await expect(getTransactionTool.execute(args)).rejects.toThrow(
		...
		it("should handle transaction not found by reference", async () => {
			...
			await expect(getTransactionByRefTool.execute(args)).rejects.toThrow(
				"Transaction not found with reference: NONEXISTENT",
			);
	...
	describe("Data Validation Errors", () => {
		...
		it("should handle missing required fields in response", async () => {
			...
			await expect(getAccountTool.execute(args)).rejects.toThrow(
				"Invalid account data: missing Code field",
			);
				...
				if (args.fromDate || args.toDate) {
					filteredData = filteredData.filter((trans) => {
						...
						if (Number.isNaN(transDate.getTime())) {
							throw new Error(
								`Invalid date format in transaction: ${trans.TransDate}`,
							);
						}
	...
	describe("Edge Case Handling", () => {
		...
		it("should handle special characters in search queries", async () => {
			...
			await expect(searchAccountsTool.execute(invalidArgs)).rejects.toThrow(
				"Invalid characters",
			);
		...
		it("should handle concurrent access gracefully", async () => {
			...
			const concurrentService = {
				async getData() {
					...
					return {
						data: [
							{ Code: `RESULT-${callCount}`, Description: `Call ${callCount}` },
						],
						pagination: { total: 1, offset: 0, limit: 50 },
					};
				},
	...
	describe("Resource Management", () => {
		it("should handle memory pressure gracefully", async () => {
			...
			searchAccountsTool.execute = async (args) => {
				...
				if (result.data.length > 1000) {
					throw new Error(
						"Result set too large, please use pagination to limit results",
					);
				}
		...
		it("should handle connection pool exhaustion", async () => {
			...
			const connectionPoolFailures = failures.filter(
				(f) =>
					f.status === "rejected" &&
					f.reason.message.includes("Connection pool exhausted"),
			);

...
`);
      for (const line of lines) {
        if (line.includes("created")) {
          ...
          if (match)
            result.created = parseInt(match[1]);
        }
        if (line.includes("updated")) {
          const match = line.match(/(\d+) updated/);
          if (match)
            result.updated = parseInt(match[1]);
        }
  ...
  async listDocuments() {
    ...
    try {
      ...
      const documents = listText.split(`
...
var TAXRATE_TSV_FIELD_MAPPING = [
  ...
  { position: 2, xmlName: "_timestamp", pascalName: "_Timestamp", dataType: "string" },
...
var fieldStructureCache = new Map;
...
function addHeaders(data, fields) {
  const headers = fields.sort((a, b) => a.position - b.position).map((f) => f.name);
  return [headers, ...data];
}
...
class BaseMoneyWorksRepository {
  ...
  async findByKey(key) {
    ...
    const search = isNumeric ? `${this.primaryKey}=${key}` : `${this.primaryKey}="${key}"`;
    const data = await this.client.smartExport(this.tableName, {
      search,
      limit: 1
    });
...
var MONEYWORKS_TAX_RATE_FIELDS = [
  {
    fieldName: "TaxCode",
    dataType: "T" /* TEXT */,
    maxLength: 5,
...
var COMPANY_INFORMATION_FIELDS = [
  "Name",
  "Address1",
  "Address2",
  "Address3",
  ...
  async getCompanyInformation(fields = COMPANY_INFORMATION_FIELDS) {
    try {
      ...
      for (let i = 0;i < fields.length; i += batchSize) {
        ...
        try {
          console.log(`[Company Info] Evaluating batch ${Math.floor(i / batchSize) + 1}:`, batchFields);
          ...
          console.warn(`[Company Info] Failed to fetch batch ${Math.floor(i / batchSize) + 1}:`, err);
          errors2.push(`Some fields in batch ${Math.floor(i / batchSize) + 1} could not be fetched`);
        }
      }
      if (errors2.length > 0) {
...
function validateName(name) {
  ...
  if (name.CustomerType !== undefined) {
    if (![0, 1, 2].includes(name.CustomerType)) {
      errors2.push({
        field: "CustomerType",
        message: "CustomerType must be 0 (Not Customer), 1 (Customer), or 2 (Debtor)",
  ...
  if (name.SupplierType !== undefined) {
    if (![0, 1, 2].includes(name.SupplierType)) {
      errors2.push({
        field: "SupplierType",
        message: "SupplierType must be 0 (Not Supplier), 1 (Supplier), or 2 (Creditor)",
  ...
  if (name.CustomerType === 2 /* DEBTOR */) {
    if (!name.RecAccount) {
      warnings.push("Debtors should have a Receivables Account (RecAccount) specified");
    }
  ...
  if (name.SupplierType === 2 /* CREDITOR */) {
    if (!name.PayAccount) {
      warnings.push("Creditors should have a Payables Account (PayAccount) specified");
    }
  }
...
class NameRepository extends BaseMoneyWorksRepository {
  ...
  postProcess(record) {
    ...
    numericFields.forEach((field) => {
      if (field in processed && processed[field] !== null && processed[field] !== "") {
  ...
  async create(data) {
    ...
    if (isDebtor({ CustomerType: nameData.CustomerType }) && !nameData.RecAccount) {
      throw new Error("Receivables Account (RecAccount) is required for Debtors");
    }
    if (isCreditor({ SupplierType: nameData.SupplierType }) && !nameData.PayAccount) {
      throw new Error("Payables Account (PayAccount) is required for Creditors");
    }
    ...
    if (result.errors > 0 || !result.created) {
      throw new Error(result.errorDetails?.[0]?.message || "Failed to create name");
    }
  ...
  async update(code, data) {
    ...
    if (isDebtor({ CustomerType: updated.CustomerType }) && !updated.RecAccount) {
      throw new Error("Receivables Account (RecAccount) is required for Debtors");
    }
    if (isCreditor({ SupplierType: updated.SupplierType }) && !updated.PayAccount) {
      throw new Error("Payables Account (PayAccount) is required for Creditors");
    }
    ...
    if (result.errors > 0) {
      throw new Error(result.errorDetails?.[0]?.message || "Failed to update name");
    }
  ...
  async delete(code) {
    throw new Error("Name deletion is not supported. Consider marking the name as inactive instead.");
  }
  ...
  async search(searchText, options = {}) {
    ...
    const searchConditions = searchFields.map((field) => `${field} CONTAINS[c] "${searchText}"`).join(" OR ");
  ...
  definition: {
    ...
    inputSchema: {
      ...
      properties: {
        expression: {
          type: "string",
          description: 'MWScript expression to evaluate (e.g., "1+1", "Date()", etc.)'
        }
      },
      required: ["expression"]
    }
  ...
  async handler(client, params) {
    ...
    try {
      ...
      if (errorMessage.includes("Unknown identifier")) {
        throw new Error(`${errorMessage}. Note: Direct table references like 'Count(TaxRate)' may not work in REST context. Try simpler expressions like '1+1' or 'Date()'.`);
      }
...
var schemaTool = {
  ...
  async handler(client, params) {
    ...
    if (!VETTED_ENTITIES2.includes(table)) {
      throw new Error(`Table '${table}' is not yet available. Currently available: ${VETTED_ENTITIES2.join(", ")}`);
    }
    try {
...
var companyInfoTool = {
  ...
  handler: async (client, _params) => {
    try {
      ...
      console.log("[Company Info Tool] Company data from repository:", companyData);
...
`;
      ...
      if (companyData.Phone || companyData.Fax || companyData.Email || companyData.Mobile) {
...
`;
      ...
      if (companyData.PeriodsInYear || companyData.CurrentPer || companyData.BaseCurrency) {
...
`;
      ...
      const gstFields = companyData.GSTProcessingSuppressed !== undefined || companyData.GSTIncomeInvoiceBasis !== undefined || companyData.GSTExpensesInvoiceBasis !== undefined;
      if (gstFields) {
...
var getOverdueDebtorsTool = {
  definition: {
    ...
    inputSchema: {
      ...
      properties: {
        daysOverdue: {
          type: "number",
          description: "Minimum days overdue (0 = any overdue, 30 = 30+ days, etc.)",
          minimum: 0,
          default: 0
        }
      }
  ...
  async handler(client, params) {
    const repo = new NameRepository(client);
    const overdueDebtors = await repo.getOverdueDebtors(params.daysOverdue || 0);
    return {
      content: [
        {
          type: "text",
...
var getNamesByCategoryTool = {
  ...
  async handler(client, params) {
    const repo = new NameRepository(client);
    const names = await repo.getByCategory(params.category, params.categoryNumber || 1);
    return {
      content: [
        {
          type: "text",
...
var updateCreditLimitTool = {
  ...
  async handler(client, params) {
    const repo = new NameRepository(client);
    const updatedName = await repo.updateCreditLimit(params.code, params.creditLimit);
    return {
      content: [
        {
          type: "text",

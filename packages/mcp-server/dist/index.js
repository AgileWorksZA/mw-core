...
const require_uri_all = __commonJS((exports, module) => {
	((global, factory) => {
		...
		function merge() {
			for (
				let _len = arguments.length, sets = Array(_len), _key = 0;
				_key < _len;
				_key++
			) {
		...
		function buildExps(isIRI2) {
			...
			const DIGIT$$ = "[0-9]";
			const DQUOTE$$ = "[\\x22]";
			const HEXDIG$$2 = merge(DIGIT$$, "[A-Fa-f]");
			const LF$$ = "[\\x0A]";
			const SP$$ = "[\\x20]";
			const PCT_ENCODED$2 = subexp(
			...
			const UCSCHAR$$ = isIRI2
				? "[\\xA0-\\u200D\\u2010-\\u2029\\u202F-\\uD7FF\\uF900-\\uFDCF\\uFDF0-\\uFFEF]"
				: "[]";
			const IPRIVATE$$ = isIRI2 ? "[\\uE000-\\uF8FF]" : "[]";
			const UNRESERVED$$2 = merge(
				ALPHA$$,
				DIGIT$$,
				"[\\-\\.\\_\\~]",
				UCSCHAR$$,
			...
			const IPV6ADDRESS6$ = subexp(
				`${subexp(`${subexp(`${H16$}\\:`)}{0,3}${H16$}`)}?\\:\\:${H16$}\\:${LS32$}`,
			);
			const IPV6ADDRESS7$ = subexp(
				`${subexp(`${subexp(`${H16$}\\:`)}{0,4}${H16$}`)}?\\:\\:${LS32$}`,
			);
			const IPV6ADDRESS8$ = subexp(
				`${subexp(`${subexp(`${H16$}\\:`)}{0,5}${H16$}`)}?\\:\\:${H16$}`,
			);
			const IPV6ADDRESS9$ = subexp(
			...
			const SEGMENT_NZ_NC$ = subexp(
				`${subexp(`${PCT_ENCODED$2}|${merge(UNRESERVED$$2, SUB_DELIMS$$, "[\\@]")}`)}+`,
			);
			const PATH_ABEMPTY$ = subexp(`${subexp(`\\/${SEGMENT$}`)}*`);
			const PATH_ABSOLUTE$ = subexp(
		...
		const encode = function encode(input) {
			...
			try {
				for (
					let _iterator = input[Symbol.iterator](), _step;
			...
			while (handledCPCount < inputLength) {
				...
				try {
					for (
						let _iterator2 = input[Symbol.iterator](), _step2;
				...
				try {
					for (
						let _iterator3 = input[Symbol.iterator](), _step3;
		...
		function pctDecChars(str) {
			...
			const il = str.length;
			while (i < il) {
				const c = Number.parseInt(str.substr(i + 1, 2), 16);
				if (c < 128) {
					newStr += String.fromCharCode(c);
					i += 3;
				} else if (c >= 194 && c < 224) {
					if (il - i >= 6) {
						const c2 = Number.parseInt(str.substr(i + 4, 2), 16);
					...
					if (il - i >= 9) {
						const _c = Number.parseInt(str.substr(i + 4, 2), 16);
						const c3 = Number.parseInt(str.substr(i + 7, 2), 16);
		...
		function _normalizeIPv6(host, protocol) {
			...
			if (address) {
				const _address$toLowerCase$ = address
					.toLowerCase()
					.split("::")
					.reverse();
			...
			if (components.host) {
				if (protocol.IPV6ADDRESS.test(components.host)) {
					...
					try {
						...
						components.error =
							components.error ||
							`Host's domain name can not be converted to ${!options.iri ? "ASCII" : "Unicode"} via punycode: ${e}`;
					}
				}
			}
			_normalizeComponentEncoding(components, protocol);
		...
		const handler$2 = {
			...
			serialize: function serialize(wsComponents, options) {
				...
				if (wsComponents.resourceName) {
					...
					const _wsComponents$resourc2 = slicedToArray(
				...
				if (to) {
					for (let x = 0, xl = to.length; x < xl; ++x) {
						...
						try {
							...
							components.error =
								components.error ||
								`Email address's domain name can not be converted to ${!options.iri ? "ASCII" : "Unicode"} via punycode: ${e}`;
						}
						to[x] = `${localPart}@${domain}`;
					}
					components.path = to.join(",");
				}
				const headers = (mailtoComponents.headers =
		...
		const handler$5 = {
			scheme: "urn",
			parse: function parse$$1(components, options) {
				const matches = components.path?.match(URN_PARSE);
...
const require_fast_deep_equal = __commonJS((exports, module) => {
...
const require_ucs2length = __commonJS((exports, module) => {
...
const require_util = __commonJS((exports, module) => {
	module.exports = {
		copy,
		checkDataType,
		checkDataTypes,
	...
	function checkDataTypes(dataTypes, data, strictNumbers) {
		switch (dataTypes.length) {
			...
			default: {
				let code = "";
				const types2 = toHash(dataTypes);
				if (types2.array && types2.object) {
					code = types2.null ? "(" : `(!${data} || `;
					code += `typeof ${data} !== "object")`;
					types2.null = undefined;
					types2.array = undefined;
					types2.object = undefined;
				}
				if (types2.number) types2.integer = undefined;
...
const require_schema_obj = __commonJS((exports, module) => {
	const util3 = require_util();
...
const require_json_schema_traverse = __commonJS((exports, module) => {
	const traverse = (module.exports = (schema, opts, cb) => {
...
const require_resolve = __commonJS((exports, module) => {
	const URI = require_uri_all();
	const equal = require_fast_deep_equal();
	const util3 = require_util();
	const SchemaObject = require_schema_obj();
	...
	const PREVENT_SCOPE_CHANGE = util3.toHash([
		"properties",
		"patternProperties",
		"enum",
		"dependencies",
	...
	const SIMPLE_INLINED = util3.toHash([
		"type",
		"format",
		"pattern",
		"maxLength",
...
const require_error_classes = __commonJS((exports, module) => {
	const resolve = require_resolve();
	module.exports = {
...
const require_fast_json_stable_stringify = __commonJS((exports, module) => {
...
const require_validate = __commonJS((exports, module) => {
	module.exports = function generate_validate(it, $keyword, $ruleType) {
		let out = "";
		const $async = it.schema.$async === true;
		let $refKeywords = it.util.schemaHasRulesExcept(
			it.schema,
			it.RULES.all,
			"$ref",
		);
		...
		if (it.opts.strictKeywords) {
			const $unknownKwd = it.util.schemaUnknownRules(
				it.schema,
				it.RULES.keywords,
			);
			if ($unknownKwd) {
		...
		if (it.isTop) {
			...
			if (
				...
				const $defaultMsg = "default is ignored in the schema root";
			...
			const { level: $lvl, dataLevel: $dataLvl } = it;
			const $data = `data${$dataLvl || ""}`;
		...
		let $typeIsArray = Array.isArray($typeSchema);
		...
		if ($typeSchema) {
			if (it.opts.coerceTypes) {
				const $coerceToTypes = it.util.coerceToTypes(
					it.opts.coerceTypes,
					$typeSchema,
				);
			}
			const $rulesGroup = it.RULES.types[$typeSchema];
			if (
				$coerceToTypes ||
				$typeIsArray ||
				$rulesGroup === true ||
				...
				const $schemaPath = `${it.schemaPath}.type`;
				const $errSchemaPath = `${it.errSchemaPath}/type`;
				const $method = $typeIsArray ? "checkDataTypes" : "checkDataType";
				out += ` if (${it.util[$method]($typeSchema, $data, it.opts.strictNumbers, true)}) { `;
				if ($coerceToTypes) {
					const $dataType = `dataType${$lvl}`;
					const $coerced = `coerced${$lvl}`;
					...
					out += ` } if (${$coerced} !== undefined) {  `;
					const $parentData = $dataLvl
						? `data${$dataLvl - 1 || ""}`
						: "parentData";
					const $parentDataProperty = $dataLvl
					...
					if ($shouldUseGroup($rulesGroup)) {
						...
						if (it.opts.useDefaults) {
							if ($rulesGroup.type === "object" && it.schema.properties) {
								const $schema = it.schema.properties;
								const $schemaKeys = Object.keys($schema);
								const arr3 = $schemaKeys;
								if (arr3) {
									let $propertyKey;
									let i3 = -1;
									const l3 = arr3.length - 1;
									while (i3 < l3) {
										$propertyKey = arr3[(i3 += 1)];
										const $sch = $schema[$propertyKey];
										if ($sch.default !== undefined) {
											const $passData =
												$data + it.util.getProperty($propertyKey);
											if (it.compositeRule) {
												if (it.opts.strictDefaults) {
													const $defaultMsg = `default is ignored for: ${$passData}`;
								...
								if (arr4) {
									let $sch;
									let $i = -1;
									const l4 = arr4.length - 1;
									while ($i < l4) {
										$sch = arr4[($i += 1)];
										if ($sch.default !== undefined) {
											const $passData = `${$data}[${$i}]`;
											if (it.compositeRule) {
												if (it.opts.strictDefaults) {
													const $defaultMsg = `default is ignored for: ${$passData}`;
													if (it.opts.strictDefaults === "log")
...
const require_compile = __commonJS((exports, module) => {
	const resolve = require_resolve();
	const util3 = require_util();
	const errorClasses = require_error_classes();
	const stableStringify = require_fast_json_stable_stringify();
	const validateGenerator = require_validate();
	const ucs2length = util3.ucs2length;
	const equal = require_fast_deep_equal();
	...
	function compile(schema, root, localRefs, baseId) {
		...
		function resolveRef(baseId2, ref, isRoot) {
			...
			let v2 = resolve.call(self, localCompile, root, ref);
			if (v2 === undefined) {
				const localSchema = localRefs?.[ref];
				if (localSchema) {
		...
		function useCustomRule(rule, schema2, parentSchema, it) {
			...
			const compile2 = rule.definition.compile;
			const inline = rule.definition.inline;
			const macro = rule.definition.macro;
			let validate;
			if (compile2) {
...
const require_cache = __commonJS((exports, module) => {
...
const require_formats = __commonJS((exports, module) => {
	const util3 = require_util();
...
const require_ref = __commonJS((exports, module) => {
...
const require_allOf = __commonJS((exports, module) => {
...
const require_anyOf = __commonJS((exports, module) => {
	module.exports = function generate_anyOf(it, $keyword, $ruleType) {
		...
		$it.level++;
		const $nextValid = `valid${$it.level}`;
		const $noEmptySchema = $schema.every(($sch2) =>
			it.opts.strictKeywords
...
const require_comment = __commonJS((exports, module) => {
...
const require_const = __commonJS((exports, module) => {
...
const require_contains = __commonJS((exports, module) => {
	module.exports = function generate_contains(it, $keyword, $ruleType) {
		...
		const $nextValid = `valid${$it.level}`;
		const $idx = `i${$lvl}`;
		const $dataNxt = ($it.dataLevel = it.dataLevel + 1);
		const $nextData = `data${$dataNxt}`;
		const $currentBaseId = it.baseId;
		const $nonEmptySchema = it.opts.strictKeywords
...
const require_dependencies = __commonJS((exports, module) => {
	module.exports = function generate_dependencies(it, $keyword, $ruleType) {
		...
		for ($property in $schema) {
			...
			const $sch = $schema[$property];
			const $deps = Array.isArray($sch) ? $propertyDeps : $schemaDeps;
			$deps[$property] = $sch;
		}
		out += `var ${$errs} = errors;`;
		const $currentErrorPath = it.errorPath;
		out += `var missing${$lvl};`;
		for (const $property in $propertyDeps) {
				...
				if ($breakOnError) {
					...
					if (arr1) {
						...
						while ($i < l1) {
							...
							const $prop = it.util.getProperty($propertyKey);
							const $useData = $data + $prop;
					...
					out += ")) {  ";
					const $propertyPath = `missing${$lvl}`;
					const $missingProperty = `' + ${$propertyPath} + '`;
					...
					if (arr2) {
						...
						while (i2 < l2) {
							...
							const $prop = it.util.getProperty($propertyKey);
							const $missingProperty = it.util.escapeQuotes($propertyKey);
							const $useData = $data + $prop;
							if (it.opts._errorDataPathProperty) {
		...
		const $currentBaseId = $it.baseId;
		for (const $property in $schemaDeps) {
			const $sch = $schemaDeps[$property];
			if (
				it.opts.strictKeywords
					? (typeof $sch === "object" && Object.keys($sch).length > 0) ||
						$sch === false
...
const require_enum = __commonJS((exports, module) => {
...
const require_format = __commonJS((exports, module) => {
	module.exports = function generate_format(it, $keyword, $ruleType) {
		...
		const $allowUnknown = Array.isArray($unknownFormats);
		if ($isData) {
			const $format = `format${$lvl}`;
			const $isObject = `isObject${$lvl}`;
			const $formatType = `formatType${$lvl}`;
			...
			const $isObject =
				...
				$format.validate;
			const $formatType = ($isObject && $format.type) || "string";
			if ($isObject) {
				const $async = $format.async === true;
			...
			if ($async) {
				...
				const $formatRef = `formats${it.util.getProperty($schema)}.validate`;
...
const require_if = __commonJS((exports, module) => {
...
const require_items = __commonJS((exports, module) => {
...
const require__limit = __commonJS((exports, module) => {
	module.exports = function generate__limit(it, $keyword, $ruleType) {
		...
		if ($isDataExcl) {
			...
			const $exclusive = `exclusive${$lvl}`;
			const $exclType = `exclType${$lvl}`;
			const $exclIsNumber = `exclIsNumber${$lvl}`;
			const $opExpr = `op${$lvl}`;
			const $opStr = `' + ${$opExpr} + '`;
			...
			const $exclIsNumber = typeof $schemaExcl === "number";
			let $opStr = $op;
...
const require__limitItems = __commonJS((exports, module) => {
...
const require__limitLength = __commonJS((exports, module) => {
...
const require__limitProperties = __commonJS((exports, module) => {
...
const require_multipleOf = __commonJS((exports, module) => {
...
const require_not = __commonJS((exports, module) => {
...
const require_oneOf = __commonJS((exports, module) => {
	module.exports = function generate_oneOf(it, $keyword, $ruleType) {
		...
		const $nextValid = `valid${$it.level}`;
		const $currentBaseId = $it.baseId;
		const $prevValid = `prevValid${$lvl}`;
		const $passingSchemas = `passingSchemas${$lvl}`;
...
const require_pattern = __commonJS((exports, module) => {
...
const require_properties = __commonJS((exports, module) => {
	module.exports = function generate_properties(it, $keyword, $ruleType) {
		...
		const $pProperties = it.schema.patternProperties || {};
		const $pPropertyKeys = Object.keys($pProperties).filter(notProto);
		const $aProperties = it.schema.additionalProperties;
		const $someProperties = $schemaKeys.length || $pPropertyKeys.length;
		const $noAdditional = $aProperties === false;
		const $additionalIsSchema =
			typeof $aProperties === "object" && Object.keys($aProperties).length;
		const $removeAdditional = it.opts.removeAdditional;
		...
		if (
			...
			const $requiredHash = it.util.toHash($required);
		}
		function notProto(p) {
			return p !== "__proto__";
		}
				...
				while (i3 < l3) {
					...
					if (
						...
						if (it.util.varOccurences($code, $nextData) < 2) {
							...
							const $useData = $passData;
						} else {
							const $useData = $nextData;
							out += ` var ${$nextData} = ${$passData}; `;
						}
						if ($hasDefault) {
							out += ` ${$code} `;
						} else {
							if ($requiredHash?.[$propertyKey]) {
								out += ` if ( ${$useData} === undefined `;
								if ($ownProperties) {
									out += ` || ! Object.prototype.hasOwnProperty.call(${$data}, '${it.util.escapeQuotes($propertyKey)}') `;
								}
		...
		if ($pPropertyKeys.length) {
			...
			if (arr4) {
				...
				while (i4 < l4) {
					$pProperty = arr4[(i4 += 1)];
					const $sch = $pProperties[$pProperty];
					if (
						it.opts.strictKeywords
							? (typeof $sch === "object" && Object.keys($sch).length > 0) ||
								$sch === false
...
const require_propertyNames = __commonJS((exports, module) => {
...
const require_required = __commonJS((exports, module) => {
	module.exports = function generate_required(it, $keyword, $ruleType) {
		...
		if ($isData || $required.length) {
			const $currentErrorPath = it.errorPath;
			const $loopRequired = $isData || $required.length >= it.opts.loopRequired;
			const $ownProperties = it.opts.ownProperties;
			if ($breakOnError) {
				out += ` var missing${$lvl}; `;
				if ($loopRequired) {
					...
					const $i = `i${$lvl}`;
					const $propertyPath = `schema${$lvl}[${$i}]`;
					const $missingProperty = `' + ${$propertyPath} + '`;
					if (it.opts._errorDataPathProperty) {
						it.errorPath = it.util.getPathExpr(
							$currentErrorPath,
...
const require_uniqueItems = __commonJS((exports, module) => {
	module.exports = function generate_uniqueItems(it, $keyword, $ruleType) {
		...
		if (($schema || $isData) && it.opts.uniqueItems !== false) {
			...
			const $itemType = it.schema.items?.type;
			const $typeIsArray = Array.isArray($itemType);
			if (
				!$itemType ||
				$itemType === "object" ||
				$itemType === "array" ||
...
const require_dotjs = __commonJS((exports, module) => {
	module.exports = {
		$ref: require_ref(),
		allOf: require_allOf(),
		anyOf: require_anyOf(),
...
const require_rules = __commonJS((exports, module) => {
	const ruleModules = require_dotjs();
	const toHash = require_util().toHash;
...
const require_data = __commonJS((exports, module) => {
	const KEYWORDS = [
		"multipleOf",
		"maximum",
		"exclusiveMaximum",
...
const require_async = __commonJS((exports, module) => {
	const MissingRefError = require_error_classes().MissingRef;
...
const require_custom = __commonJS((exports, module) => {
	module.exports = function generate_custom(it, $keyword, $ruleType) {
		...
		const $definition = `definition${$lvl}`;
		const $rDef = this.definition;
		let $closingBraces = "";
		let $compile;
		let $inline;
		let $macro;
		...
		if ($rDef.valid) {
			...
			out = "";
			if (it.createErrors !== false) {
				out += ` { keyword: '${$errorKeyword || "custom"}' , dataPath: (dataPath || '') + ${it.errorPath} , schemaPath: ${it.util.toQuotedString($errSchemaPath)} , params: { keyword: '${this.keyword}' } `;
				if (it.opts.messages !== false) {
...
const require_json_schema_draft_07 = __commonJS((exports, module) => {
	module.exports = {
...
const require_definition_schema = __commonJS((exports, module) => {
	const metaSchema = require_json_schema_draft_07();
	module.exports = {
...
const require_keyword = __commonJS((exports, module) => {
	const IDENTIFIER = /^[a-z_$][a-z0-9_$-]*$/i;
	const customRuleCode = require_custom();
	const definitionSchema = require_definition_schema();
	module.exports = {
		add: addKeyword,
		get: getKeyword,
...
const require_data2 = __commonJS((exports, module) => {
	module.exports = {
		$schema: "http://json-schema.org/draft-07/schema#",
...
const require_ajv = __commonJS((exports, module) => {
	const compileSchema = require_compile();
	const resolve = require_resolve();
	...
	const $dataMetaSchema = require_data();
	const util3 = require_util();
	module.exports = Ajv;
	...
	const customKeyword = require_keyword();
	...
	function addDefaultMetaSchema(self) {
		...
		let metaSchema = require_json_schema_draft_07();
		if (self._opts.$data)
	...
	function getMetaSchemaOptions(self) {
		const metaOpts = util3.copy(self._opts);
...
// ../data/src/config/index.ts
async function loadConfig(configPath) {
	console.warn(
		"⚠️  loadConfig is deprecated. Use token-based authentication with the API instead.",
	);
	...
	try {
		...
		const config = {
			...
			port: Number.parseInt(process.env.MW_PORT || configFile.port),
			protocol: process.env.MW_PROTOCOL || configFile.protocol || "http",
		...
		if (configFile.folderName || process.env.MW_FOLDER_NAME) {
			config.folderAuth = {
				...
				folderPassword:
					process.env.MW_FOLDER_PASSWORD || configFile.folderPassword || "",
			};
		}
		return config;
	} catch (error) {
		if (error instanceof Error && error.message.includes("ENOENT")) {
			console.warn(
				`⚠️  Config file not found at ${path}. If using the API with token auth, this is expected.`,
			);
			return {
				host: "not-configured",
				port: 6710,
...
const ProgressTokenSchema = exports_external.union([
	exports_external.string(),
	exports_external.number().int(),
]);
const CursorSchema = exports_external.string();
const RequestMetaSchema = exports_external
	.object({
		progressToken: exports_external.optional(ProgressTokenSchema),
	})
	.passthrough();
const BaseRequestParamsSchema = exports_external
	.object({
		_meta: exports_external.optional(RequestMetaSchema),
	})
	.passthrough();
const RequestSchema = exports_external.object({
	method: exports_external.string(),
	params: exports_external.optional(BaseRequestParamsSchema),
});
const BaseNotificationParamsSchema = exports_external
	.object({
		_meta: exports_external.optional(exports_external.object({}).passthrough()),
	})
	.passthrough();
const NotificationSchema = exports_external.object({
	method: exports_external.string(),
	params: exports_external.optional(BaseNotificationParamsSchema),
});
const ResultSchema = exports_external
	.object({
		_meta: exports_external.optional(exports_external.object({}).passthrough()),
	})
	.passthrough();
const RequestIdSchema = exports_external.union([
	exports_external.string(),
	exports_external.number().int(),
]);
const JSONRPCRequestSchema = exports_external
	.object({
...
const JSONRPCNotificationSchema = exports_external
	.object({
		jsonrpc: exports_external.literal(JSONRPC_VERSION),
	})
...
const JSONRPCResponseSchema = exports_external
	.object({
		jsonrpc: exports_external.literal(JSONRPC_VERSION),
		id: RequestIdSchema,
		result: ResultSchema,
...
const JSONRPCErrorSchema = exports_external
	.object({
		jsonrpc: exports_external.literal(JSONRPC_VERSION),
		id: RequestIdSchema,
...
const JSONRPCMessageSchema = exports_external.union([
...
const BaseMetadataSchema = exports_external
	.object({
...
const ClientCapabilitiesSchema = exports_external
	.object({
...
const ServerCapabilitiesSchema = exports_external
	.object({
		experimental: exports_external.optional(
			exports_external.object({}).passthrough(),
		),
...
const ProgressSchema = exports_external
	.object({
...
const ResourceContentsSchema = exports_external
	.object({
...
const PromptArgumentSchema = exports_external
	.object({
		name: exports_external.string(),
		description: exports_external.optional(exports_external.string()),
...
const TextContentSchema = exports_external
	.object({
	...
	.passthrough();
const ImageContentSchema = exports_external
	.object({
	...
	.passthrough();
const AudioContentSchema = exports_external
	.object({
	...
	.passthrough();
const EmbeddedResourceSchema = exports_external
	.object({
...
const ContentBlockSchema = exports_external.union([
	TextContentSchema,
	ImageContentSchema,
	AudioContentSchema,
	ResourceLinkSchema,
...
const PromptMessageSchema = exports_external
	.object({
...
const ToolAnnotationsSchema = exports_external
	.object({
		title: exports_external.optional(exports_external.string()),
...
const LoggingLevelSchema = exports_external.enum([
	"debug",
	"info",
	"notice",
	"warning",
...
const ModelHintSchema = exports_external
	.object({
		name: exports_external.string().optional(),
	})
	.passthrough();
const ModelPreferencesSchema = exports_external
	.object({
	...
	.passthrough();
const SamplingMessageSchema = exports_external
	.object({
		role: exports_external.enum(["user", "assistant"]),
...
const BooleanSchemaSchema = exports_external
	.object({
	...
	.passthrough();
const StringSchemaSchema = exports_external
	.object({
	...
	.passthrough();
const NumberSchemaSchema = exports_external
	.object({
	...
	.passthrough();
const EnumSchemaSchema = exports_external
	.object({
		type: exports_external.literal("string"),
		title: exports_external.optional(exports_external.string()),
		description: exports_external.optional(exports_external.string()),
	...
	.passthrough();
const PrimitiveSchemaDefinitionSchema = exports_external.union([
	BooleanSchemaSchema,
	StringSchemaSchema,
	NumberSchemaSchema,
	EnumSchemaSchema,
...
const ResourceTemplateReferenceSchema = exports_external
	.object({
		type: exports_external.literal("ref/resource"),
		uri: exports_external.string(),
	})
	.passthrough();
const PromptReferenceSchema = exports_external
	.object({
...
const RootSchema = exports_external
	.object({
...
const ClientRequestSchema = exports_external.union([
	PingRequestSchema,
...
const ClientNotificationSchema = exports_external.union([
	CancelledNotificationSchema,
	ProgressNotificationSchema,
	InitializedNotificationSchema,
	RootsListChangedNotificationSchema,
]);
const ClientResultSchema = exports_external.union([
	EmptyResultSchema,
	CreateMessageResultSchema,
	ElicitResultSchema,
	ListRootsResultSchema,
]);
const ServerRequestSchema = exports_external.union([
	PingRequestSchema,
	CreateMessageRequestSchema,
	ElicitRequestSchema,
	ListRootsRequestSchema,
]);
const ServerNotificationSchema = exports_external.union([
	CancelledNotificationSchema,
	ProgressNotificationSchema,
...
const ServerResultSchema = exports_external.union([
	EmptyResultSchema,
...
const import_ajv = __toESM(require_ajv(), 1);
...
// ../data/src/client/moneyworks-rest-client.ts
class MoneyWorksRESTClient {
	config;
	baseUrl;
	authHeaders;
	constructor(config) {
...
`);
	...
	async import(table, records, options = {}) {
		const params = new URLSearchParams();
		params.append("table", table);
		if (options.mode) {
...
// ../data/src/parsers/xml/moneyworks-xml-parser.ts
function parseMoneyWorksXML(xml) {
	...
	const recordRegex = new RegExp(
		`<${recordTagName}>([\\s\\S]*?)</${recordTagName}>`,
		"g",
	);
	let recordMatch;
...
function extractFieldOrder(xml) {
	const tableMatch = xml.match(/<table\s+name="([^"]+)"/);
	if (!tableMatch) {
		throw new Error("Could not find table name in XML");
	}
...
function xmlFieldToPascalCase(xmlField) {
	const specialCases = {
	...
	return xmlField.replace(/(?:^|[-_])(\w)/g, (_, char) => char.toUpperCase());
}

// ../data/src/parsers/moneyworks-field-mappings.ts
const TAXRATE_TSV_FIELD_MAPPING = [
	{
		position: 0,
...
function getTSVFieldMapping(tableName) {
...
// ../data/src/parsers/field-discovery.ts
const fieldStructureCache = new Map();
async function discoverTableStructure(client, tableName, useCache = true) {
	...
	if (tsvMapping) {
		...
		fieldStructureCache.set(tableName, structure2);
		return structure2;
	}
...
function getCachedStructure(tableName) {
	return fieldStructureCache.get(tableName);
}

// ../data/src/converters/export-format-converter.ts
function arrayToObject(data, fields) {
	return data.map((row) => {
		const obj = {};
...
function addHeaders(data, fields) {
	const headers = fields
		.sort((a, b) => a.position - b.position)
		.map((f) => f.name);
	return [headers, ...data];
}
function enrichWithSchema(data, structure, table) {
	const schema = {};
	structure.fields.forEach((field) => {
		schema[field.name] = {
			...
			description:
				field.description || `MoneyWorks ${table} field: ${field.name}`,
		};
	});
	return {
		schema,
...
// ../data/src/client/moneyworks-smart-client.ts
class SmartMoneyWorksClient extends MoneyWorksRESTClient {
...
function createSmartClient(config) {
	return new SmartMoneyWorksClient(config);
}

// ../data/src/repositories/base.repository.ts
class BaseMoneyWorksRepository {
	client;
	constructor(client) {
		this.client = client;
	}
...
// ../canonical/src/entities/tax-rates/fields.ts
const MONEYWORKS_TAX_RATE_FIELDS = [
	{
		fieldName: "TaxCode",
		dataType: "T" /* TEXT */,
		maxLength: 5,
...
// ../utilities/src/date/yyyymmdd.ts
...
function isYYYYMMDD(value) {
	...
	const month = Number.parseInt(value.substring(4, 6), 10);
...
function createYYYYMMDD(value) {
	if (!isYYYYMMDD(value)) {
		throw new Error(`Invalid YYYYMMDD format: ${value}`);
	}
	return value;
}
function tryCreateYYYYMMDD(value) {
...
function parseToYYYYMMDD(value) {
...
function daysBetweenYYYYMMDD(from, to) {
...
function isBeforeYYYYMMDD(date, compare) {
	return date < compare;
}
function isAfterYYYYMMDD(date, compare) {
	return date > compare;
}
function startOfMonthYYYYMMDD(date) {
	return createYYYYMMDD(`${date.substring(0, 6)}01`);
}
function endOfMonthYYYYMMDD(date) {
	const d = yyyymmddToDate(date);
...
// ../utilities/src/date/period.ts
function isPeriod(value) {
	if (!Number.isInteger(value) || value < 1e5 || value > 999999) {
		return false;
	}
...
function periodFromYearMonth(year, month) {
...
function monthsBetweenPeriods(from, to) {
...
function formatPeriod(period, separator = "/") {
	const year = periodYear(period);
	const month = String(periodMonth(period)).padStart(2, "0");
...
let PeriodUtils;
((PeriodUtils) => {
	PeriodUtils.is = isPeriod;
	PeriodUtils.create = createPeriod;
	PeriodUtils.fromYearMonth = periodFromYearMonth;
	PeriodUtils.current = currentPeriod;
	PeriodUtils.year = periodYear;
	PeriodUtils.month = periodMonth;
	PeriodUtils.toDate = periodToDate;
	PeriodUtils.fromDate = dateToPeriod;
	PeriodUtils.fromYYYYMMDD = yyyymmddToPeriod;
	PeriodUtils.startDate = periodStartDate;
	PeriodUtils.endDate = periodEndDate;
	PeriodUtils.addMonths = addMonthsToPeriod;
	PeriodUtils.monthsBetween = monthsBetweenPeriods;
	PeriodUtils.format = formatPeriod;
	PeriodUtils.range = periodRange;
})((PeriodUtils ||= {}));
// ../utilities/src/json/revivers.ts
...
// ../data/src/repositories/company-information.repository.ts
...
class CompanyInformationRepository {
	...
	async getCompanyInformation(fields = COMPANY_INFORMATION_FIELDS) {
		try {
			const companyData = {};
			const errors2 = [];
			const batchSize = 15;
			...
			if (errors2.length > 0) {
				console.warn("[Company Info] Errors during fetch:", errors2);
			}
			return companyData;
		} catch (error) {
...
// ../canonical/src/entities/names/validators.ts
function validateName(name) {
	...
	validateTextFieldLength(name, "Name", 255, errors2);
	validateTextFieldLength(name, "Address1", 59, errors2);
	validateTextFieldLength(name, "Address2", 59, errors2);
	validateTextFieldLength(name, "Address3", 59, errors2);
	validateTextFieldLength(name, "Address4", 59, errors2);
	validateTextFieldLength(name, "PostCode", 11, errors2);
	validateTextFieldLength(name, "State", 3, errors2);
	validateTextFieldLength(name, "Contact", 25, errors2);
	validateTextFieldLength(name, "email", 80, errors2);
	validateTextFieldLength(name, "Phone", 19, errors2);
	validateTextFieldLength(name, "Currency", 3, errors2);
	validateTextFieldLength(name, "TaxCode", 5, errors2);
	validateTextFieldLength(name, "TaxNumber", 19, errors2);
	...
	if (name.SupplierType === 2 /* CREDITOR */) {
		if (!name.PayAccount) {
			warnings.push(
...
function validateTextFieldLength(name, fieldName, maxLength, errors2) {
...
function isDebtor(name) {
	return name.CustomerType === 2 /* DEBTOR */;
}
function isCreditor(name) {
	return name.SupplierType === 2 /* CREDITOR */;
}
// ../data/src/repositories/name.repository.ts
class NameRepository extends BaseMoneyWorksRepository {
	tableName = "Name";
	primaryKey = "Code";
	prepare(data) {
		const prepared = { ...data };
	...
	async create(data) {
		const validation = validateName(data);
		...
		const created = await this.findByKey(data.Code);
		if (!created) {
	...
	async update(code, data) {
		...
		const validation = validateName(updated);
		if (!validation.isValid) {
			throw new Error(
				`Validation failed: ${validation.errors.map((e) => e.message).join(", ")}`,
			);
	...
	async getByCategory(category, categoryNumber = 1) {
		const search = `Category${categoryNumber}="${category}"`;
		return this.find(search, { sort: "Name" });
	}
	async getOverdueDebtors(daysOverdue = 0) {
	...
	async putOnHold(code, onHold = true) {
		return this.update(code, { Hold: onHold });
	}
	async updateCreditLimit(code, creditLimit) {
...
// src/resources/moneyworks-docs.ts
const moneyworksDocsResource = {
	definition: {
...
// src/tools/schema.tool.ts
const VETTED_ENTITIES2 = [
	"TaxRate",
	"Account",
	"Transaction",
	"Name",
...
const schemaTool = {
	...
	async handler(client, params) {
		const { table } = params;
		if (!VETTED_ENTITIES2.includes(table)) {
			throw new Error(
				`Table '${table}' is not yet available. Currently available: ${VETTED_ENTITIES2.join(", ")}`,
			);
		}
		try {
...
const listTablesTool = {
	...
	async handler(_client, _params) {
		const result = {
			available: VETTED_ENTITIES3,
			vetted: VETTED_ENTITIES3,
...
// src/tools/company-info.tool.ts
...
`;
				if (companyData.Phone)
					output += `- **Phone**: ${companyData.Phone}
`;
				if (companyData.Mobile)
					output += `- **Mobile**: ${companyData.Mobile}
`;
				if (companyData.Fax)
					output += `- **Fax**: ${companyData.Fax}
`;
				if (companyData.Email)
					output += `- **Email**: ${companyData.Email}
`;
...
// src/tools/names.tool.ts
const searchNamesTool = {
	definition: {
...
const updateNameHoldStatusTool = {
	definition: {
	...
	async handler(client, params) {
		const repo = new NameRepository(client);
		const updatedName = await repo.putOnHold(params.code, params.onHold);
		return {
			content: [
				{
					type: "text",
		...
		this.server.setRequestHandler(CallToolRequestSchema, async (request) => {
			...
			switch (name) {
				...
				case "mw_search_names":
					return await searchNamesTool.handler(client, args);
				case "mw_get_customers":
					return await getCustomersTool.handler(client, args);
				case "mw_get_debtors":
					return await getDebtorsTool.handler(client, args);
				case "mw_get_suppliers":
					return await getSuppliersTool.handler(client, args);
				case "mw_get_creditors":
					return await getCreditorsTool.handler(client, args);
				case "mw_get_overdue_debtors":
					return await getOverdueDebtorsTool.handler(client, args);
				case "mw_get_balances_summary":
					return await getBalancesSummaryTool.handler(client, args);
				case "mw_get_name_by_code":
					return await getNameByCodeTool.handler(client, args);
				case "mw_get_names_by_category":
					return await getNamesByCategoryTool.handler(client, args);
				case "mw_update_name_hold_status":
					return await updateNameHoldStatusTool.handler(client, args);
				case "mw_update_credit_limit":
					return await updateCreditLimitTool.handler(client, args);
				default:

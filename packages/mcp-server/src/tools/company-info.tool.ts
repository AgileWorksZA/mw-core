  ...
  handler: async (
    ...
    try {
      ...
      if (companyData.Phone || companyData.Fax || companyData.Email || companyData.Mobile) {
        ...
        if (companyData.Mobile) output += `- **Mobile**: ${companyData.Mobile}\n`;
        if (companyData.Fax) output += `- **Fax**: ${companyData.Fax}\n`;
        if (companyData.Email) output += `- **Email**: ${companyData.Email}\n`;
        if (companyData.WebURL) output += `- **Website**: ${companyData.WebURL}\n`;
        output += "\n";
      }
      ...
      if (companyData.GstNum || companyData.RegNum || companyData.TaxName) {
        ...
        if (companyData.TaxName) output += `- **Tax Name**: ${companyData.TaxName}\n`;
        if (companyData.GstNum) output += `- **GST Number**: ${companyData.GstNum}\n`;
        if (companyData.RegNum) output += `- **Registration Number**: ${companyData.RegNum}\n`;
        if (companyData.GSTCycleMonths) output += `- **GST Cycle**: ${companyData.GSTCycleMonths} months\n`;
        if (companyData.GstCycleEndDate) output += `- **GST Cycle End**: ${companyData.GstCycleEndDate}\n`;
        output += "\n";
      }
      ...
      if (companyData.PeriodsInYear || companyData.CurrentPer || companyData.BaseCurrency) {
        ...
        if (companyData.PeriodsInYear) output += `- **Periods in Year**: ${companyData.PeriodsInYear}\n`;
        if (companyData.CurrentPer) output += `- **Current Period**: ${companyData.CurrentPer}\n`;
        if (companyData.BaseCurrency) output += `- **Base Currency**: ${companyData.BaseCurrency}\n`;
      ...
      if (companyData.Version || companyData.Locale) {
        ...
        if (companyData.Version) output += `- **MoneyWorks Version**: ${companyData.Version}\n`;
        if (companyData.Locale) output += `- **Locale**: ${companyData.Locale}\n`;
        if (companyData.LocaleFriendlyName) output += `- **Locale Name**: ${companyData.LocaleFriendlyName}\n`;
        output += "\n";
      }
      
      // GST Processing
      const gstFields = companyData.GSTProcessingSuppressed !== undefined || 

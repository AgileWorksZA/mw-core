...
const inventoryToolSchema = z.object({
	operation: z
		...
		.describe(
			"The operation to perform: search for inventorys, get specific inventory, or list available fields",
		),
	...
	sequenceNumber: z
		.number()
		.optional()
		.describe("The inventory sequence number to retrieve (get operation only)"),
});

export const inventoryTool = {
	description:
		"Unified tool for inventory operations: search inventorys, get specific inventory, or list available fields",
	...
	async execute(args: z.infer<typeof inventoryToolSchema>) {
		switch (args.operation) {
			case "search": {
				...
				if (args.query) {
					// Search by Identifier field since there's no Code field
					search.Identifier = args.query;
				}

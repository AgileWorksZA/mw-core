...
const linkToolSchema = z.object({
	operation: z
		...
		.describe(
			"The operation to perform: search for links, get specific link, or list available fields",
		),
	...
	sequenceNumber: z
		.number()
		.optional()
		.describe("The link sequence number to retrieve (get operation only)"),
});

export const linkTool = {
	description:
		"Unified tool for link operations: search links, get specific link, or list available fields",
	...
	async execute(args: z.infer<typeof linkToolSchema>) {
		switch (args.operation) {
			case "search": {
				...
				if (args.query) {
					// Search by Dept field since there's no Code field
					search.Dept = args.query;
				}

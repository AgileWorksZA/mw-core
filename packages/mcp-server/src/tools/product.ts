...
const productToolSchema = z.object({
	operation: z
		...
		.describe(
			"The operation to perform: search for products, get specific product, or list available fields",
		),
	...
	sequenceNumber: z
		.number()
		.optional()
		.describe("The product sequence number to retrieve (get operation only)"),
	code: z
		.string()
		.optional()
		.describe("The product code to retrieve (get operation only)"),
});

export const productTool = {
	description:
		"Unified tool for product operations: search products, get specific product, or list available fields",

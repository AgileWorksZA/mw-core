...
const filterToolSchema = z.object({
	operation: z
		...
		.describe(
			"The operation to perform: search for filters, get specific filter, or list available fields",
		),
	...
	sequenceNumber: z
		.number()
		.optional()
		.describe("The filter sequence number to retrieve (get operation only)"),
});

export const filterTool = {
	description:
		"Unified tool for filter operations: search filters, get specific filter, or list available fields",
	...
	async execute(args: z.infer<typeof filterToolSchema>) {
		switch (args.operation) {
			case "search": {
				...
				if (args.query) {
					// Search by Name field since there's no Code field

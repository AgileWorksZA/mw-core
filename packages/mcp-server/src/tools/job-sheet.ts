import { JobSheetService } from "@moneyworks/api/src/services/tables/job-sheet.service";
...
const jobsheetToolSchema = z.object({
	operation: z
		...
		.describe(
			"The operation to perform: search for jobsheets, get specific jobsheet, or list available fields",
		),
	...
	sequenceNumber: z
		.number()
		.optional()
		.describe("The jobsheet sequence number to retrieve (get operation only)"),
});

export const jobsheetTool = {
	description:
		"Unified tool for jobsheet operations: search jobsheets, get specific jobsheet, or list available fields",
	...
	async execute(args: z.infer<typeof jobsheetToolSchema>) {
		switch (args.operation) {
			case "search": {
				...
				if (args.query) {
					// Search by job or resource
					search.Job = args.query;
				}

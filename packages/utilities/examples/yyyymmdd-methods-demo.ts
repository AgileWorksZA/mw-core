...
const milestone1 = contractStart
  .addMonths(3)
  .addDays(-7); // 7 days before 3 months

const milestone2 = milestone1
  .addMonths(3)
  .addDays(14); // 2 weeks after milestone 1 + 3 months
...
const transaction = {
  ...
  isUrgent: d`20250115`.addDays(30).lt(d`${new Date().setDate(new Date().getDate() + 7)}`)
};

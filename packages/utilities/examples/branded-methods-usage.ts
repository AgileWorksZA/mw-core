...
const futureDate = classDate.addMonths(1).addDays(15);
...
const result = chain(chainDate).addMonths(2).addDays(10).format("/");
...
const isNextQuarter = chain(chainDate).addMonths(3).gt(new Date());
...
const paymentDueClass = new YYYYMMDDClass(orderDate).addDays(30).format();
...
const paymentDueChain = chain(orderDate).addDays(30).format();

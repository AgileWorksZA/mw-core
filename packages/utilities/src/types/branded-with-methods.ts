...
export class YYYYMMDDClass {
  ...
  private normalizeDate(other: YYYYMMDDClass | Date | string): string & Brand<string, 'YYYYMMDD'> {
...
export type YYYYMMDDWithMethods = string & Brand<string, 'YYYYMMDD'> & {
...
export function createYYYYMMDDWithMethods(value: string & Brand<string, 'YYYYMMDD'>): YYYYMMDDWithMethods {
  return new Proxy(value as any, {
    get(target, prop) {
      ...
      switch (prop) {
        ...
        case 'addDays':
          return (days: number) => createYYYYMMDDWithMethods(addDaysToYYYYMMDD(target, days));
        case 'addMonths':
          return (months: number) => createYYYYMMDDWithMethods(addMonthsToYYYYMMDD(target, months));
        case 'daysBetween':
          return (other: Date | string) => {
            const otherDate = other instanceof Date ? dateToYYYYMMDD(other) : other;
...
export const YYYYMMDD = {
  gt: (date: string & Brand<string, 'YYYYMMDD'>, other: Date | string): boolean => {
    const otherDate = other instanceof Date ? dateToYYYYMMDD(other) : other;
    return date > otherDate;
  },
  
  gte: (date: string & Brand<string, 'YYYYMMDD'>, other: Date | string): boolean => {
    const otherDate = other instanceof Date ? dateToYYYYMMDD(other) : other;
    return date >= otherDate;
  },
  
  lt: (date: string & Brand<string, 'YYYYMMDD'>, other: Date | string): boolean => {
    const otherDate = other instanceof Date ? dateToYYYYMMDD(other) : other;
    return date < otherDate;
  },
  
  lte: (date: string & Brand<string, 'YYYYMMDD'>, other: Date | string): boolean => {
    const otherDate = other instanceof Date ? dateToYYYYMMDD(other) : other;
    return date <= otherDate;
  },
  
  eq: (date: string & Brand<string, 'YYYYMMDD'>, other: Date | string): boolean => {
    const otherDate = other instanceof Date ? dateToYYYYMMDD(other) : other;
    return date === otherDate;
  },

...
export function yyyymmddToDate(value: YYYYMMDD): Date {
	const year = Number.parseInt(value.substring(0, 4), 10);
	const month = Number.parseInt(value.substring(4, 6), 10) - 1; // JS months are 0-based
	const day = Number.parseInt(value.substring(6, 8), 10);
 ...
 */
export function formatYYYYMMDD(value: YYYYMMDD, separator = "-"): string {
			...
			switch (prop) {
				...
				case "addMonths":
					return (months: number) =>
						withMethods(addMonthsToYYYYMMDD(value, months));
				case "subtract":
					return (other: Date | string | YYYYMMDD | YYYYMMDDWithMethods) => {
						const otherDate = normalizeToYYYYMMDD(other);
...
function normalizeToYYYYMMDD(
	value: Date | string | YYYYMMDD | YYYYMMDDWithMethods,
): YYYYMMDD {
	if (value instanceof Date) {
		return dateToYYYYMMDD(value);
	}

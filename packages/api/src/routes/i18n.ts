...
const UI_TRANSLATIONS: Record<SupportedLanguage, Record<string, string>> = {
	...
	af: {
		...
		"message.unsavedChanges":
			"Jy het ongestoorde veranderinge. Wil jy voortgaan?",
...
export function createI18nRoutes(
	...
	const labelsController =
		client && cache ? new LabelsController(client, cache) : null;

	return (
		new Elysia({ prefix: "/i18n" })
...
Supported languages: ${SUPPORTED_LANGUAGES.join(", ")}`,
			...
			.get(
				"/labels/:lang",
				async ({ params: { lang }, set, headers }) => {
					if (!labelsController) {
						set.status = 501;
						throw new Error(
							"NOT_IMPLEMENTED: Labels endpoint requires client and cache configuration",
						);
					}

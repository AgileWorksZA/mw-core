				...
				async ({ body, set, headers }) => {
					...
					try {
						...
						if (errorMessage.includes("Unknown identifier")) {
							set.status = 400;
							throw new Error(
								`MWSCRIPT_ERROR: ${errorMessage}. Note: Direct table references like 'Count(TaxRate)' may not work in REST context.`,
							);
						}
			...
			.post(
				...
				async ({ params, body, set, headers }): Promise<any> => {
					...
					try {
						...
						if (errorMessage.includes("Unknown field")) {
							set.status = 400;
							throw new Error(
								`TEMPLATE_ERROR: ${errorMessage}. Check that all field names in square brackets exist in the ${table} table.`,
							);
						}
				...
				},
				{
					...
					body: t.Object({
						template: t.String({
							description:
								"Template string with field names in square brackets",
							examples: [
								"[Code] - [Description]",
								"[Name] (Phone: [Phone])",
...
function detectDataType(
	...
	if (/^\d{8}$/.test(trimmed)) {
		const year = Number.parseInt(trimmed.substring(0, 4));
		const month = Number.parseInt(trimmed.substring(4, 6));
		const day = Number.parseInt(trimmed.substring(6, 8));

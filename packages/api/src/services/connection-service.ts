...
export class ConnectionService {
  ...
  constructor(dbPath: string = './connections.db') {
    // Initialize database
    this.db = new Database(dbPath);
    this.initDatabase();

    // Generate encryption key from environment or create one
    const secret = process.env.API_ENCRYPTION_SECRET || 'default-dev-secret-change-in-production';
  ...
  async createConnection(config: MoneyWorksConfig & { description?: string }): Promise<ConnectionTokens> {
  ...
  async getConnectionByToken(token: string): Promise<MoneyWorksConfig | null> {
    ...
    // Decrypt config
    const config = JSON.parse(this.decrypt(connection.encrypted_config)) as MoneyWorksConfig;
  ...
  async deleteConnection(connectionId: string, token: string): Promise<boolean> {
  ...
  async cleanupOldConnections(daysOld: number = 30) {
    const stmt = this.db.prepare(`

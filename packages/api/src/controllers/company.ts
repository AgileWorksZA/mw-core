...
import { type CompanyField, CompanyFields } from "../schemas/company";
...
export class CompanyController {
	...
	async getCompanyInfo(
		...
		const result = await this.cache.getOrSet(
			...
			async () => {
				...
				const data = await this.fetchCompanyData(
					requestedFields as CompanyField[],
				);

				// Format based on preference
				return format === "nested" ? this.formatNested(data) : data;
			},
	...
	private formatNested(data: Record<string, string>): CompanyInfo {
		...
		// Address
		if (data.Address1 || data.Address2 || data.Address3 || data.Address4) {
			info.address = {
				line1: data.Address1,
				line2: data.Address2,
				line3: data.Address3,
		...
		// Contact
		if (data.Phone || data.Fax || data.Email || data.WebURL) {
			info.contact = {
				phone: data.Phone,
				fax: data.Fax,
				email: data.Email,
		...
		if (
			...
			data.CurrentPer ||
			data.BaseCurrency ||
			data.MultiCurrencyEnabled !== undefined ||
			data.ExtendedJobCosting !== undefined
		) {
			info.accounting = {
				periodsInYear: data.PeriodsInYear
					? Number.parseInt(data.PeriodsInYear)
					: undefined,
				currentPeriod: data.CurrentPer
					? Number.parseInt(data.CurrentPer)
					: undefined,
				baseCurrency: data.BaseCurrency,
				multiCurrencyEnabled:
					data.MultiCurrencyEnabled === "true" ||
					data.MultiCurrencyEnabled === "1",
				extendedJobCosting:
					data.ExtendedJobCosting === "true" || data.ExtendedJobCosting === "1",
			};
		}
		...
		if (data.GSTCycleMonths || data.GstNum) {
			info.tax = {
				...
				gstCycleMonths: data.GSTCycleMonths
					? Number.parseInt(data.GSTCycleMonths)
					: undefined,

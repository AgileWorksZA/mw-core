...
export class LabelsController {
	...
	private async translateLabels(
		...
		if (englishResponse.enumerated) {
			for (const [enumField, enumLabels] of Object.entries(
				englishResponse.enumerated,
			)) {
				translatedEnumerated[enumField] = {};
				for (const [value, label] of Object.entries(enumLabels)) {
					translatedEnumerated[enumField][value] =
		...
		return {
			...
			enumerated:
				Object.keys(translatedEnumerated).length > 0
					? translatedEnumerated
					: undefined,
		};
	}
			...
			async () => {
				...
				await Promise.all(
					tables.map(async (tableName: string) => {
						try {
							allLabels[tableName] = await this.getTableLabels(
								tableName,
								language,
							);
						} catch (error) {

...
export const MONEYWORKS_LOGIN_FIELDS = [
	...
	},
	{
		...
		privilegeEncoding: {
			description:
				"65-character encoded string representing privilege settings",
			accessMethod:
				"Use Allowed('privilegeName') function to test specific privileges",
			...
			stringBasedAccess:
				"Privileges accessed by exact string names like 'Pay Invoices', 'Enter Receipts', 'Users and Security (Administrator)'",
			customPrivileges:
				"User Privilege 1-6 available for custom script access control",
		},
	},
	{
		fieldName: "Role",
...
export const MONEYWORKS_LOGIN_BUSINESS_RULES = [
	...
	},
	{
		...
		ruleType: "uniqueness" as const,
		canonicalRule:
			"User Initials should be unique for tracking and attribution purposes",
		manualSource: "moneyworks_appendix_login_file.html",
	},
	{
		entitySource: "Login",
		...
		canonicalRule:
			"Passwords are stored encrypted and never accessible in plain text",
		manualSource: "moneyworks_appendix_login_file.html",
	},
	{
		entitySource: "Login",
		...
		canonicalRule:
			"Privilege map encodes specific access rights and restrictions for user",
		manualSource: "moneyworks_appendix_login_file.html",
	},
	{
		entitySource: "Login",
		...
		canonicalRule:
			"65-character encoded string representing on/off state of 80+ individual privileges",
	...
	},
	{
		...
		canonicalRule:
			"Privileges tested using Allowed('privilegeName') function with exact string names",
	...
	},
	{
		...
		canonicalRule:
			"Privileges organized in 11 categories: Administration, Purchases, Creditors, Sales, Debtors, Cash, General Ledger, Jobs, Products, Names, Other",
		manualSource: "moneyworks_privileges_moneyworks_privileges.html",
	},
	{
		entitySource: "Login",
		...
		canonicalRule:
			"User Privilege 1-6 available for custom script access control using Allowed() function",
		manualSource: "moneyworks_privileges_moneyworks_privileges.html",
	},
	{
		entitySource: "Login",
		...
		canonicalRule:
			"Role determines base privilege set that can be customized per user",
		manualSource: "moneyworks_appendix_login_file.html",
	},
	{
		entitySource: "Login",
		...
		canonicalRule:
			"Security level provides hierarchical access control mechanism",
		manualSource: "moneyworks_appendix_login_file.html",
	},
	{
		entitySource: "Login",
	...
	},
	{
		...
		canonicalRule:
			"Category groups users by business function or organizational unit",
		manualSource: "moneyworks_appendix_login_file.html",
	},
] as const;
...
export const LOGIN_UNIVERSALITY_EXAMPLES = [
	{
		...
		universalApplicability:
			"Role-based access applies to any business hierarchy",
	},
	{
		businessType: "legal",
		...
		universalApplicability:
			"Professional services use same authentication model",
	},
	{
		...
		universalApplicability:
			"Manufacturing roles work with universal access control",
	},
	{
...
export const LOGIN_ENTITY_RELATIONSHIPS = [
	{
		...
		businessRule:
			"User Initials appear in transaction creation and modification tracking",
		manualSource: "moneyworks_appendix_login_file.html",
	},
	{
		sourceEntity: "Login",
 ...
 */
export function validateLoginName(name: string): {
	isValid: boolean;
	errors: string[];
} {
	const errors: string[] = [];
 ...
 */
export function validateLoginSecurityLevel(level: number): {
	isValid: boolean;
	errors: string[];
} {
	const errors: string[] = [];
 ...
 */
export function validatePrivilegeName(privilegeName: string): {
	isValid: boolean;
	errors: string[];
} {
	const errors: string[] = [];
	...
	if (!MONEYWORKS_CANONICAL_PRIVILEGES.includes(privilegeName as any)) {
		...
		errors.push(
			`Valid privileges include: ${MONEYWORKS_CANONICAL_PRIVILEGES.slice(0, 5).join(", ")}...`,
		);
	}
 ...
 */
export function validatePrivilegesField(privileges: string): {
	isValid: boolean;
	errors: string[];
} {
	const errors: string[] = [];
 ...
 */
export function validateLoginRecord(login: MoneyWorksLogin): {
	isValid: boolean;
	errors: string[];
} {
	const errors: string[] = [];

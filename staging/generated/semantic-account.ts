...
export interface AccountSemanticTag {
  category: "balance_sheet" | "income_statement" | "cash_flow" | "tax" | "management";
...
export interface MoneyWorksAccountSemanticMCP {
  ...
  analyze_account_balances(period: string, accountCodes?: string[]): Promise<BalanceAnalysis>;
...
export interface BalanceAlert {
  accountCode: string;
  alertType: "negative_asset" | "credit_limit" | "unusual_balance" | "zero_balance";
...
export function parseAccountPurpose(naturalLanguage: string): AccountPurpose {
  ...
  // Cash and banking
  if (lower.includes("cash") || lower.includes("bank") || lower.includes("checking")) {
    return AccountPurpose.CASH_MANAGEMENT;
  }
  
  // Customer related
  if (lower.includes("customer") || lower.includes("receivable") || lower.includes("debtor")) {
    return AccountPurpose.CUSTOMER_RECEIVABLES;
  }
  
  // Supplier related
  if (lower.includes("supplier") || lower.includes("payable") || lower.includes("creditor")) {
  ...
  if (lower.includes("expense") || lower.includes("cost")) {
    if (lower.includes("office") || lower.includes("admin")) return AccountPurpose.ADMINISTRATIVE_COSTS;
    if (lower.includes("marketing") || lower.includes("advertising")) return AccountPurpose.MARKETING_EXPENSES;
    if (lower.includes("goods") || lower.includes("material")) return AccountPurpose.COST_OF_GOODS;
    return AccountPurpose.OPERATING_EXPENSES;
  }
  
  // Assets
  if (lower.includes("asset") || lower.includes("equipment") || lower.includes("property")) {
    if (lower.includes("equipment") || lower.includes("computer")) return AccountPurpose.EQUIPMENT_ASSETS;
    if (lower.includes("property") || lower.includes("building")) return AccountPurpose.PROPERTY_ASSETS;
...
export function determineAccountCategory(type: string): AccountCategory {
  switch (type) {
    case "A": return AccountCategory.CURRENT_ASSET;
    case "F": return AccountCategory.FIXED_ASSET;
    case "T": return AccountCategory.CURRENT_ASSET; // Term assets
    case "L": return AccountCategory.CURRENT_LIABILITY;
    case "M": return AccountCategory.LONG_TERM_LIABILITY;
    case "H": return AccountCategory.EQUITY;
    case "S": return AccountCategory.OPERATING_INCOME;
    case "I": return AccountCategory.NON_OPERATING_INCOME;
    case "C": return AccountCategory.OPERATING_EXPENSE; // Cost of sales
    case "E": return AccountCategory.OPERATING_EXPENSE;
    default: return AccountCategory.OPERATING_EXPENSE;
  }
}
...
export function generateAccountSemanticTags(account: SemanticAccount): AccountSemanticTag[] {
  const tags: AccountSemanticTag[] = [];
  
  // Balance sheet vs income statement
  if (["A", "L", "F", "T", "M", "H"].includes(account.classification.moneyWorksType)) {
    tags.push({ 
  ...
  // Liquidity tags
  if (account.purpose === AccountPurpose.CASH_MANAGEMENT || 
...
export function upgradeAccountToSemantic(legacy: any): SemanticAccount {
  ...
  return {
    ...
    purpose: purpose,
    status: legacy.Flags & 0x1 ? AccountStatus.HEADING_ONLY : AccountStatus.ACTIVE,
    ...
    reporting: {
      ...
      customCategories: [legacy.Category, legacy.Category2, legacy.Category3, legacy.Category4].filter(Boolean),
...
export function enrichAccountBusinessContext(account: SemanticAccount): AccountBusinessContext {

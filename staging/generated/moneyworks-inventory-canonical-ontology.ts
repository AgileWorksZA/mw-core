...
export const MONEYWORKS_INVENTORY_FIELDS = [
	{
		...
		relationshipRule:
			"Used when product has batch/lot tracking with expiration",
	},
	{
	...
	},
	{
		...
		canonicalDescription:
			"The sequence number of the product master record for the item",
		manualSource: "moneyworks_appendix_inventory.html",
		isRequired: true,
		isIndexed: true,
		relationshipTarget: "Products",
	...
	},
	{
		...
		dataType: "N" as const,
		canonicalDescription:
			"The qty (stock on hand) of the item at this location. Use the SOHForLocation(location) function for the stock at a given location.",
		manualSource: "moneyworks_appendix_inventory.html",
		isRequired: true,
	},
	{
		...
		dataType: "N" as const,
		canonicalDescription:
			"The total entered stock count for a stock take. Use the StocktakeNewQtyForLocation(location) function for the stock at a given location.",
		manualSource: "moneyworks_appendix_inventory.html",
	},
	{
		...
		dataType: "N" as const,
		canonicalDescription:
			"The total stock on hand when a stock take is commenced. Use the StocktakeStartQtyForLocation(location) function for the stock at a given location.",
		manualSource: "moneyworks_appendix_inventory.html",
	},
] as const;
...
export const MONEYWORKS_INVENTORY_FUNCTIONS = [
	{
		...
		description:
			'Returns the quantity of stock on hand for the specified location. Use empty string ("") for default location.',
		manualSource: "moneyworks_appendix_inventory.html",
	},
	{
		...
		description:
			"Returns the entered stock count for a stock take at the specified location",
		manualSource: "moneyworks_appendix_inventory.html",
	},
	{
		...
		description:
			"Returns the stock on hand when stock take was commenced at the specified location",
		manualSource: "moneyworks_appendix_inventory.html",
	},
] as const;
...
export function validateInventoryIdentifierCanonical(identifier: string): {
	...
	if (identifier.length > 31) {
		warnings.push(
			"MoneyWorks inventory identifier maximum length is 31 characters",
		);
	}
...
export function validateProductSequenceRelationship(productSeq: number): {
	...
	if (!productSeq || productSeq <= 0) {
		warnings.push(
			"ProductSeq must reference a valid Product record sequence number",
		);
	}
...
export function parseInventoryRecordKey(key: string): {
	...
	const productSeq = Number.parseInt(parts[0], 10);

	return {
		productSeq,
		location: parts[1],
		identifier: parts[2],
		isValid: !Number.isNaN(productSeq) && productSeq > 0,
	};
}

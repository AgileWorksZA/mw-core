...
export const MONEYWORKS_TRANSACTION_FIELDS: MoneyWorksFieldDefinition[] = [
	...
	},
	{
		...
		canonicalDescription:
			"For debtor invoices, customer's Order No. For creditor invoices, supplier's invoice number.",
		manualSource: "moneyworks_appendix_transactions.html",
	},
	{
...
export const MONEYWORKS_ACCOUNT_FIELDS: MoneyWorksFieldDefinition[] = [
	...
	},
	{
		...
		maxLength: 8,
		canonicalDescription:
			"The primary category code for the account, used for grouping accounts for reporting and enquiry. May be empty. References the General table (Kind='C').",
		manualSource: "moneyworks_appendix_accounts.html",
		isIndexed: true,
	},
];
 ...
 */
export function validateCanonicalTerminology(
	concept: string,
	context: string,
): {
	isCanonical: boolean;
	...
	if (concept.toLowerCase().includes("supplier")) {
		issues.push(
			"Uses 'Supplier' instead of MoneyWorks canonical term 'Creditor'",
		);
		suggestions.push(
			"Replace 'Supplier' with 'Creditor' to match MoneyWorks terminology",
		);
	}

	if (concept.toLowerCase().includes("customer")) {
		issues.push(
			"Uses 'Customer' instead of MoneyWorks canonical term 'Debtor'",
		);
		suggestions.push(
			"Replace 'Customer' with 'Debtor' to match MoneyWorks terminology",
		);
	}

	if (concept.toLowerCase().includes("vendor")) {
		issues.push(
			"Uses 'Vendor' instead of MoneyWorks canonical term 'Creditor'",
		);
		suggestions.push(
			"Replace 'Vendor' with 'Creditor' to match MoneyWorks terminology",
		);
	}
...
export function getCanonicalTransactionTypeDefinition(
	...
	return (
		MONEYWORKS_TRANSACTION_TYPE_DEFINITIONS.find((def) => def.code === code) ||
		null
	);
}
...
export function getCanonicalAccountTypeDefinition(
	...
	return (
		MONEYWORKS_ACCOUNT_TYPE_DEFINITIONS.find((def) => def.code === code) || null
	);
}
...
export function validateFieldDefinition(
	...
	if (!canonicalField) {
		return {
			isValid: false,
			errors: [
				`Field '${fieldName}' not found in MoneyWorks canonical specification`,
			],
		};
	}
	...
	if (
		...
		canonicalField.maxLength &&
		definition.maxLength > canonicalField.maxLength
	) {
		errors.push(
			`Field '${fieldName}' max length ${definition.maxLength} exceeds MoneyWorks canonical max ${canonicalField.maxLength}`,
		);
	}

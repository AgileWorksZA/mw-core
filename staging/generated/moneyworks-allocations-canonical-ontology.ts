...
export const MONEYWORKS_ALLOCATION_FIELDS = [
	...
	},
	{
		...
		relationshipRule:
			"Pattern matching against transaction descriptions, payee names, or other transaction fields",
	},
	{
		fieldName: "MatchName",
		dataType: "T" as const,
	...
	},
	{
		...
		relationshipTarget: "Accounts.Code",
		relationshipRule:
			"Optional fourth account for maximum flexibility split allocation",
	},
	{
		...
		relationshipRule:
			"Allocation amount/percentage for first split account, interpretation depends on SplitMode",
	},
	{
		...
		relationshipRule:
			"Allocation amount/percentage for second split account, interpretation depends on SplitMode",
	},
	{
		...
		relationshipRule:
			"Allocation amount/percentage for third split account, interpretation depends on SplitMode",
	},
	{
		fieldName: "SplitMode",
		dataType: "N" as const,
		...
		isRequired: true,
		relationshipRule:
			"Determines how SplitAmount values are interpreted (percentage, fixed amount, ratio, etc.)",
	},
] as const;
...
export const MONEYWORKS_ALLOCATION_BUSINESS_RULES = [
	{
		...
		canonicalRule:
			"Rules processed in priority order, lower numbers processed first",
		manualSource: "moneyworks_appendix_allocation_file.html",
		businessContext:
			"Multiple rules may match same transaction, priority determines which rule applies",
	},

	{
		...
		canonicalRule:
			"Supports text matching against transaction fields including description, payee, reference",
		manualSource: "moneyworks_appendix_allocation_file.html",
		businessContext:
			"Pattern matching enables automated transaction categorization during bank import",
	},

	{
		...
		canonicalRule:
			"All split accounts must reference valid accounts in chart of accounts",
		manualSource: "moneyworks_appendix_allocation_file.html",
		businessContext:
			"Account validation ensures allocation targets exist and are properly configured",
	},

	{
		...
		canonicalRule:
			"Split amounts interpreted based on SplitMode: percentage (0-100), fixed amounts, or ratios",
		manualSource: "moneyworks_appendix_allocation_file.html",
		businessContext:
			"Flexible calculation modes support various allocation scenarios (overhead, cost distribution, etc.)",
	},

	{
		...
		canonicalRule:
			"Human-readable rule name for identification and management purposes",
		manualSource: "moneyworks_appendix_allocation_file.html",
		businessContext:
			"Named rules enable organized allocation rule management and troubleshooting",
	},

	{
		...
		canonicalRule:
			"System automatically tracks rule modification timestamps for audit trail",
		manualSource: "moneyworks_appendix_allocation_file.html",
		businessContext:
			"Audit trail supports compliance and troubleshooting of allocation rule changes",
	},
];
...
export const MONEYWORKS_ALLOCATION_USAGE_PATTERNS = [
	{
		...
		businessTypes: ["all"],
		allocationPattern:
			"Match supplier name patterns to automatically split invoices to appropriate expense accounts",
		canonicalExample: {
		...
		universalApplicability:
			"Any business needs automated expense categorization from bank feeds",
	},

	{
		...
		allocationPattern:
			"Split utility bills across departments based on predetermined percentages",
		canonicalExample: {
		...
		universalApplicability:
			"Multi-location or multi-department businesses need automated overhead distribution",
	},

	{
		...
		businessTypes: ["all"],
		allocationPattern:
			"Automatically split credit card transactions based on merchant patterns",
		canonicalExample: {
		...
		universalApplicability:
			"Any business using credit cards needs merchant-based categorization",
	},

	{
		...
		businessTypes: ["all"],
		allocationPattern:
			"Split recurring services (insurance, software subscriptions) across cost centers",
		canonicalExample: {
		...
		universalApplicability:
			"All businesses have recurring services requiring departmental allocation",
	},
];
...
export function validateAllocationRuleCanonical(
	...
	if (splitAmounts.length !== splitAccounts.length) {
		warnings.push(
			"Number of split amounts must match number of split accounts",
		);
	}
	...
	if (splitMode === MoneyWorksAllocationSplitMode.PERCENTAGE) {
		...
		if (totalPercentage < 100 && splitAccounts.length === splitAmounts.length) {
			recommendations.push(
				"Consider using REMAINING_TO_LAST mode for automatic remainder handling",
			);
		}
	}
...
export function getAllocationComplexityLevel(splitAccounts: string[]): {
	...
	if (accountCount === 2) {
		return {
			level: "moderate",
			explanation:
				"Two-way split - moderate complexity for departmental or category splitting",
		};
	}
	if (accountCount === 3) {
		return {
			level: "complex",
			explanation:
				"Three-way split - complex allocation for multi-departmental or detailed categorization",
		};
	}
	return {
		level: "maximum",
		explanation:
			"Four-way split - maximum MoneyWorks allocation complexity for sophisticated cost distribution",
	};
}

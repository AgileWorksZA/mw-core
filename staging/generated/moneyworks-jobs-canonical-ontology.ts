...
export const MONEYWORKS_JOB_FIELDS = [
	...
	},
	{
		...
		canonicalDescription:
			'The status of the job. "QU" for quoted, "OP" for active, "CO" for complete',
		manualSource: "moneyworks_appendix_jobs.html",
		isRequired: true,
		isIndexed: true,
	},
	...
	},
	{
		...
		maxLength: 11,
		canonicalDescription:
			"The code of the client for whom the job is being done. Must be a debtor.",
		manualSource: "moneyworks_appendix_jobs.html",
		isRequired: true,
		relationshipTarget: "Names",
		relationshipRule:
			"Must reference valid Name with CustomerType = 2 (Debtor)",
	},
	{
		fieldName: "Project",
		dataType: "T" as const,
		...
		relationshipRule:
			"Must reference valid Job.Code for hierarchical project structure",
	},
	{
		fieldName: "Manager",
		dataType: "T" as const,
	...
	},
	{
		...
		dataType: "S" as const,
		canonicalDescription:
			"The date that this job record was last changed. Transactions related to the job do not change the modification date of the job record",
		manualSource: "moneyworks_appendix_jobs.html",
	},
] as const;
...
export function validateJobStatusCanonical(status: string): {
	...
	if (!validStatuses.includes(status)) {
		warnings.push(
			`Invalid Job Status '${status}'. MoneyWorks canonical values: QU (quoted), OP (active), CO (complete)`,
		);
	}
...
export function validateJobClientCanonical(clientCode: string): {
	...
	if (!clientCode || clientCode.trim().length === 0) {
		...
		warnings.push(
			`Client Code '${clientCode}' exceeds maximum length of 11 characters`,
		);
	}
	...
	return {
		...
		warnings,
		requirement:
			"Client must reference valid Name with CustomerType = 2 (Debtor)",
	};
}
...
export function validateJobColourCanonical(colour: number): {
	...
	if (colour < 0 || colour > 7) {
		warnings.push(
			`Job Colour ${colour} out of range. MoneyWorks canonical range: 0-7`,
		);
	}
 ...
 */
export function validateProjectHierarchyCanonical(
	jobCode: string,
	projectCode?: string,
): {
	isValid: boolean;
	...
	if (projectCode && projectCode.length > 9) {
		warnings.push(
			`Project Code '${projectCode}' exceeds maximum length of 9 characters`,
		);
	}
	...
	return {
		...
		warnings,
		hierarchyNote:
			"MoneyWorks supports nested project structure where jobs can belong to other jobs",
	};
}
 ...
 */
export function getCanonicalJobStatusExplanation(
	status: MoneyWorksJobStatus,
): string {
	const statusDef = MONEYWORKS_JOB_STATUS_DEFINITIONS.find(
		(def) => def.status === status,
	);
	return statusDef
		? `${statusDef.canonicalName}: ${statusDef.businessContext}`
		: "Unknown job status";
}
 ...
 */
export function validateJobFinancialsCanonical(
	quote?: number,
	billed?: number,
	markup?: number,
	...
	if (quote !== undefined && billed !== undefined) {
		if (billed > quote) {
			insights.push(
				"Billed amount exceeds quote - may indicate scope changes or additional work",
			);
		}
		...
		if (percentComplete !== undefined) {
			...
			if (completionGap > 20) {
				insights.push(
					`Billing ratio (${billingRatio.toFixed(1)}%) and completion (${percentComplete}%) significantly different`,
				);
			}
		}
	}
...
export function getCanonicalJobEntityRelationships(): {
	...
	return {
		requiredReferences: ["Names.Code (Client field - must be Debtor)"],
		optionalReferences: ["Jobs.Code (Project field - hierarchical structure)"],
		businessRules: [

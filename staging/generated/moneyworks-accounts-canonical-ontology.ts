...
export const MONEYWORKS_ACCOUNT_FIELDS = [
	{
		...
		canonicalDescription:
			"Code in accountant's chart that corresponds to this account.",
	...
	},
	{
		...
		maxLength: 7,
		canonicalDescription:
			"The category code for the account (blank if Category is None).",
		manualSource: "moneyworks_appendix_accounts.html",
		relationshipTarget: "General.Code",
		relationshipRule:
			"References General Classifications with Kind='C' for account categorization",
	},
	{
		fieldName: "Category2",
		dataType: "T" as const,
	...
	},
	{
		...
		relationshipRule:
			"Primary key for account entity, referenced by transactions",
	},
	{
		fieldName: "Colour",
		dataType: "N" as const,
	...
	},
	{
		...
		maxLength: 3,
		canonicalDescription:
			"The currency code (empty for local currency accounts)",
		manualSource: "moneyworks_appendix_accounts.html",
	...
	},
	{
		...
		relationshipRule:
			"Human-readable account name for transactions and reports",
	},
	{
		fieldName: "EBITDA",
		dataType: "T" as const,
		maxLength: 1,
		canonicalDescription:
			'Tag to specify EBITDA status of account for reporting ("I" for Interest, "T" for Tax, "D" for Depreciation/Amortisation, otherwise blank)',
		manualSource: "moneyworks_appendix_accounts.html",
	...
	},
	{
		...
		relationshipTarget: "General.Code",
		relationshipRule:
			"References General Classifications with Kind='G' for departmental grouping",
	},
	{
		fieldName: "LastModifiedTime",
		dataType: "S" as const,
		canonicalDescription:
			"The date and time that this account was last changed. This means a change to the account record itself, not a change to the account balance.",
		manualSource: "moneyworks_appendix_accounts.html",
		relationshipTarget: "audit trail",
	...
	},
	{
		...
		relationshipRule:
			"Access control level for account visibility and modification",
	},
	{
		fieldName: "System",
		dataType: "A" as const,
		maxLength: 2,
		canonicalDescription:
			'The "system" account type. The account types Bank Account, Profit & Loss, Accounts Receivable, Accounts Payable, GST Received and GST Paid are special system account types. Accounts of these types contain the codes: "BK", "PL", "AR", "AP", "GR", or "GP" (respectively) in the System field. Ordinary account types have "  " (2 spaces) in the System field.',
		manualSource: "moneyworks_appendix_accounts.html",
		isRequired: true,
		relationshipTarget: "MoneyWorksSystemAccountType",
		relationshipRule:
			"System account designation for special MoneyWorks functionality",
	},
	{
	...
	},
	{
		...
		maxLength: 2,
		canonicalDescription:
			"The account type. This will be one of IN, SA, EX, CS, CA, CL, FA, TA, TL or SF for Income, Sales, Expense, Cost of Sales, Current Asset, Current Liability, Fixed Asset, Term Asset, Term Liability or Shareholder's Funds respectively.",
		manualSource: "moneyworks_appendix_accounts.html",
		isRequired: true,
	...
	[
		{
			...
			systemFunctionality:
				"Enables bank reconciliation, cheque management, and statement import",
		},
		{
			...
			systemFunctionality:
				"Automatic year-end transfer destination for income and expense accounts",
		},
		{
		...
		},
		{
			...
			systemFunctionality:
				"Automatic posting for creditor invoices and payments",
		},
		{
			...
			systemFunctionality:
				"Automatic GST/VAT collection tracking for tax reporting",
		},
		{
			...
			systemFunctionality:
				"Automatic GST/VAT payment tracking for tax reporting",
		},
		{
			...
			systemFunctionality:
				"Regular account behavior without automatic posting rules",
		},
	];
...
export function validateAccountTypeCanonical(type: string): {
	...
	if (!validTypes.includes(type as MoneyWorksAccountType)) {
		warnings.push(
			`Invalid account type ${type}. MoneyWorks canonical types: ${validTypes.join(", ")}`,
		);
	}
...
export function validateSystemAccountTypeCanonical(system: string): {
	...
	if (!validSystemTypes.includes(system as MoneyWorksSystemAccountType)) {
		warnings.push(
			`Invalid system account type ${system}. MoneyWorks canonical system types: ${validSystemTypes.join(", ")}`,
		);
	}
 ...
 */
export function getCanonicalAccountTypeExplanation(
	type: MoneyWorksAccountType,
): string {
	const typeDef = MONEYWORKS_ACCOUNT_TYPE_DEFINITIONS.find(
		(def) => def.type === type,
	);
	if (!typeDef) {
 ...
 */
export function getCanonicalSystemAccountTypeExplanation(
	systemType: MoneyWorksSystemAccountType,
): string {
	const systemDef = MONEYWORKS_SYSTEM_ACCOUNT_TYPE_DEFINITIONS.find(
		(def) => def.type === systemType,
	);
	if (!systemDef) {
 ...
 */
export function isCanonicalBalanceSheetAccount(
	type: MoneyWorksAccountType,
): boolean {
	return [
 ...
 */
export function isCanonicalIncomeStatementAccount(
	type: MoneyWorksAccountType,
): boolean {
	return [
 ...
 */
export function isCanonicalLiabilityAccount(
	type: MoneyWorksAccountType,
): boolean {
	return [
		MoneyWorksAccountType.CURRENT_LIABILITY,
...
export function increasesWithCredits(type: MoneyWorksAccountType): boolean {
	...
	return (
		isCanonicalLiabilityAccount(type) ||
		type === MoneyWorksAccountType.SHAREHOLDERS_FUNDS ||
		type === MoneyWorksAccountType.INCOME ||
		type === MoneyWorksAccountType.SALES
	);
 ...
 */
export function getCanonicalAccountNormalBalance(
	type: MoneyWorksAccountType,
): "debit" | "credit" {
 ...
 */
export function getCanonicalFinancialStatementSection(
	type: MoneyWorksAccountType,
): string {
	const typeDef = MONEYWORKS_ACCOUNT_TYPE_DEFINITIONS.find(
		(def) => def.type === type,
 ...
 */
export function testAccountFlag(
	flags: number,
	flag: MoneyWorksAccountFlags,
): boolean {
	return (flags & flag) !== 0;
}
...
export function getAccountFlagDescriptions(flags: number): string[] {
	...
	if (
		testAccountFlag(flags, MoneyWorksAccountFlags.SYNCHRONISE_CHEQUE_NUMBERS)
	) {

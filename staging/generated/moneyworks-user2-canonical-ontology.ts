...
export const MONEYWORKS_USER2_FIELDS = [
	...
	},
	{
		...
		maxLength: undefined,
		canonicalDescription:
			"An unsigned integer greater than #FFFF (65,535)--lower valued keys are reserved for plug-ins and must be pre-allocated by Cognito.",
		manualSource: "moneyworks_appendix_user2_file.html",
		isRequired: true,
		isIndexed: true,
		businessRule:
			"Must be > 65535 for user scripts, <= 65535 reserved for plug-ins",
	},
	{
		fieldName: "Key",
		dataType: "T" as const,
		maxLength: 28,
		canonicalDescription:
			"A unique key to identify the record. Use this in conjunction with a (pre-allocated) DevKey to ensure your own storage",
		manualSource: "moneyworks_appendix_user2_file.html",
		isRequired: true,
		isIndexed: true,
...
export const MONEYWORKS_USER2_BUSINESS_RULES = [
	{
		...
		canonicalRule:
			"DevKey must be > 65535 for user scripts; <= 65535 reserved for plug-ins and pre-allocated by Cognito",
		manualSource: "moneyworks_appendix_user2_file.html",
	},
	{
		entitySource: "User2",
	...
	},
	{
		...
		canonicalRule:
			"Record update is complete rewrite - cannot update individual fields",
		manualSource: "moneyworks_appendix_user2_file.html",
	},
	{
		entitySource: "User2",
		...
		canonicalRule:
			"If only DevKey and Key are present and all other fields missing, record is deleted",
		manualSource: "moneyworks_appendix_user2_file.html",
	},
	{
		entitySource: "User2",
		...
		canonicalRule:
			"Records can be inserted with pseudo-map ':/User2', XML import, or SetPersistent function",
		manualSource: "moneyworks_appendix_user2_file.html",
	},
	{
		entitySource: "User2",
		...
		canonicalRule:
			"Tab-delimited import: DevKey, Key, then fields in table order",
		manualSource: "moneyworks_appendix_user2_file.html",
	},
	{
		entitySource: "User2",
		...
		canonicalRule:
			"Apart from DevKey and Key, all fields are optional; missing fields treated as empty/zero",
		manualSource: "moneyworks_appendix_user2_file.html",
	},
] as const;
...
export const USER2_ENTITY_RELATIONSHIPS = [
	{
		...
		cardinality: "n/a",
		businessRule:
			"User2 entity is deliberately isolated for enhanced script storage",
		manualSource: "moneyworks_appendix_user2_file.html",
	},
] as const;
 ...
 */
export function validateUser2DevKey(devKey: number): {
	isValid: boolean;
	errors: string[];
} {
	const errors: string[] = [];
	...
	if (devKey <= 65535) {
		errors.push(
			"DevKey must be greater than 65535 for user scripts (values <= 65535 reserved for plug-ins)",
		);
	}
 ...
 */
export function validateUser2Key(key: string): {
	isValid: boolean;
	errors: string[];
} {
	const errors: string[] = [];
 ...
 */
export function validateUser2TextField(
	value: string,
	fieldName: string,
 ...
 */
export function validateUser2Record(user2: MoneyWorksUser2): {
	isValid: boolean;
	errors: string[];
} {
	const errors: string[] = [];

 ...
 */
export interface MoneyWorksAccountCategory
	extends MoneyWorksGeneralClassification {
 ...
 */
export interface MoneyWorksDepartmentClassification
	extends MoneyWorksGeneralClassification {
 ...
 */
export interface MoneyWorksDepartmentGroup
	extends MoneyWorksGeneralClassification {
...
export const GeneralClassificationValidation = {
	...
	fields: {
		Code: {
			...
			description:
				"Classification code with prefix (C/D/S), max 5 chars, alphanumeric with underscores",
		},
		Description: {
			required: true,
			maxLength: 25,
	...
	businessRules: {
		...
		prefixValidation: {
			C: {
				...
				usage:
					"Optional association with account codes (one or more categories per account)",
			...
			S: {
				...
				keyFeature:
					"Creates sub-ledgers through department-account association",
				constraint:
					"Only groups (not individual departments) can be associated with accounts",
				requirement:
					"Group must contain at least one department before account association",
			},
		},
	...
	relationships: {
		...
		departmentGroups: {
			...
			relationship:
				"Department Groups enable sophisticated departmental accounting",
 ...
 */
export const isAccountCategory = (
	classification: MoneyWorksGeneralClassification,
): classification is MoneyWorksAccountCategory => {
	return classification.Code.startsWith("C");
};

export const isDepartmentClassification = (
	classification: MoneyWorksGeneralClassification,
): classification is MoneyWorksDepartmentClassification => {
	return classification.Code.startsWith("D");
};

export const isDepartmentGroup = (
	classification: MoneyWorksGeneralClassification,
): classification is MoneyWorksDepartmentGroup => {
...
export const getClassificationType = (
	code: string,
): GeneralClassificationType | null => {
	const prefix = code.charAt(0).toUpperCase();
	switch (prefix) {
		case "C":
			return GeneralClassificationType.ACCOUNT_CATEGORY;
		case "D":
			return GeneralClassificationType.DEPARTMENT_CLASSIFICATION;
		case "S":
			return GeneralClassificationType.DEPARTMENT_GROUP;
		default:
			return null;
	}
};

...
export const MONEYWORKS_USER_FIELDS = [
	...
	},
	{
		...
		maxLength: 9,
		canonicalDescription:
			"A unique key to identify the record. Note that many plug-ins use their own set of keys, so you need to manage any conflicts.",
		manualSource: "moneyworks_appendix_user_file.html",
		isRequired: true,
		isIndexed: true,
	...
	},
	{
		...
		isRequired: false,
		businessRule:
			"If empty when updating existing record, the record will be deleted",
	},
] as const;
...
export const MONEYWORKS_USER_BUSINESS_RULES = [
	...
	},
	{
		...
		canonicalRule:
			"If Data is empty when updating existing record, the record will be deleted",
		manualSource: "moneyworks_appendix_user_file.html",
	},
	{
		entitySource: "User",
		...
		canonicalRule:
			"Plug-ins use their own set of keys, requiring conflict management",
		manualSource: "moneyworks_appendix_user_file.html",
	},
	{
		entitySource: "User",
		...
		canonicalRule:
			"Records can be inserted by importing with pseudo-map ':/User' or using SetPersistent function",
		manualSource: "moneyworks_appendix_user_file.html",
	},
	{
		entitySource: "User",
		...
		canonicalRule:
			"Import format: Key (first field) + Data (second field, tab-separated)",
		manualSource: "moneyworks_appendix_user_file.html",
	},
] as const;
...
export const USER_UNIVERSALITY_EXAMPLES = [
	{
		...
		universalApplicability:
			"Generic key-value storage works for any configuration",
	},
	{
		businessType: "legal",
		...
		universalApplicability:
			"Any professional service can use same storage pattern",
	},
	{
		...
		universalApplicability:
			"Manufacturing-specific data stored in generic structure",
	},
	{
...
export const USER_ENTITY_RELATIONSHIPS = [
	{
		...
		cardinality: "n/a",
		businessRule:
			"User entity is deliberately isolated for generic script storage",
		manualSource: "moneyworks_appendix_user_file.html",
	},
] as const;
 ...
 */
export function validateUserKey(key: string): {
	isValid: boolean;
	errors: string[];
} {
	const errors: string[] = [];
 ...
 */
export function validateUserData(data: string): {
	isValid: boolean;
	errors: string[];
} {
	const errors: string[] = [];
 ...
 */
export function validateUserRecord(user: MoneyWorksUser): {
	isValid: boolean;
	errors: string[];
} {
	const errors: string[] = [];

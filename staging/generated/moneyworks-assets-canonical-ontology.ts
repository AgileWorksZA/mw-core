...
export const MONEYWORKS_ASSET_FIELDS = [
	...
	},
	{
		...
		canonicalDescription:
			"The quantity of the asset (normally 1, but might be a set of similar assets such as desks)",
		manualSource: "moneyworks_appendix_assets.html",
	},
	{
	...
	},
	{
		...
		dataType: "N" as const,
		canonicalDescription:
			"The sequencenumber of the purchase transaction for the asset",
		manualSource: "moneyworks_appendix_assets.html",
		relationshipTarget: "Transactions",
		relationshipRule:
			"References Transaction.SequenceNumber for asset acquisition",
	},
	{
		...
		relationshipTarget: "Transactions",
		relationshipRule:
			"References Transaction.SequenceNumber for asset disposal",
	},
	{
		fieldName: "Location",
		dataType: "T" as const,
	...
	},
	{
		...
		maxLength: 5,
		canonicalDescription:
			"The asset department (must be a MoneyWorks Department)",
		manualSource: "moneyworks_appendix_assets.html",
	...
	},
	{
		...
		maxLength: 3,
		canonicalDescription:
			"The asset status (NEW, ACT - active, NDP - non-depreciable, OTH - other, DSP - disposed)",
		manualSource: "moneyworks_appendix_assets.html",
	},
	{
...
export const MONEYWORKS_ASSET_RELATIONSHIPS = [
	{
		...
		targetField: "Code",
		canonicalDescription:
			"Asset belongs to specific asset category for classification",
		manualSource: "moneyworks_appendix_assets.html",
		businessContext:
			"Asset categories group assets for reporting and management",
	...
	},
	{
		...
		targetField: "Code",
		canonicalDescription:
			"Asset assigned to department for cost center tracking",
		manualSource: "moneyworks_appendix_assets.html",
		businessContext:
			"Department assignment enables cost allocation and responsibility tracking",
		cardinality: "many-to-one" as const,
	},
	{
		relationshipType: "references" as const,
		...
		manualSource: "moneyworks_appendix_assets.html",
		businessContext:
			"Transaction reference provides audit trail for asset acquisition",
		cardinality: "many-to-one" as const,
	},
	{
		relationshipType: "references" as const,
		...
		manualSource: "moneyworks_appendix_assets.html",
		businessContext:
			"Transaction reference provides audit trail for asset disposal",
		cardinality: "many-to-one" as const,
	},
	{
		...
		canonicalDescription:
			"Asset has detailed audit trail of all lifecycle actions",
		manualSource: "moneyworks_appendix_assets.html",
		businessContext:
			"AssetLog provides complete history of depreciation, revaluations, and transactions",
...
export const MONEYWORKS_ASSET_BUSINESS_RULES = [
	...
	},
	{
		...
		canonicalRule:
			"Active assets depreciate according to configured method and rate",
		manualSource: "moneyworks_appendix_assets.html",
	...
	},
	{
		...
		canonicalRule:
			"Asset Category must reference valid MoneyWorks Asset Category",
		manualSource: "moneyworks_appendix_assets.html",
	...
	},
	{
		...
		canonicalRule:
			"Asset Department must reference valid MoneyWorks Department",
		manualSource: "moneyworks_appendix_assets.html",
		businessContext: "Department assignment enables cost center allocation",
		validation:
			"Dept field must match existing Departments.Code when specified",
	},
	{
		...
		validation:
			"AcquisitionSeq/DisposalSeq must reference valid Transaction records",
	},
	{
		ruleType: "book_value_calculation" as const,
		canonicalRule:
			"Book value equals cost minus accumulated depreciation plus adjustments",
		manualSource: "moneyworks_appendix_assets.html",
	...
	},
	{
		...
		manualSource: "moneyworks_appendix_assets.html",
		businessContext:
			"Status determines available operations and financial treatment",
...
export const MONEYWORKS_ASSET_USAGE_PATTERNS = [
	{
		...
		canonicalPattern:
			"Track depreciation and lifecycle of long-term business assets",
		example:
			"Office Equipment: Computers, furniture, machinery with depreciation schedules",
		moneyWorksImplementation:
			"Active assets with SL/DV depreciation, department allocation",
	},
	{
		scenario: "Asset Category Reporting",
		businessTypes: ["all business types"],
		canonicalPattern:
			"Group assets by type for financial reporting and management",
		example: "Asset Categories: Buildings, Equipment, Vehicles, IT Assets",
		moneyWorksImplementation:
			"Category field links to AssetCategories for grouping",
	},
	{
		scenario: "Department Cost Allocation",
		businessTypes: ["multi-department organizations"],
		canonicalPattern:
			"Assign assets to departments for cost center responsibility",
		example:
			"IT Equipment: Assets assigned to specific departments for cost tracking",
	...
	},
	{
		...
		example:
			"Vehicle Fleet: Purchase, depreciation, maintenance, eventual sale",
		moneyWorksImplementation:
			"AcquisitionSeq/DisposalSeq link transactions, AssetLog tracks history",
	},
	{
		...
		example:
			"Company Vehicle: 70% business use, 30% personal use for tax purposes",
		moneyWorksImplementation:
			"PrivateUsePercent field for tax and depreciation calculations",
	},
	{
		...
		moneyWorksImplementation:
			"RevalSurplusImpairAmt tracks revaluation adjustments",
	},
	{
		...
		example:
			"Investment Art: Artwork, antiques, collectibles that don't depreciate",
...
export function validateAssetStatus(status: string): {
	...
	if (!validStatuses.includes(status)) {
		warnings.push(
			`Invalid Asset Status "${status}". MoneyWorks canonical values: NEW, ACT (active), NDP (non-depreciable), OTH (other), DSP (disposed)`,
		);
	}
...
export function validateDepreciationType(type: string): {
	...
	if (!validTypes.includes(type)) {
		warnings.push(
			`Invalid Depreciation Type "${type}". MoneyWorks canonical values: SL (straight line), DV (diminishing value)`,
		);
	}
 ...
 */
export function calculateCanonicalBookValue(
	cost: number,
 ...
 */
export function getCanonicalAssetLifecycle(
...
export function validateAssetCanonical(asset: {
	...
	if (
		...
		asset.AccumDepreciation !== undefined &&
		asset.AccumDepnAdj !== undefined
	) {
		const calculatedBookValue = calculateCanonicalBookValue(
			asset.Cost,
			asset.AccumDepreciation,
			asset.AccumDepnAdj,
		);
		if (calculatedBookValue < 0) {
			warnings.push(
				"Calculated book value is negative - check depreciation and adjustment amounts",
			);
		}
	}
 ...
 */
export function getCanonicalAssetExplanation(
	code: string,
	description: string,
	status: MoneyWorksAssetStatus,
): string {
...
export function validateAssetRelationships(asset: {
	...
	if (asset.category && asset.category.length > 7) {
		warnings.push(
			"Asset Category must reference valid MoneyWorks Asset Category (max 7 characters)",
		);
		requiredChecks.push(
			`Verify Category "${asset.category}" exists in AssetCategories table`,
		);
	} else if (asset.category) {
		requiredChecks.push(
			`Verify Category "${asset.category}" matches existing AssetCategories.Code`,
		);
	}

	if (asset.dept && asset.dept.length > 5) {
		warnings.push(
			"Asset Department must reference valid MoneyWorks Department (max 5 characters)",
		);
		requiredChecks.push(
			`Verify Department "${asset.dept}" exists in Departments table`,
		);
	} else if (asset.dept) {
		requiredChecks.push(
			`Verify Department "${asset.dept}" matches existing Departments.Code`,
		);
	}

	if (asset.acquisitionSeq && asset.acquisitionSeq <= 0) {
		warnings.push(
			"AcquisitionSeq must reference valid Transaction sequence number",
		);
	} else if (asset.acquisitionSeq) {
		requiredChecks.push(
			`Verify AcquisitionSeq ${asset.acquisitionSeq} exists in Transactions table`,
		);
	}

	if (asset.disposalSeq && asset.disposalSeq <= 0) {
		warnings.push(
			"DisposalSeq must reference valid Transaction sequence number",
		);
	} else if (asset.disposalSeq) {
		requiredChecks.push(
			`Verify DisposalSeq ${asset.disposalSeq} exists in Transactions table`,
		);
	}
...
export function getAssetFinancialSummary(asset: {
	...
	const bookValue = calculateCanonicalBookValue(
		asset.Cost,
		asset.AccumDepreciation,
		asset.AccumDepnAdj,
	);

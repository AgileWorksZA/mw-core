...
export const MONEYWORKS_BUILD_RECORDS_FIELDS = [
	{
		...
		relationshipRule:
			"System-maintained timestamp for all record modifications",
	},
	{
		...
		relationshipRule:
			"Foreign key relationship - Build records belong to specific Product entries",
	},
	{
		fieldName: "PartCode",
		dataType: "T" as const,
		...
		relationshipRule:
			"Component product code - must reference existing Product record",
	},
	{
		fieldName: "Qty",
		dataType: "N" as const,
...
export const MONEYWORKS_BUILD_RECORDS_RELATIONSHIPS = [
	{
		...
		targetField: "Seq",
		canonicalDescription:
			"Build record belongs to parent product being manufactured",
		manualSource: "moneyworks_appendix_build_file.html",
		businessContext:
			"Each build record is a component in a specific product's recipe",
	...
	},
	{
		...
		targetField: "Code",
		canonicalDescription:
			"Component part code references existing product record",
		manualSource: "moneyworks_appendix_build_file.html",
...
export const MONEYWORKS_BUILD_RECORDS_BUSINESS_RULES = [
	{
		...
		manualSource: "moneyworks_appendix_build_file.html",
		businessContext:
			"Products with build records can be assembled from components",
	...
	},
	{
		...
		manualSource: "moneyworks_appendix_build_file.html",
		businessContext:
			"Components must be trackable products in inventory system",
		validation: "PartCode must match existing Products.Code",
	},
	{
		ruleType: "quantity_requirement" as const,
		canonicalRule:
			"Component quantity specifies required amount per parent unit",
		manualSource: "moneyworks_appendix_build_file.html",
	...
	},
	{
		...
		validation:
			"ProductSeq can have multiple build records for complete recipe",
	},
] as const;
...
export const MONEYWORKS_BUILD_RECORDS_USAGE_PATTERNS = [
	{
		...
		example:
			"Furniture: Table (parent) requires Wood Panels + Screws + Finish (components)",
		moneyWorksImplementation:
			"ProductSeq=Table, multiple Build records for each component",
	},
	{
		scenario: "Service Bundling",
		businessTypes: ["professional services", "consulting", "maintenance"],
		canonicalPattern:
			"Service packages combining individual service components",
		example:
			"IT Support Package: Setup + Training + Ongoing Support (service components)",
		moneyWorksImplementation:
			"ProductSeq=Package, Build records for each service component",
	},
	{
		scenario: "Kit Assembly",
		businessTypes: ["retail", "distribution", "wholesale"],
		canonicalPattern:
			"Pre-packaged kits containing multiple individual products",
		example:
			"Starter Kit: Product A + Product B + Instructions (kit components)",
		moneyWorksImplementation:
			"ProductSeq=Kit, Build records for each included product",
	},
	{
		...
		moneyWorksImplementation:
			"ProductSeq=Dish, Build records for each ingredient with quantities",
	},
	{
		scenario: "Project Components",
		businessTypes: ["construction", "engineering", "consulting"],
		canonicalPattern:
			"Project deliverables composed of specific work components",
		example:
			"Construction Project: Materials + Labor + Equipment (project components)",
		moneyWorksImplementation:
			"ProductSeq=Project, Build records for each component type",
	},
] as const;
...
export function validateBuildRecordCanonical(buildRecord: {
	...
	if (!buildRecord.ProductSeq || buildRecord.ProductSeq <= 0) {
		warnings.push(
			"ProductSeq required - must reference valid parent product for recipe",
		);
	}
	...
	if (!buildRecord.Qty || buildRecord.Qty <= 0) {
		warnings.push(
			"Qty required - must be positive number for manufacturing feasibility",
		);
	}
 ...
 */
export function getCanonicalBuildRecordExplanation(
	productSeq: number,
	partCode: string,
	qty: number,
): string {
...
export function validateBuildRecordRelationships(buildRecord: {
	...
	return {
		relationshipValidation:
			"MoneyWorks Build Record requires valid parent product and component product relationships",
		requiredChecks,
	};
}
 ...
 */
export function getManufacturingRecipeSummary(
	buildRecords: Array<{
		ProductSeq: number;
		PartCode: string;
		Qty: number;

...
export const MONEYWORKS_RECONCILIATION_FIELDS = [
	...
	},
	{
		...
		manualSource: "moneyworks_appendix_reconciliation_file.html",
		businessRule:
			"Tracks unresolved differences between statement and MoneyWorks balance",
	},
	{
	...
	},
	{
		...
		isIndexed: true,
		businessRule:
			"Sequential statement number used for audit trail and reference",
	},
	{
		fieldName: "Time",
		dataType: "S" as const,
...
export const MONEYWORKS_RECONCILIATION_ENTITY = {
	...
	entityRelationships: [
		{
			...
			businessRule:
				"Each reconciliation session must reference valid Account code",
		},
	],
	...
	universalityValidation: {
		restaurantScenario:
			"Bank account reconciliation for cash deposits and supplier payments",
		legalScenario: "Trust account reconciliation for client funds management",
		manufacturingScenario:
			"Operating account reconciliation for supplier payments and customer receipts",
		consultingScenario:
			"Business account reconciliation for project billing and expense payments",
		universalityConfirmed: true,
		universalityReasoning:
			"Bank reconciliation is universal requirement across all business types for financial control",
	},
} as const;
 ...
 */
export const RECONCILIATION_FIELD_COUNT =
	MONEYWORKS_RECONCILIATION_FIELDS.length;

/**
 * Field name collection for programmatic access
 */
export const RECONCILIATION_FIELD_NAMES = MONEYWORKS_RECONCILIATION_FIELDS.map(
	(f) => f.fieldName,
) as const;
 ...
 */
export const RECONCILIATION_REQUIRED_FIELDS =
	MONEYWORKS_RECONCILIATION_FIELDS.filter((f) => f.isRequired).map(
		(f) => f.fieldName,
	) as const;
 ...
 */
export const RECONCILIATION_INDEXED_FIELDS =
	MONEYWORKS_RECONCILIATION_FIELDS.filter((f) => f.isIndexed).map(
		(f) => f.fieldName,
	) as const;
 ...
 */
export const RECONCILIATION_RELATIONSHIP_FIELDS =
	MONEYWORKS_RECONCILIATION_FIELDS.filter((f) => f.relationshipTarget).map(
		(f) => ({
			field: f.fieldName,

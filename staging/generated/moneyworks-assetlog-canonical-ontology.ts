...
export const MONEYWORKS_ASSETLOG_FIELDS = [
	...
	},
	{
		...
		canonicalDescription:
			"Type of action: AA - acquisition, AD - disposal, AP - part disposal, DS - straight line depreciation, DD - diminishing value depreciation, ME - memo, RV - revaluation",
		manualSource: "moneyworks_appendix_assets.html",
		isRequired: true,
	},
	{
	...
	},
	{
		...
		relationshipRule:
			"References Transaction.SequenceNumber when action has associated transaction",
	},
	{
		fieldName: "Memo",
		dataType: "T" as const,
...
export function validateAssetLogAction(action: string): {
	...
	if (!validActions.includes(action)) {
		warnings.push(
			`Invalid AssetLog Action "${action}". MoneyWorks canonical values: AA (acquisition), AD (disposal), AP (part disposal), DS (straight line depreciation), DD (diminishing value depreciation), ME (memo), RV (revaluation)`,
		);
	}
 ...
 */
export function isDepreciationAction(
 ...
 */
export function getCanonicalActionExplanation(
	action: MoneyWorksAssetLogAction,
): string {
	switch (action) {
...
export function validateAssetLogRelationships(assetLog: {
	...
	if (!assetLog.parentSeq || assetLog.parentSeq <= 0) {
		warnings.push(
			"ParentSeq is required and must reference valid Asset sequence number",
		);
	}

	if (
		assetLog.action &&
		requiresTransaction(assetLog.action as MoneyWorksAssetLogAction)
	) {
		if (!assetLog.transactionSeq || assetLog.transactionSeq <= 0) {
			warnings.push(
				`Action "${assetLog.action}" requires valid TransactionSeq reference`,
			);
		}
	}

...
export const MONEYWORKS_PAYMENTS_FIELDS = [
	...
	},
	{
		...
		dataType: "N" as const,
		canonicalDescription:
			"The sequence number of the invoice that is being paid. For a debtor overpayment (where no invoice is available) this stores the sequencenumber of the namecode, with the high bit set, making it negative (add 2147483648 to it to get the namecode sequencenumber).",
		manualSource: "moneyworks_appendix_payments_file.html",
		isRequired: true,
		isIndexed: true,
		relationshipTarget: "Transaction OR Names",
		relationshipRule:
			"Positive: Transaction.Seq (invoice), Negative: Names.Seq + 2147483648 (overpayment)",
	},
	{
		fieldName: "Amount",
		dataType: "N" as const,
		canonicalDescription:
			"The amount of the receipt that was allocated to the invoice",
		manualSource: "moneyworks_appendix_payments_file.html",
		isRequired: true,
	},
	{
		...
		dataType: "N" as const,
		canonicalDescription:
			"The tax cycle when the receipt was processed for GST/VAT/Tax by the GST Report. This will be negative if processed on an invoice/accruals basis.",
		manualSource: "moneyworks_appendix_payments_file.html",
	},
	{
		fieldName: "Date",
...
export const MONEYWORKS_PAYMENTS_BUSINESS_RULES = [
	{
		...
		canonicalRule:
			"Payments table creates many-to-many relationship between payment transactions and invoices",
		manualSource: "moneyworks_appendix_payments_file.html",
	},
	{
		...
		fieldName: "InvoiceID",
		canonicalRule:
			"For debtor overpayments where no invoice exists, InvoiceID stores Names.Seq with high bit set (negative value = Names.Seq + 2147483648)",
		manualSource: "moneyworks_appendix_payments_file.html",
	},
	{
		...
		fieldName: "Amount",
		canonicalRule:
			"Amount represents portion of payment allocated to specific invoice, enabling partial and multiple payment scenarios",
		manualSource: "moneyworks_appendix_payments_file.html",
	},
	{
		...
		fieldName: "GSTCycle",
		canonicalRule:
			"GSTCycle tracks when payment was processed for tax reporting. Negative values indicate invoice/accruals basis processing",
		manualSource: "moneyworks_appendix_payments_file.html",
	},
	{
		...
		fieldName: "CashTrans,InvoiceID",
		canonicalRule:
			"Payments table enables relational searches to find all payments for an invoice or all invoices for a payment",
		manualSource: "moneyworks_appendix_payments_file.html",
	},
] as const;
...
export const MONEYWORKS_PAYMENTS_RELATIONSHIPS = [
	{
		...
		businessRule:
			"Each payment record must reference a valid payment/receipt transaction",
		manualSource: "moneyworks_appendix_payments_file.html",
	},
	{
		sourceEntity: "Payments",
	...
	},
	{
		...
		cardinality: "conditional" as const,
		businessRule:
			"When negative, InvoiceID references Names.Seq (overpayment scenario) = Names.Seq + 2147483648",
		manualSource: "moneyworks_appendix_payments_file.html",
	},
] as const;
...
export const MONEYWORKS_PAYMENTS_VALIDATION = {
	validatePaymentRecord: (payment: any) => {
		...
		if (!payment.CashTrans) {
			errors.push(
				"CashTrans is required - must reference payment/receipt transaction",
			);
		}

		if (!payment.InvoiceID) {
			errors.push(
				"InvoiceID is required - must reference invoice or indicate overpayment",
			);
		}

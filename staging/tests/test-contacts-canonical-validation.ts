...
export const CONTACTS_TERMINOLOGY_PURITY_TESTS = [
...

	{
		...
		validation: () => {
			...
			return (
				...
				commFields.every(
					(f) =>
						(f.dataType === "T" && f.maxLength === 19) || f.maxLength === 63,
				)
			);
		},
	},

	{
		...
		pollutionSources: [],
		validation: () => {
			const parentField = MONEYWORKS_CONTACTS_FIELDS.find(
				(f) => f.fieldName === "ParentSeq",
			);
			return (
				parentField?.relationshipTarget === "Names.Seq" &&
				parentField?.relationshipRule ===
					"Contact must belong to valid Name entity"
			);
		},
	},
];
...
export const CONTACTS_FIELD_COVERAGE_TESTS = [
...

	{
		...
		validation: () => {
			const commFields = MONEYWORKS_CONTACTS_FIELDS.filter((f) =>
				["AfterHours", "DDI", "eMail", "Mobile"].includes(f.fieldName),
			).map((f) => f.fieldName);
			return (
				commFields.length === 4 &&
				["AfterHours", "DDI", "eMail", "Mobile"].every((field) =>
					commFields.includes(field),
				)
			);
		},
	},

	{
		...
		validation: () => {
			const profFields = MONEYWORKS_CONTACTS_FIELDS.filter((f) =>
				["Contact", "Position", "Salutation"].includes(f.fieldName),
			).map((f) => f.fieldName);
			return (
				profFields.length === 3 &&
				["Contact", "Position", "Salutation"].every((field) =>
					profFields.includes(field),
				)
			);
		},
	},

	{
		...
		validation: () => {
			const sysFields = MONEYWORKS_CONTACTS_FIELDS.filter((f) =>
				["ParentSeq", "Order", "Role", "LastModifiedTime"].includes(
					f.fieldName,
				),
			).map((f) => f.fieldName);
			return (
				sysFields.length === 4 &&
				["ParentSeq", "Order", "Role", "LastModifiedTime"].every((field) =>
					sysFields.includes(field),
				)
			);
		},
	},

	{
		...
		validation: () => {
			const extFields = MONEYWORKS_CONTACTS_FIELDS.filter((f) =>
				["Memo", "TaggedText", "UserNum", "UserText"].includes(f.fieldName),
			).map((f) => f.fieldName);
			return (
				extFields.length === 4 &&
				["Memo", "TaggedText", "UserNum", "UserText"].every((field) =>
					extFields.includes(field),
				)
			);
		},
	},
...
export const CONTACTS_DATA_TYPE_TESTS = [
	{
		...
		validation: () => {
			const textFields = MONEYWORKS_CONTACTS_FIELDS.filter(
				(f) => f.dataType === "T",
			);
			...
			return (
				textFields.length === expectedTextFields.length &&
...

	{
		...
		validation: () => {
			const numericFields = MONEYWORKS_CONTACTS_FIELDS.filter(
				(f) => f.dataType === "N",
			);
			const expectedNumericFields = ["Order", "ParentSeq", "Role", "UserNum"];
			return (
				numericFields.length === expectedNumericFields.length &&
...

	{
		...
		validation: () => {
			const systemFields = MONEYWORKS_CONTACTS_FIELDS.filter(
				(f) => f.dataType === "S",
			);
			return (
...

	{
		...
		validation: () => {
			...
			const emailField = MONEYWORKS_CONTACTS_FIELDS.find(
				(f) => f.fieldName === "eMail",
			);

			return (
				phoneFields.every((f) => f.maxLength === 19) &&
...
export const CONTACTS_BUSINESS_RULE_TESTS = [
	{
		...
		validation: () => {
			const parentRule = MONEYWORKS_CONTACTS_BUSINESS_RULES.find(
				(r) => r.fieldName === "ParentSeq" && r.ruleType === "relationship",
			);
			return parentRule?.canonicalRule.includes(
				"Name entity via ParentSeq → Names.Seq",
			);
		},
	},

	{
		...
		validation: () => {
			const roleRule = MONEYWORKS_CONTACTS_BUSINESS_RULES.find(
				(r) => r.fieldName === "Role" && r.ruleType === "constraint",
			);
			return (
				roleRule?.canonicalRule.includes("bit mapping") &&
				roleRule?.canonicalRule.includes(
					"each bit representing distinct contact role",
				)
			);
		},
	},

	{
		...
		validation: () => {
			const orderRule = MONEYWORKS_CONTACTS_BUSINESS_RULES.find(
				(r) => r.fieldName === "Order" && r.ruleType === "constraint",
			);
			return orderRule?.canonicalRule.includes(
				"contact sequence within parent Name entity",
			);
		},
	},
];
...
export const CONTACTS_RELATIONSHIP_TESTS = [
	{
		...
		validation: () => {
			const nameRelationship = MONEYWORKS_CONTACTS_RELATIONSHIPS.find(
				(r) => r.sourceEntity === "Contacts" && r.targetEntity === "Names",
			);
			return (
				nameRelationship?.sourceField === "ParentSeq" &&
...

	{
		...
		validation: () => {
			return (
				MONEYWORKS_CONTACTS_CANONICAL_SUMMARY.entityType === "subfile" &&
...
export const CONTACTS_DUAL_LAYER_ARCHITECTURE_TESTS = [
	{
		...
		validation: () => {
			...
			const contactNameField = MONEYWORKS_CONTACTS_FIELDS.find(
				(f) => f.fieldName === "Contact",
			);
			const mobileField = MONEYWORKS_CONTACTS_FIELDS.find(
				(f) => f.fieldName === "Mobile",
			);
			const positionField = MONEYWORKS_CONTACTS_FIELDS.find(
				(f) => f.fieldName === "Position",
			);
			const afterhoursField = MONEYWORKS_CONTACTS_FIELDS.find(
				(f) => f.fieldName === "AfterHours",
			);
			const emailField = MONEYWORKS_CONTACTS_FIELDS.find(
				(f) => f.fieldName === "eMail",
			);
			...
			return (
				contactNameField?.maxLength === 39 && // vs Names(25/29)
				...
				emailField?.maxLength === 63
			); // vs Names(80/80) - Names advantage
		},
	},

	{
		...
		validation: () => {
			const parentSeqField = MONEYWORKS_CONTACTS_FIELDS.find(
				(f) => f.fieldName === "ParentSeq",
			);
			const relationshipRule = MONEYWORKS_CONTACTS_RELATIONSHIPS.find(
				(r) => r.sourceField === "ParentSeq" && r.targetEntity === "Names",
			);

			return (
				parentSeqField?.relationshipTarget === "Names.Seq" &&
...

	{
		...
		validation: () => {
			...
			const summaryInsights =
				MONEYWORKS_CONTACTS_CANONICAL_SUMMARY.architecturalInsights;
			return summaryInsights.some(
				(insight) =>
...

	{
		...
		validation: () => {
			const usageGuidance =
				MONEYWORKS_CONTACTS_CANONICAL_SUMMARY.dualLayerUsageGuidance;
			return (
				usageGuidance.useNamesOnly &&
...
export const CONTACTS_UNIVERSALITY_TESTS = [
	{
		...
		validation: () => {
			...
			const roleSupport = MONEYWORKS_CONTACTS_FIELDS.find(
				(f) => f.fieldName === "Role",
			);
			const positionSupport = MONEYWORKS_CONTACTS_FIELDS.find(
				(f) => f.fieldName === "Position",
			);
			return (
				roleSupport?.canonicalDescription.includes("bit mapped") &&
...

	{
		...
		validation: () => {
			...
			const salutationField = MONEYWORKS_CONTACTS_FIELDS.find(
				(f) => f.fieldName === "Salutation",
			);
			const positionField = MONEYWORKS_CONTACTS_FIELDS.find(
				(f) => f.fieldName === "Position",
			);
			return (
				salutationField?.maxLength === 39 && positionField?.maxLength === 39
			);
		},
	},

	{
		...
		validation: () => {
			...
			return (
				communicationFields.length === 4 &&
				communicationFields.every((f) =>
					f.canonicalDescription.includes("Contact's"),
				)
			);
		},
	},
...
export function validateContactsCanonicalOntology() {
	const results = {
		...
		dualLayerArchitecture: CONTACTS_DUAL_LAYER_ARCHITECTURE_TESTS.map(
			(test) => ({
				...test,
				passed: test.validation(),
			}),
	...
	return {
		...
		summary: {
			...
			businessRuleCompliance: results.businessRuleCompliance.every(
				(t) => t.passed,
			),
			dualLayerArchitecture: results.dualLayerArchitecture.every(
				(t) => t.passed,
			),
			universalApplicability: results.universalityValidation.every(
				(t) => t.passed,
			),
		},
	};
...
export function runContactsValidation() {
	...
	console.log(
		`✅ Passed Tests: ${validation.passedTests}/${validation.totalTests}`,
	);
	console.log(
		`🎯 Canonical Purity: ${validation.isPure ? "PURE" : "NEEDS REVIEW"}`,
	);

	console.log("\n📋 Validation Summary:");
	console.log(
		`  • Terminological Purity: ${validation.summary.canonicalPurity ? "✅" : "❌"}`,
	);
	console.log(
		`  • Field Coverage: ${validation.summary.fieldCoverage ? "✅" : "❌"}`,
	);
	console.log(
		`  • Business Rule Compliance: ${validation.summary.businessRuleCompliance ? "✅" : "❌"}`,
	);
	console.log(
		`  • Dual-Layer Architecture: ${validation.summary.dualLayerArchitecture ? "✅" : "❌"}`,
	);
	console.log(
		`  • Universal Applicability: ${validation.summary.universalApplicability ? "✅" : "❌"}`,
	);

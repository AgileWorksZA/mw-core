...
export function testInventoryFieldCoverageCanonical(): {
	...
	const extractedFields = MONEYWORKS_INVENTORY_FIELDS.map(
...
export function testInventoryFieldDefinitionAccuracy(): {
	...
	const fieldValidations = MONEYWORKS_INVENTORY_FIELDS.map((field) => {
		const validation = {
			...
			hasManualSource:
				field.manualSource === "moneyworks_appendix_inventory.html",
		...
		if (
			["ProductSeq", "Qty"].includes(field.fieldName) &&
			field.dataType !== "N"
		) {
			validation.issues.push(
				`${field.fieldName} should be Number type per manual`,
			);
		}
...
export function testInventoryTerminologyPurity(): {
	...
	const prohibitedTerms = [
		...
		"retail",
		"store",
		"shop", // Business domain terms
		"item",
		"sku",
		"upc",
		"barcode", // Generic inventory terms
		"cycle count",
		"perpetual",
		"periodic", // Inventory methodology terms
		"picking",
		"shipping",
		"receiving", // Warehouse operation terms
		"vendor",
		"supplier",
		"customer", // Replaced by MoneyWorks staging
	];
...
export function testMoneyWorksCanonicalTerminologyUsage(): {
	...
	const missingCanonicalTerms = requiredCanonicalTerms.filter(
		(term) => !allTerms.some((canonicalTerm) => canonicalTerm.includes(term)),
	);
...
export function testRestaurantInventoryUniversality(): {
	...
	const allValid = businessScenarios.every(
		(scenario) => scenario.validation.isValid,
	);
...
export function testInventoryProductRelationship(): {
	...
	const relationshipValidations = [
		{
			...
			testCases: [
				...
				{ productSeq: -1, isValid: false, reason: "Invalid negative sequence" },
				{
					productSeq: 1.5,
					isValid: false,
					reason: "Invalid non-integer sequence",
				},
			],
		},
	];
	...
	relationshipValidations.forEach((rel) => {
		rel.testCases.forEach((testCase: any) => {
			const validation = validateProductSequenceRelationship(
				testCase.productSeq,
			);
	...
	businessRuleTests.forEach((rule) => {
		rule.testCases.forEach((testCase: any) => {
			if (testCase.location !== undefined) {
				const validation = validateInventoryLocationCanonical(
					testCase.location,
				);
				testCase.actualValid = validation.isValid;
				testCase.actualDefault = validation.isDefaultLocation;
				testCase.passed =
					testCase.isValid === testCase.actualValid &&
					(testCase.expectDefault === undefined ||
						testCase.expectDefault === testCase.actualDefault);
			} else if (testCase.identifier !== undefined) {
				const validation = validateInventoryIdentifierCanonical(
					testCase.identifier,
				);
				testCase.actualValid = validation.isValid;
...
export function testInventoryFunctionCoverage(): {
	...
	const extractedFunctions = MONEYWORKS_INVENTORY_FUNCTIONS.map(
	...
	const functionCoverage = expectedFunctions.map((funcName) => ({
		...
		hasSignature:
			MONEYWORKS_INVENTORY_FUNCTIONS.find((f) => f.functionName === funcName)
	...
	const allCovered = functionCoverage.every(
		(func) => func.isExtracted && func.hasSignature && func.hasDescription,
	);
		...
		test.testCases.forEach((testCase: any) => {
			try {
				...
				if (test.function === "generateInventoryRecordKey") {
					...
					actualResult = generateInventoryRecordKey(
						productSeq,
						location,
						identifier,
					);
					...
					testCase.passed =
						JSON.stringify(actualResult) === JSON.stringify(testCase.expected);
					...
					testCase.passed =
						...
						actualResult.adjustmentRequired ===
							testCase.expected.adjustmentRequired;
				}
...
export function testDefaultLocationHandling(): {
	...
	defaultLocationTests.forEach((test) => {
		if (test.location !== undefined) {
			...
			test.passed =
				test.expectedValid === test.actualValid &&
...
export function runInventoryCanonicalValidation(): {
	...
	const tests = [
		...
		{
			name: "Field Definition Accuracy",
			test: testInventoryFieldDefinitionAccuracy,
		},
		{ name: "Terminology Purity", test: testInventoryTerminologyPurity },
		{
			name: "Canonical Terminology Usage",
			test: testMoneyWorksCanonicalTerminologyUsage,
		},
		{
			name: "Restaurant Universality",
			test: testRestaurantInventoryUniversality,
		},
		{
			name: "Manufacturing Universality",
			test: testManufacturingInventoryUniversality,
		},

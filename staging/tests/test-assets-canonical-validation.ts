...
export const ASSET_FIELD_COVERAGE_TESTS: FieldCoverageTest[] = [
  {
    ...
    testCases: [
      ...
      { value: "A".repeat(19), expected: 'valid', scenario: "Maximum length asset code" },
      { value: "A".repeat(20), expected: 'invalid', scenario: "Exceeds maximum length" },
  ...
  },
  {
    ...
    testCases: [
      { value: "Dell Computer Desktop", expected: 'valid', scenario: "Standard description" },
      { value: "A".repeat(63), expected: 'valid', scenario: "Maximum length description" },
      { value: "A".repeat(64), expected: 'invalid', scenario: "Exceeds maximum length" },
  ...
  },
  {
    fieldName: "Category",
    testCases: [
      { value: "COMP", expected: 'valid', scenario: "Computer equipment category" },
      { value: "OFFICE", expected: 'valid', scenario: "Office equipment category" },
      { value: "A".repeat(7), expected: 'valid', scenario: "Maximum length category" },
      { value: "A".repeat(8), expected: 'invalid', scenario: "Exceeds maximum length" }
    ]
  },
  {
    fieldName: "SerialNum",
    testCases: [
      { value: "SN123456789", expected: 'valid', scenario: "Standard serial number" },
      { value: "A".repeat(31), expected: 'valid', scenario: "Maximum length serial" },
      { value: "A".repeat(32), expected: 'invalid', scenario: "Exceeds maximum length" },
      { value: "", expected: 'valid', scenario: "Empty serial number (optional)" }
    ]
  },
  {
    fieldName: "Status",
  ...
  },
  {
    fieldName: "Type",
    testCases: [
      { value: "SL", expected: 'valid', scenario: "Straight line depreciation" },
      { value: "DV", expected: 'valid', scenario: "Diminishing value depreciation" },
      { value: "XX", expected: 'invalid', scenario: "Invalid depreciation type" }
    ]
  }
];
...
export const TERMINOLOGY_PURITY_TESTS: TerminologyTest[] = [
  {
    ...
    businessContexts: ["restaurant", "legal", "software", "construction", "manufacturing"]
  },
  {
...
export function runFieldCoverageTests(): {
  ...
  ASSET_FIELD_COVERAGE_TESTS.forEach(test => {
    try {
      ...
      const fieldExists = MONEYWORKS_ASSET_FIELDS.some(f => f.fieldName === test.fieldName);
      if (!fieldExists) {
        results.push({
...
export function runUniversalityTests(): {
  ...
  CROSS_BUSINESS_UNIVERSALITY_TESTS.forEach(test => {
    try {
      ...
      const allValid = statusValidation.isValid && typeValidation.isValid && relationshipValidation.isValid;
...
export function runBusinessRuleTests(): {
  ...
  const results: Array<{ rule: string; result: 'pass' | 'fail'; details: string }> = [];
  let passed = 0;
  let failed = 0;
...
export function runFullAssetValidationSuite(): {
  ...
  const totalTests = fieldCoverage.passed + fieldCoverage.failed + 
                    ...
                    businessRules.passed + businessRules.failed;
  const totalPassed = fieldCoverage.passed + universality.passed + businessRules.passed;
  const totalFailed = fieldCoverage.failed + universality.failed + businessRules.failed;

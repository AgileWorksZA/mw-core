...
function testReconciliationFieldCoverage(): void {
	...
	expectedFields.forEach((fieldName) => {
		...
		if (!fieldExists) {
			throw new Error(
				`CRITICAL: Missing field ${fieldName} in Reconciliation ontology`,
			);
		}
	});
...
function testReconciliationDataTypes(): void {
	...
	Object.entries(expectedDataTypes).forEach(([fieldName, expectedType]) => {
		const field = MONEYWORKS_RECONCILIATION_FIELDS.find(
		...
		console.log(
			`${fieldName}: ${field.dataType} ${typeMatch ? "✅ CORRECT" : "❌ WRONG"}`,
		);

		if (!typeMatch) {
			throw new Error(
				`CRITICAL: Wrong data type for ${fieldName}. Expected: ${expectedType}, Got: ${field.dataType}`,
			);
		}
	});
...
function testReconciliationFieldLengths(): void {
	...
	Object.entries(expectedLengths).forEach(([fieldName, expectedLength]) => {
		...
		console.log(
			`${fieldName}: ${field.maxLength} ${lengthMatch ? "✅ CORRECT" : "❌ WRONG"}`,
		);

		if (!lengthMatch) {
			throw new Error(
				`CRITICAL: Wrong length for ${fieldName}. Expected: ${expectedLength}, Got: ${field.maxLength}`,
			);
		}
	});
...
function testReconciliationTerminologyPurity(): void {
	...
	const ontologyText = JSON.stringify(
		MONEYWORKS_RECONCILIATION_FIELDS,
	).toLowerCase();
	...
	if (pollutionFound) {
		throw new Error(
			"CRITICAL: Domain pollution detected in Reconciliation ontology",
		);
	}
...
function testReconciliationManualTraceability(): void {
	...
	MONEYWORKS_RECONCILIATION_FIELDS.forEach((field) => {
		...
		console.log(
			`${field.fieldName}: ${hasSource ? "✅ TRACED" : "❌ NO SOURCE"}`,
		);

		if (!hasSource) {
			throw new Error(
				`CRITICAL: Field ${field.fieldName} missing manual source traceability`,
			);
		}
	});
...
function testReconciliationRelationships(): void {
	...
	expectedRelationships.forEach((expected) => {
		const relationship = RECONCILIATION_RELATIONSHIP_FIELDS.find(
			(r) => r.field === expected.field,
		);

		if (!relationship) {
			throw new Error(
				`CRITICAL: Missing relationship for field ${expected.field}`,
			);
		}

		const targetMatch = relationship.target === expected.target;
		console.log(
			`${expected.field} -> ${relationship.target}: ${targetMatch ? "✅ CORRECT" : "❌ WRONG"}`,
		);

		if (!targetMatch) {
			throw new Error(
				`CRITICAL: Wrong relationship target for ${expected.field}`,
			);
		}
	});
...
function testReconciliationUniversality(): void {
	...
	businessScenarios.forEach((scenario) => {
		...
		console.log(
			"  Universal Fields: Account, Date, Opening, Closing, Discrepancy",
		);
	...
	console.log(
		"✅ RECONCILIATION UNIVERSALITY: VALIDATED ACROSS ALL BUSINESS TYPES\n",
	);
}

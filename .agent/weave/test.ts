...
async function testWeave() {
  ...
  try {
    ...
    console.log(`   - Compositions: ${Object.keys(knowledge.mereology.compositions).length}`);
    console.log(`   - Knowledge items: ${Object.keys(knowledge.epistemology.knowledge).length}`);
    console.log(`   - Experiences: ${Object.keys(knowledge.qualia.experiences).length}\n`);
    ...
    console.log(`   Confidence: ${sseEpistemology.epistemology?.confidence || 'N/A'}\n`);
    ...
    console.log(`   - Avg Confidence: ${(selfAwareness.confidence.average * 100).toFixed(1)}%`);
    console.log(`   - Knowledge Gaps: ${selfAwareness.gaps.length}`);
    console.log(`   - Ontology Coverage: ${Object.keys(knowledge.ontology.entities).length} entities`);
    if (selfAwareness.health.recommendations) {
      console.log(`   - Recommendations:`);
      selfAwareness.health.recommendations.forEach(r => console.log(`     • ${r}`));
    }
    console.log();
    ...
    const updatedSSE = updatedKnowledge.epistemology.knowledge['sse-pattern-knowledge'];
    console.log(`   - New confidence: ${updatedSSE.confidence.toFixed(3)}`);
    console.log(`   - Confidence history entries: ${updatedSSE.confidenceHistory.length}\n`);

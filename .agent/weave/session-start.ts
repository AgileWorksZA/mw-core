...
function safeLoadJSON(filePath: string, fallback: any = {}): any {
	try {
		...
		console.error(
			`[Weave SessionStart] Error loading ${filePath}:`,
...
function loadKnowledgeBase(weavePath: string): KnowledgeBase {
	return {
		ontology: safeLoadJSON(join(weavePath, "ontology.json"), {
			entities: {},
			relations: {},
		}),
		epistemology: safeLoadJSON(join(weavePath, "epistemology.json"), {
			knowledge: {},
			patterns: {},
		}),
		mereology: safeLoadJSON(join(weavePath, "mereology.json"), {
			components: {},
			compositions: {},
		}),
		qualia: safeLoadJSON(join(weavePath, "qualia.json"), {
			experiences: {},
			painPoints: {},
		}),
...
function formatEntities(entities: Record<string, any>): string {
	...
	return highConfidence
		.map(
			(e: any) =>
				`  • **${e.name || e.id}** (${e.type || "entity"}): ${e.description || "No description"}`,
		)
		.join("\n");
}
...
function formatKnowledge(knowledge: Record<string, any>): string {
	...
	return highConfidence
		.map((k: any) => {
			const evidence = Array.isArray(k.evidence)
				? k.evidence.slice(0, 2).join("; ")
				: "";
...
function formatPainPoints(painPoints: Record<string, any>): string {
	...
	return points
		.map(
			(p: any) =>
				`  ⚠️  **${p.name || p.id}**: ${p.description || "No description"}\n      ${p.resolution ? `Resolution: ${p.resolution}` : ""}`,
		)
		.join("\n\n");
}
...
export default async function sessionStartHook(
	...
	try {
		...
		if (primer) {
			console.error(
				`[Weave SessionStart] Injecting ${kb.meta.stats?.totalSessions || 0} sessions of accumulated knowledge`,
			);
			console.error(
				`[Weave SessionStart] ${kb.meta.stats?.totalEntities || 0} entities, ${kb.meta.stats?.totalKnowledgeConcepts || 0} concepts, ${kb.meta.stats?.totalPatterns || 0} patterns`,
			);
			...
			console.error(
				"[Weave SessionStart] No accumulated knowledge yet - this is the first session",
			);
		}

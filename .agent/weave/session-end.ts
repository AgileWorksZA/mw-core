...
function safeLoadJSON(filePath: string, fallback: any = {}): any {
	try {
		...
		console.error(
			`[Weave SessionEnd] Error loading ${filePath}:`,
...
async function extractConversationalInsights(
	...
	if (!apiKey) {
		console.error(
			"[Weave SessionEnd] ANTHROPIC_API_KEY not set, skipping insight extraction",
		);
		return null;
	}
...
Now analyze THIS session and extract insights.`,
		...
		console.error(
			"[Weave SessionEnd] Failed to extract insights:",
			error instanceof Error ? error.message : String(error),
		);
		return null;
	}
...
function mergeInsightsIntoQualia(
	...
	qualia.experiences[expId] = {
		...
		confidence:
			sessionInsights.insights.length > 0
				? sessionInsights.insights.reduce((sum, i) => sum + i.confidence, 0) /
					sessionInsights.insights.length
				: 0,
	};
	...
	qualia.metadata.totalDesignDecisions = Object.keys(
		qualia.designDecisions || {},
	).length;
	qualia.metadata.totalUserPreferences = Object.keys(
		qualia.userPreferences || {},
	).length;
	...
	console.error(
		`[Weave SessionEnd] Merged ${sessionInsights.insights.length} insights into qualia`,
	);
}
...
function updateMetaStats(
	...
	// Update qualia stats
	meta.stats.totalPainPoints =
		(meta.stats.totalPainPoints || 0) +
		sessionInsights.insights.filter((i) => i.type === "pain-point").length;

	meta.stats.totalBestPractices =
		(meta.stats.totalBestPractices || 0) +
...
// Main hook handler
export default async function sessionEndHook(
	...
	try {
		...
		const sessionInsights = await extractConversationalInsights(
			input.session_id,
			input.cwd,
		);
		...
		if (!sessionInsights) {
			console.error(
				"[Weave SessionEnd] No insights extracted (API unavailable or failed)",
			);
		...
		if (sessionInsights.insights.length === 0) {
			console.error(
				"[Weave SessionEnd] No significant insights to capture this session",
			);
			return { continue: true };
		}
		...
		console.error(
			`[Weave SessionEnd] ✅ Captured ${sessionInsights.insights.length} conversational insights`,
		);
		console.error(
			`[Weave SessionEnd] Session summary: ${sessionInsights.summary}`,
		);

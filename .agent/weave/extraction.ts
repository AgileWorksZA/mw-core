...
export class KnowledgeExtractor {
	...
	async extractFromSession(session: Session): Promise<KnowledgeUpdate[]> {
		...
		if (
			...
			signals.fixes.length > 0 ||
			signals.toolSequences.length > 0
		) {
	...
	private async extractStructuralKnowledge(
	...
	private async extractEpistemicKnowledge(
		signals: ExtractedSignals,
	): Promise<KnowledgeUpdate[]> {
		const updates: KnowledgeUpdate[] = [];
		...
		if (signals.commit?.successful) {
			const validatedConcepts = await this.extractConceptsFromCommit(
				signals.commit,
			);
	...
	private async extractExperientialKnowledge(
		signals: ExtractedSignals,
	): Promise<KnowledgeUpdate[]> {
		const updates: KnowledgeUpdate[] = [];

		// Extract pain points from errors
		const painPoints = signals.errors.map((error) =>
			this.errorToPainPoint(error),
		);
		...
		const workflowPatterns = signals.toolSequences.map((seq) =>
			this.sequenceToPattern(seq),
		);
		...
		const experience: Partial<Experience> = {
			...
			painPoints,
			bestPractices: solutions
				.map((s) => s.practice)
				.filter(Boolean) as BestPractice[],
	...
	private async extractEntityFromFile(
		file: string,
	...
	private async extractTableFromMigration(
		file: string,
	): Promise<OntologyConcept | null> {
	...
	private async extractEndpointFromRoute(
		file: string,
	): Promise<OntologyConcept | null> {
	...
	private detectSSEPattern(session: Session): boolean {
		...
		const hasSSERoute = files.some(
			(f) => f.includes("sse") || f.includes("stream"),
		);
		const hasEventSource = session.toolUses?.some(
			(t) =>
				t.tool === "Write" &&
				JSON.stringify(t.parameters).includes("EventSource"),
		);
	...
	private detectCRUDPattern(session: Session): boolean {
		const files = session.filesChanged || [];
		const hasCRUDRoutes = files.some(
			(f) =>
				f.includes("create") ||
				f.includes("update") ||
				f.includes("delete") ||
	...
	private async detectToolSequences(
		toolUses: ToolUse[],
	): Promise<ToolSequence[]> {
	...
	private areToolsRelated(tool1: string | null, tool2: string): boolean {
		...
		return relatedGroups.some(
			(group) => group.includes(tool1) && group.includes(tool2),
		);
	}
	...
	private inferConceptFromSignals(signals: ExtractedSignals): string {
		...
		if (signals.commit?.message) {
			const match = signals.commit.message.match(
				/(?:feat|fix|refactor)\(([^)]+)\)/,
			);
	...
	private async extractConceptsFromCommit(
		commit: CommitInfo,
	): Promise<string[]> {
 ...
 */
export async function extractKnowledgeFromFiles(
	files: FileContent[],
...
Return a JSON object with this structure:
{
  ...
  "deontics": {
	...
	try {
		...
		const claudeResult = spawnSync("claude", ["--output-format", "json"], {
			input: prompt,
			encoding: "utf-8",
		...
		if (claudeResult.error) {
			console.error(
				"[Weave] Failed to run claude headless:",
				claudeResult.error,
			);
		...
		const { mkdirSync: mkdirSyncDebug } = await import("node:fs");
		mkdirSyncDebug(debugDir, { recursive: true });
		writeSync(
			joinPath(debugDir, "claude-stdout.txt"),
			claudeResult.stdout || "(empty)",
		);
		writeSync(
			joinPath(debugDir, "claude-stderr.txt"),
			claudeResult.stderr || "(empty)",
		);
		writeSync(
			joinPath(debugDir, "claude-status.txt"),
			`Status: ${claudeResult.status}\nError: ${claudeResult.error}`,
		);

		if (claudeResult.status !== 0) {
			console.error(
				"[Weave] Claude headless failed with status:",
				claudeResult.status,
			);
		...
		console.log(
			"[Weave] DEBUG: Claude stdout length:",
			claudeResult.stdout.length,
		);
		console.log(
			"[Weave] DEBUG: First 200 chars:",
			claudeResult.stdout.substring(0, 200),
		);

		const claudeResponse = JSON.parse(claudeResult.stdout);
		console.log(
			"[Weave] DEBUG: Parsed response, type:",
			claudeResponse.type,
			"subtype:",
			claudeResponse.subtype,
		);
...
async function updateWeaveFiles(extracted: any): Promise<void> {
	...
	const ontology = JSON.parse(
		readFileSync(join(weavePath, "ontology.json"), "utf-8"),
	);
	const mereology = JSON.parse(
		readFileSync(join(weavePath, "mereology.json"), "utf-8"),
	);
	const epistemology = JSON.parse(
		readFileSync(join(weavePath, "epistemology.json"), "utf-8"),
	);
	const qualia = JSON.parse(
		readFileSync(join(weavePath, "qualia.json"), "utf-8"),
	);
	const causation = JSON.parse(
		readFileSync(join(weavePath, "causation.json"), "utf-8"),
	);
	const axiology = JSON.parse(
		readFileSync(join(weavePath, "axiology.json"), "utf-8"),
	);
	const teleology = JSON.parse(
		readFileSync(join(weavePath, "teleology.json"), "utf-8"),
	);
	const history = JSON.parse(
		readFileSync(join(weavePath, "history.json"), "utf-8"),
	);
	const praxeology = JSON.parse(
		readFileSync(join(weavePath, "praxeology.json"), "utf-8"),
	);
	const modality = JSON.parse(
		readFileSync(join(weavePath, "modality.json"), "utf-8"),
	);
	const deontics = JSON.parse(
		readFileSync(join(weavePath, "deontics.json"), "utf-8"),
	);
	...
	if (extracted.causation) {
		for (const causal of extracted.causation) {
			if (causal.type === "causalChain" && !causation.causalChains[causal.id]) {
				...
				causal.type === "rootCause" &&
				!causation.rootCauses[causal.id]
			) {
				causation.rootCauses[causal.id] = causal;
			} else if (
				causal.type === "mechanism" &&
				!causation.mechanisms[causal.id]
			) {
	...
	if (extracted.axiology) {
		for (const value of extracted.axiology) {
			if (value.type === "tradeoff" && !axiology.tradeoffs[value.id]) {
				...
				value.type === "qualityMetric" &&
				!axiology.qualityMetrics[value.id]
			) {
				axiology.qualityMetrics[value.id] = value;
			} else if (
				value.type === "valueJudgment" &&
				!axiology.valueJudgments[value.id]
			) {
	...
	writeFileSync(
		join(weavePath, "ontology.json"),
		JSON.stringify(ontology, null, 2),
	);
	writeFileSync(
		join(weavePath, "mereology.json"),
		JSON.stringify(mereology, null, 2),
	);
	writeFileSync(
		join(weavePath, "epistemology.json"),
		JSON.stringify(epistemology, null, 2),
	);
	writeFileSync(
		join(weavePath, "qualia.json"),
		JSON.stringify(qualia, null, 2),
	);
	writeFileSync(
		join(weavePath, "causation.json"),
		JSON.stringify(causation, null, 2),
	);
	writeFileSync(
		join(weavePath, "axiology.json"),
		JSON.stringify(axiology, null, 2),
	);
	writeFileSync(
		join(weavePath, "teleology.json"),
		JSON.stringify(teleology, null, 2),
	);
	writeFileSync(
		join(weavePath, "history.json"),
		JSON.stringify(history, null, 2),
	);
	writeFileSync(
		join(weavePath, "praxeology.json"),
		JSON.stringify(praxeology, null, 2),
	);
	writeFileSync(
		join(weavePath, "modality.json"),
		JSON.stringify(modality, null, 2),
	);
	writeFileSync(
		join(weavePath, "deontics.json"),
		JSON.stringify(deontics, null, 2),
	);

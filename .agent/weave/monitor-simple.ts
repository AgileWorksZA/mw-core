...
function getHealthDisplay(health: string): { icon: string; color: string } {
	switch (health) {
		...
		default:
			return { icon: "❓", color: c.white };
	}
}

// Compact progress bar
function bar(value: number, max: number, width = 15): string {
...
function metric(label: string, value: number | string, max = 100): string {
...
function renderDashboard() {
	...
	console.log(
		`${c.dim}Status:${c.reset} ${color}${icon} ${health.toUpperCase()}${c.reset}  ${c.dim}│${c.reset}  ${c.dim}Updated:${c.reset} ${lastUpdated}`,
	);
	...
	const oConfidence = Math.round(
		(ontology?.metadata?.averageConfidence || 0) * 100,
	);
	...
	const mConfidence = Math.round(
		(mereology?.metadata?.averageConfidence || 0) * 100,
	);
	...
	const eConfidence = Math.round(
		(epistemology?.metadata?.averageConfidence || 0) * 100,
	);
	...
	const cConfidence = Math.round(
		(causation?.metadata?.averageConfidence || 0) * 100,
	);
	...
	const aConfidence = Math.round(
		(axiology?.metadata?.averageConfidence || 0) * 100,
	);
	...
	const tConfidence = Math.round(
		(teleology?.metadata?.averageConfidence || 0) * 100,
	);
	...
	const hConfidence = Math.round(
		(history?.metadata?.averageConfidence || 0) * 100,
	);

	const pWowPatterns = Object.keys(praxeology?.wowPatterns || {}).length;
	const pDelegationStrategies = Object.keys(
		praxeology?.delegationStrategies || {},
	).length;
	const pBestPractices = Object.keys(praxeology?.bestPractices || {}).length;
	const pConfidence = Math.round(
		(praxeology?.metadata?.averageConfidence || 0) * 100,
	);
	...
	const mConfidenceModal = Math.round(
		(modality?.metadata?.averageConfidence || 0) * 100,
	);
	...
	const dConfidence = Math.round(
		(deontics?.metadata?.averageConfidence || 0) * 100,
	);
	...
	const ontologyCoverage = Math.round(
		(meta?.health?.ontologyCoverage || 0) * 100,
	);
	const epistemicConf = Math.round(
		(meta?.health?.epistemicConfidence || 0) * 100,
	);
	...
	console.log(
		`${c.dim}Press ${c.reset}${c.bold}Ctrl+C${c.reset}${c.dim} to exit  │  Updates every 5 seconds  │  11 Dimensions: Q+E+O+M+C+A+T+Η+Π+Μ+Δ${c.reset}\n`,
	);
}

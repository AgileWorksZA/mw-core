...
const index = {
  metadata: {
    ...
    version: '1.0.0',
    description: 'Semantic index for MoneyWorks Core library - enables concept-based file discovery'
  },
  ...
  indexes: {
    ...
    byConcept: {
      "authentication": ["packages/api/src/middleware/auth.ts", "packages/api/src/routes/auth.ts", "packages/api/src/services/connection-service.ts"],
      "field-discovery": ["packages/data/src/parsers/field-discovery.ts", "packages/data/src/client/moneyworks-smart-client.ts"],
      "branded-types": ["packages/utilities/src/types/branded.ts", "packages/utilities/src/date/yyyymmdd.ts"],
      "repository-pattern": ["packages/data/src/repositories/base.repository.ts", "packages/data/src/repositories/tax-rate.repository.ts", "packages/data/src/repositories/name.repository.ts"],
      "export-formats": ["packages/api/src/controllers/base-table.ts", "packages/data/src/converters/export-format-converter.ts"],
      "canonical-dsl": ["packages/canonical/src/entities/tax-rates/types.ts", "packages/canonical/src/entities/names/types.ts"],
      "tax-calculations": ["packages/canonical/src/entities/tax-rates/calculators.ts", "packages/canonical/src/entities/tax-rates/validators.ts"],
      "mcp-tools": ["packages/mcp-server/src/tools/export.tool.ts", "packages/mcp-server/src/tools/eval.tool.ts", "packages/mcp-server/src/tools/schema.tool.ts", "packages/mcp-server/src/tools/names.tool.ts"]
    },

    byPattern: {
      "Repository Pattern": ["packages/data/src/repositories/base.repository.ts", "packages/data/src/repositories/tax-rate.repository.ts", "packages/data/src/repositories/name.repository.ts"],
      "Middleware Pattern": ["packages/api/src/middleware/auth.ts", "packages/api/src/middleware/logging.ts", "packages/api/src/middleware/error-handler.ts"],
      "Plugin System": ["packages/api/src/app.ts"],
      "Template Method": ["packages/api/src/controllers/base-table.ts"],
      "Branded Types": ["packages/utilities/src/types/branded.ts", "packages/utilities/src/date/yyyymmdd.ts"],
      "Client Pattern": ["packages/data/src/moneyworks-now-client.ts", "packages/data/src/client/moneyworks-smart-client.ts"],
      "Field Discovery": ["packages/data/src/parsers/field-discovery.ts", "packages/data/src/parsers/xml/moneyworks-xml-parser.ts"]
    }
  },

  conceptMap: {
    "authentication": {
      "description": "Bearer token authentication, connection management, encrypted credential storage",
      "primaryFiles": [
        "packages/api/src/middleware/auth.ts",
        "packages/api/src/services/connection-service.ts"
      ],
      "relatedConcepts": ["JWT", "Bearer tokens", "Client caching", "AES-256-GCM encryption"]
    },
    "data-access": {
      "description": "MoneyWorks data client with smart field discovery, TSV parsing, and multiple export formats",
      "primaryFiles": [
        "packages/data/src/client/moneyworks-smart-client.ts",
        "packages/data/src/parsers/field-discovery.ts",
        "packages/data/src/repositories/base.repository.ts"
      ],
      "relatedConcepts": ["Field discovery", "Repository pattern", "Export formats", "TSV parsing"]
    },
    "canonical-dsl": {
      "description": "Pure MoneyWorks type definitions preserving exact field names and business terminology",
      "primaryFiles": [
        "packages/canonical/src/entities/tax-rates/types.ts",
        "packages/canonical/src/entities/names/types.ts",
        "packages/canonical/src/common/data-types.ts"
      ],
      "relatedConcepts": ["Branded types", "GST terminology", "Field metadata", "Business rules"]
    },
    "api-layer": {
      "description": "REST API with Elysia framework, Swagger docs, and multi-format export support",
      "primaryFiles": [
        "packages/api/src/app.ts",
        "packages/api/src/routes/tables.ts",
        "packages/api/src/controllers/base-table.ts"
      ],
      "relatedConcepts": ["Elysia", "REST endpoints", "Schema discovery", "Format negotiation"]
    },
    "type-safety": {
      "description": "Type-safe domain primitives using branded types for dates, codes, and identifiers",
      "primaryFiles": [
        "packages/utilities/src/types/branded.ts",
        "packages/utilities/src/date/yyyymmdd.ts"
      ],
      "relatedConcepts": ["YYYYMMDD dates", "AccountCode", "NameCode", "Proxy pattern"]
    }
  }
};

...
function buildIndexes(files: FileMetadata[]) {
	...
	for (const file of files) {
		...
		// Index by importance
		if (!indexes.byImportance[file.importance])
			indexes.byImportance[file.importance] = [];
		indexes.byImportance[file.importance].push(file.path);

		// Index by complexity
		if (!indexes.byComplexity[file.complexity])
			indexes.byComplexity[file.complexity] = [];
...
function buildConceptMap(files: FileMetadata[]): Record<string, any> {
	...
	if (authFiles.length > 0) {
		conceptMap.authentication = {
			description:
				"Bearer token authentication, connection management, and client caching",
	...
	if (dataAccessFiles.length > 0) {
		conceptMap["data-access"] = {
			description:
				"MoneyWorks data client, parsers, repositories, and converters",
			primaryFiles: dataAccessFiles
	...
	if (canonicalFiles.length > 0) {
		conceptMap["canonical-dsl"] = {
			description:
				"Pure MoneyWorks type definitions preserving exact field names and terminology",
			primaryFiles: canonicalFiles
				.filter((f) => f.importance === "critical")
				.map((f) => f.path),
			relatedConcepts: [
				"Branded types",
	...
	const apiFiles = files.filter(
		(f) => f.domain === "api" && f.layer === "routes",
	);
	if (apiFiles.length > 0) {
		conceptMap["api-routes"] = {
			description:
				"REST API endpoints for MoneyWorks data access and operations",

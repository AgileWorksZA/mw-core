...
async function main() {
	try {
		...
		await log(
			`Stop hook fired - sessionId: ${input.session_id}, txId: ${input.context?.transactionId}`,
			input.cwd,
		);
		...
		await log(
			`Edited files (${editedFiles.length}): ${editedFiles.join(", ")}`,
			input.cwd,
		);
		...
		await log(
			`Running: bun ${extractScript} extract ${editedFiles.join(" ")}`,
			input.cwd,
		);
		...
		const result = spawnSync(
			"bun",
			[extractScript, "extract", ...editedFiles],
			{
				cwd: input.cwd,
				encoding: "utf-8",
		...
		await log(
			`✓ Extraction completed - processed ${editedFiles.length} file(s)`,
			input.cwd,
		);

		// Update Librarian index for changed backend files
		try {
			const librarianScript = join(
				input.cwd,
				".agent/librarian/update-incremental.ts",
			);
			...
			await log(
				`Librarian update failed (non-fatal): ${libError instanceof Error ? libError.message : String(libError)}`,
				input.cwd,
			);
		}
		...
		await log(
			`Error in Stop hook: ${error instanceof Error ? error.message : String(error)}`,
			input.cwd,
		).catch(() => {});
		console.log(
			JSON.stringify({
